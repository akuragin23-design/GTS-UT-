...
Процедура ОбновитьДанныеСтрокиПриИзмененииПорядковогоНомераТовара(ТекущаяСтрока, Объект)
	...
	Если ЗначениеЗаполнено(Объект.НомерДекларации) Тогда
		...
		Если ТекущаяСтрока.НомерДляСФ <> НовыйНомерДляСФ Тогда
			...
			//++ASP_СмирновН 13.08.24 000024188	//ASP_Crow 18.03.2026 000029567 В расширение
			//ТекущаяСтрока.НомерГТД = Неопределено;
			//--ASP_СмирновН 13.08.24 000024188
		КонецЕсли;	
	КонецЕсли;
...
Процедура ЗаполнитьПорядковыйНомерТовараВТаблицеТоварыСервер()
	...
	Для каждого СтрокаТовара Из Объект.Товары Цикл
		...
		//++ASP_СмирновН 09.10.24 000024681
		Если СтрокаТовара.Номенклатура.ПрослеживаемыйТовар Тогда  
		//--//ASP_Crow 18.03.2026 000029567 В расширение
		...
		//++//ASP_Crow 18.03.2026 000029567 В расширение	
		Иначе
			СтрокаТовара.ПорядковыйНомерТовара = "";
		КонецЕсли;
		//--ASP_СмирновН 09.10.24 000024681 
		//--//ASP_Crow 18.03.2026 000029567 В расширение
	КонецЦикла;	
...
&НаСервере
Процедура ПерезаполнитьНаСервере(Раздел, Страна) 
	//++ASP_СмирновН 17.07.24 000023868
	Выгрузка = Объект.Товары.Выгрузить();
	//++ASP_Crow 30.08.2024 000024409 Удаляем только строки, выделенного раздела
	МассивУдаления = новый Массив;
	Для каждого СтрТов из Объект.Товары Цикл
		Если СтрТов.НомерРаздела = Раздел Тогда
			МассивУдаления.Добавить(СтрТов);
		КонецЕсли;
	КонецЦикла;
	
	Если МассивУдаления.Количество() = 0 Тогда
		ТолькоДобавление = Истина;
	Иначе
		ТолькоДобавление = Ложь;
	КонецЕсли;
	
	Для каждого СтрУдал из МассивУдаления Цикл
		Объект.Товары.Удалить(СтрУдал);
	КонецЦикла;
	//--ASP_Crow 30.08.2024 000024409
	МассивУникального = новый Массив; 
	ОтсутствующиеСтроки = новый Массив; 
	Если ТолькоДобавление Тогда
		МассивУникального.Добавить(ПоступлениеТоваров);
	Иначе
		Для каждого СтрТовар из Выгрузка Цикл
			Если НЕ СтрТовар.НомерРаздела = Раздел Тогда
				Продолжить
			КонецЕсли;    
			
			Если МассивУникального.Найти(СтрТовар.ДокументПоступления) = Неопределено Тогда
				МассивУникального.Добавить(СтрТовар.ДокументПоступления);	
			КонецЕсли;;
		КонецЦикла;
	КонецЕсли;
	...
	Если МассивУникального.Количество() = 0 тогда
		Запрос.УстановитьПараметр("Ссылка", ПоступлениеТоваров);
	Иначе                                                       
		Запрос.УстановитьПараметр("Ссылка", МассивУникального[0]); // предполагается что программа не даст заполнить разными валютами
	КонецЕсли;
	...
	МассивНеобработанныхСтрок = новый Массив;
	//++ASP_Crow 29.07.2024 000023868 Механизм распределения
	Если МассивУникального.Количество() = 0 Тогда
		Для каждого СтрТов из ПоступлениеТоваров.Товары Цикл
			НовСтрока = Объект.Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтрока, СтрТов);
			НовСтрока.НомерРаздела = Раздел;
		КонецЦикла;
	Иначе
		Для каждого СтрПоступ из МассивУникального Цикл
			
			ТоварыПересчет = СтрПоступ.Товары.ВЫгрузить();
			ТоварыПересчет.Колонки.Добавить("ДокументПоступления");
			//++ASP_Crow 21.10.2024 000024927 Для пересчета в валюту на курс ГТД
			КурсыСтарые = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(Выборка.ВалютаПоступления, СтрПоступ.Дата, ВалютаРегламентированногоУчета);
			//--ASP_Crow 21.10.2024 000024927
			Ценообразование.ПересчитатьСуммыТабличнойЧастиВВалюту(
				ТоварыПересчет, Ложь, Выборка.ВалютаПоступления, Объект.Валюта, КурсыСтарые, КурсыНовые);
			
			Для каждого СтрТов из ТоварыПересчет Цикл		
				#Область СтарыйКод
				////++ASP_Crow 08.08.2024 000023868 ответвление, если ранее уже было заполнено назначение
				//Отбор = новый Структура;
				//Отбор.Вставить("Номенклатура",СтрТов.Номенклатура);
				//Отбор.Вставить("ДокументПоступления",СтрПоступ);
				//Отбор.Вставить("Назначение", СтрТов.Назначение);
				//Если ЗначениеЗаполнено(СтрТов.Назначение) Тогда
				//	СтрокиГТДсНазначением = Выгрузка.НайтиСтроки(Отбор);
				//Иначе
				//	СтрокиГТДсНазначением = новый Массив;
				//КонецЕсли;
				//
				//Если СтрокиГТДсНазначением.Количество() = 1  тогда				
				//	СтрокаГТД = СтрокиГТДсНазначением[0];
				//	Если СтрТов.КоличествоУпаковок = СтрокаГТД.КоличествоУпаковок Тогда
				//		// идеальный вариант, просто указать назначение
				//		НовСтрока = Объект.Товары.Добавить();
				//		ЗаполнитьЗначенияСвойств(НовСтрока, СтрокаГТД,,"НомерСтроки");
				//		НовСтрока.Назначение = СтрТов.Назначение;
				//	ИначеЕсли СтрТов.КоличествоУпаковок < СтрокаГТД.КоличествоУпаковок Тогда
				//		ПроцентПропорции = ОКР(СтрТов.КоличествоУпаковок * 100/СтрокаГТД.КоличествоУпаковок, 4);							
				//		// две строки 1 с назначением, другая без
				//		НовСтрока = Объект.Товары.Добавить();
				//		ЗаполнитьЗначенияСвойств(НовСтрока, СтрокаГТД,,"НомерСтроки");
				//		НовСтрока.Назначение = СтрТов.Назначение;
				//		НовСтрока.КоличествоУпаковок = СтрТов.КоличествоУпаковок;
				//		НовСтрока.Количество = СтрТов.Количество;
				//		НовСтрока.КоличествоПоРНПТ = ?(СтрокаГТД.КоличествоПоРНПТ = 0, 0, СтрТов.КоличествоУпаковок);
				//		НовСтрока.ТаможеннаяСтоимость = СтрокаГТД.ТаможеннаяСтоимость * ПроцентПропорции/100;        
				//		
				//		
				//		ВтораяСтрока = Объект.Товары.Добавить();
				//		ЗаполнитьЗначенияСвойств(ВтораяСтрока, СтрокаГТД,,"НомерСтроки");
				//		ВтораяСтрока.КоличествоУпаковок = СтрокаГТД.КоличествоУпаковок - НовСтрока.КоличествоУпаковок;
				//		ВтораяСтрока.Количество = СтрокаГТД.Количество - НовСтрока.Количество;
				//		ВтораяСтрока.КоличествоПоРНПТ = СтрокаГТД.КоличествоПоРНПТ - НовСтрока.КоличествоПоРНПТ;
				//		ВтораяСтрока.ТаможеннаяСтоимость = СтрокаГТД.ТаможеннаяСтоимость - НовСтрока.ТаможеннаяСтоимость
				...
				//		
				//	КонецЕсли;
				//ИначеЕсли СтрокиГТДсНазначением.Количество() > 1 Тогда
				//	ОстатокРаспределения = СтрТов.КоличествоУпаковок;
				//	Для каждого СтрокаГТД из СтрокиГТДсНазначением Цикл
				//		
				...
				//			ЗаполнитьЗначенияСвойств(НовСтрока, СтрокаГТД,,"НомерСтроки");
				//			НовСтрока.Назначение = СтрТов.Назначение;
				//			ОстатокРаспределения = ОстатокРаспределения - НовСтрока.КоличествоУпаковок;
				//		ИначеЕсли СтрТов.КоличествоУпаковок < СтрокаГТД.КоличествоУпаковок Тогда
				//			ПроцентПропорции = ОКР(СтрТов.КоличествоУпаковок * 100/СтрокаГТД.КоличествоУпаковок, 4);							
				//			// две строки 1 с назначением, другая без
				//			НовСтрока = Объект.Товары.Добавить();
				...
				//	КонецЦикла;
				//КонецЕсли;
				////--ASP_Crow 08.08.2024 000023868
				//
				//Если СтрокиГТДсНазначением.Количество() = 0 Тогда
				//	Отбор = новый Структура;
				//	Отбор.Вставить("Номенклатура",СтрТов.Номенклатура);
				//	Отбор.Вставить("ДокументПоступления",СтрПоступ);
				//					
				//	СтрокиГТД = Выгрузка.НайтиСтроки(Отбор);
				//	
				//	Если СтрокиГТД.Количество() = 1 тогда				
				//		СтрокаГТД = СтрокиГТД[0];
				//		Если СтрТов.КоличествоУпаковок = СтрокаГТД.КоличествоУпаковок Тогда
				//			// идеальный вариант, просто указать назначение
				//			НовСтрока = Объект.Товары.Добавить();
				//			ЗаполнитьЗначенияСвойств(НовСтрока, СтрокаГТД,,"НомерСтроки");
				...
				//			
				//		КонецЕсли;
				//	ИначеЕсли СтрокиГТД.Количество() > 1 Тогда
				//		ОстатокРаспределения = СтрТов.КоличествоУпаковок;
				//		Для каждого СтрокаГТД из СтрокиГТД Цикл
				//			//++ASP_Crow 08.08.2024 000023868 строки у которых есть назначение обрабатывались в другой ветке
				//			Если ЗначениеЗаполнено(СтрокаГТД.Назначение) Тогда
				//				Продолжить
				//			КонецЕсли;                                                                                      
				//			//--ASP_Crow 08.08.2024 000023868
				//			
				...
				//		КонецЦикла;
				//	Иначе
				//		ОтсутствующиеСтроки.Добавить(СтрТов);
				//	КонецЕсли;
				//КонецЕсли;
				#КонецОбласти
				//++ASP_Crow 30.08.2024 000024409 Добавим в проверочный массив
				Отбор = новый Структура;
				Отбор.Вставить("Номенклатура",СтрТов.Номенклатура);
				Отбор.Вставить("ДокументПоступления",СтрПоступ);
				Отбор.Вставить("НомерРаздела", Раздел);
				
				
				СтрокиГТД = Выгрузка.НайтиСтроки(Отбор);
				
				Отбор = Новый Структура("НомерРаздела", Раздел);
				ПоискРаздела = Объект.Разделы.НайтиСтроки(Отбор);
				РазделИзменения = ПоискРаздела[0];
				
				Если СтрокиГТД.Количество() = 0 Тогда
					СтрТов.ДокументПоступления = СтрПоступ;
					МассивНеобработанныхСтрок.Добавить(СтрТов);
					Продолжить
				КонецЕсли;
				//--ASP_Crow 30.08.2024 000024409
				НовСтрока = Объект.Товары.Добавить();
				ЗаполнитьЗначенияСвойств(НовСтрока, СтрТов,,"ВидЗапасов, АналитикаУчетаНоменклатуры");;
				НовСтрока.НомерРаздела = Раздел; 
				НовСтрока.СтранаПроисхождения = Страна;
				НовСтрока.ДокументПоступления = СтрПоступ;
				НовСтрока.ЗакупкаПодДеятельность = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
				НовСтрока.СтавкаНДС = РазделИзменения.СтавкаНДС;
				Если НовСтрока.Номенклатура.ПрослеживаемыйТовар Тогда // ASP_Lebedev заполнение только для прослеживаемых товаров 28/12/24
					НовСтрока.КоличествоПоРНПТ = СтрТов.КоличествоУпаковок;     
				КонецЕсли;				НовСтрока.ХозяйственнаяОперация = СтрПоступ.ХозяйственнаяОперация;
				СумТаможСтоимОбщ = СтрТов.Сумма;
				
				СуммаНДСРаспр = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
				СтрТов.Сумма, СтрТов.СтавкаНДС,
						Ложь);
						
				СуммаПошлины = 0;
				КоличествоПоСтрокам = 0;
				УникальностьГТД = новый Массив;
				УникальностьРНПТ = новый Массив;
				УникальностьНомераДляСФ = новый Массив;
				Для каждого СтрокаГТД из СтрокиГТД Цикл
					
					//СумТаможСтоимОбщ = СумТаможСтоимОбщ + СтрокаГТД.ТаможеннаяСтоимость;
					//СуммаНДСРаспр = СуммаНДСРаспр + СтрокаГТД.СуммаНДС;
					СуммаПошлины = СуммаПошлины + СтрокаГТД.СуммаПошлины;
					КоличествоПоСтрокам = КоличествоПоСтрокам + СтрокаГТД.КоличествоУпаковок; 
					
					Если УникальностьГТД.Найти(СтрокаГТД.НомерГТД) = Неопределено Тогда
						УникальностьГТД.Добавить(СтрокаГТД.НомерГТД);	
					КонецЕсли; 
					
					Если УникальностьРНПТ.Найти(СтрокаГТД.ПорядковыйНомерТовара) = Неопределено Тогда
						УникальностьРНПТ.Добавить(СтрокаГТД.ПорядковыйНомерТовара);	
					КонецЕсли;
					
					Если УникальностьНомераДляСФ.Найти(СтрокаГТД.НомерДляСФ) = Неопределено Тогда
						УникальностьНомераДляСФ.Добавить(СтрокаГТД.НомерДляСФ);	
					КонецЕсли;
				КонецЦикла;                                              
				
				Если УникальностьГТД.Количество() = 1 Тогда
					НовСтрока.НомерГТД = УникальностьГТД[0];
					НовСтрока.ВедетсяУчетПоГТД = истина;
				КонецЕсли;
						
				Если УникальностьРНПТ.Количество() = 1 И НовСтрока.Номенклатура.ПрослеживаемыйТовар Тогда // ASP_Lebedev заполнение только для прослеживаемых товаров 28/12/24
					НовСтрока.ПорядковыйНомерТовара = УникальностьРНПТ[0];
					НовСтрока.ВедетсяУчетПоРНПТ = Истина;
					НовСтрока.КоличествоПоРНПТ = НовСтрока.КоличествоУпаковок;
				КонецЕсли;
				
				Если УникальностьНомераДляСФ.Количество() = 1 Тогда
					НовСтрока.НомерДляСФ = УникальностьНомераДляСФ[0];
				КонецЕсли; 
				
				Если НовСтрока.КоличествоУпаковок = СтрокаГТД.КоличествоУпаковок Тогда
					НовСтрока.СуммаНДС = СуммаНДСРаспр;
					НовСтрока.СуммаПошлины = СуммаПошлины;
					НовСтрока.ТаможеннаяСтоимость = СумТаможСтоимОбщ;
				Иначе
					Пропорция = ОКР(СтрТов.КоличествоУпаковок * 100/КоличествоПоСтрокам, 4)/100;
					НовСтрока.СуммаНДС = СуммаНДСРаспр - СуммаНДСРаспр*Пропорция;
					НовСтрока.СуммаПошлины = СуммаПошлины - СуммаПошлины*Пропорция;
					НовСтрока.ТаможеннаяСтоимость = СумТаможСтоимОбщ - СумТаможСтоимОбщ*Пропорция;	
				КонецЕсли;
	...
	//++ASP_Crow 30.08.2024 000024409 Проверим, есть ли товар в других разделах, если нет, то добавим	
	Если МассивНеобработанныхСтрок.Количество() <> 0 Тогда 
		МассивНомеровРазделов = новый Массив;
		МассивРазрешенныхКДобавлению = новый Массив;
		
		Для каждого СтрРаздел из Объект.Разделы Цикл
			Если СтрРаздел.НомерРаздела = Раздел Тогда
				Продолжить
			КонецЕсли;
			
			Если МассивНомеровРазделов.Найти(СтрРаздел.НомерРаздела) = Неопределено Тогда
				 МассивНомеровРазделов.Добавить(СтрРаздел.НомерРаздела);
			КонецЕсли;
		КонецЦикла;
		
		Для каждого СтрТов из МассивНеобработанныхСтрок Цикл
			МожноДобавить = Ложь;
			Для каждого РазделПров из МассивНомеровРазделов Цикл
				Отбор = новый Структура;
				Отбор.Вставить("Номенклатура",СтрТов.Номенклатура);
				Отбор.Вставить("ДокументПоступления",СтрПоступ);
				Отбор.Вставить("НомерРаздела", РазделПров);
			...
			Если МожноДобавить Тогда
				МассивРазрешенныхКДобавлению.Добавить(СтрТов);
			КонецЕсли;
			
		КонецЦикла;
		
		Если МассивРазрешенныхКДобавлению.Количество() <> 0 Тогда
			
			Отбор = Новый Структура("НомерРаздела", Раздел);
			ПоискРаздела = Объект.Разделы.НайтиСтроки(Отбор);
			РазделИзменения = ПоискРаздела[0];
		    Для каждого СтрТов из МассивРазрешенныхКДобавлению Цикл 
				НовСтрока = Объект.Товары.Добавить();
				ЗаполнитьЗначенияСвойств(НовСтрока, СтрТов,,"ВидЗапасов, АналитикаУчетаНоменклатуры");
				НовСтрока.НомерРаздела = Раздел; 
				НовСтрока.СтранаПроисхождения = Страна;
				НовСтрока.НомерДляСФ = НомерДляСФ(СокрЛП(Объект.НомерДекларации), НовСтрока.ПорядковыйНомерТовара);
				НовСтрока.СтавкаНДС = РазделИзменения.СтавкаНДС; 
				НовСтрока.ВедетсяУчетПоГТД = Истина;
				НовСтрока.ЗакупкаПодДеятельность = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
				Если НовСтрока.Номенклатура.ПрослеживаемыйТовар Тогда // ASP_Lebedev заполнение только для прослеживаемых товаров 28/12/24
					НовСтрока.КоличествоПоРНПТ = СтрТов.КоличествоУпаковок;     
				КонецЕсли;
				НовСтрока.ХозяйственнаяОперация = СтрТов.ДокументПоступления.ХозяйственнаяОперация;
				
				СумТаможСтоимОбщ = СтрТов.Сумма;
				
				СуммаНДСРаспр = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
				СтрТов.Сумма, НовСтрока.СтавкаНДС,
						Ложь);
	...
	Отбор = Новый Структура("НомерРаздела");
	Для Каждого РазделИзм Из Объект.Разделы Цикл
		Отбор.НомерРаздела = РазделИзм.НомерРаздела;
		ТоварыРаздела = Объект.Товары.НайтиСтроки(Отбор);
		Если ТоварыРаздела.Количество() > 0 Тогда
			РазделИзм.ТаможеннаяСтоимость = 0.;
			ПересчитатьПоля(РазделИзм, "СуммаПошлины, СуммаНДС", РазделИзм, Объект.ИспользоватьРазделы);
			Для Каждого Товар Из ТоварыРаздела Цикл
				РазделИзм.ТаможеннаяСтоимость = РазделИзм.ТаможеннаяСтоимость + Товар.ТаможеннаяСтоимость;
				РазделИзм.СуммаПошлины = РазделИзм.СуммаПошлины + Товар.СуммаПошлины;
				РазделИзм.СуммаНДС = РазделИзм.СуммаНДС + Товар.СуммаНДС;
			КонецЦикла;
			ПересчитатьПоля(РазделИзм, "СтавкаПошлины", РазделИзм, Объект.ИспользоватьРазделы);
		КонецЕсли;
	КонецЦикла;
	
	ЗаполнитьЗначенияСвойств(ЭтаФорма, РазностиИтогов(Объект.Разделы, Объект.Товары));
	//--ASP_Crow 30.08.2024 000024409
	//--ASP_Crow 29.07.2024 000023868
	//--ASP_СмирновН 17.07.24 000023868 
...
Процедура Перезаполнить(Команда)
	...
	ПерезаполнитьНаСервере(ТекущийРаздел.НомерРаздела, ТекущийРаздел.СтранаПроисхождения);
	РаспределитьНаТовары("");
