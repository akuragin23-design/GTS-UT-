...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ГТС заполняем колонку ожидается
	...
	Отбор.Вставить("КодСтроки", ВыборкаДетальныеЗаписи.КодСтроки);
	НайденноеЗначение = Объект.Товары.НайтиСтроки(Отбор);
	Для каждого ТекСтрока ИЗ НайденноеЗначение Цикл
	ТекСтрока.Ожидается=ВыборкаДетальныеЗаписи.ЗаказаноОстаток;	
	КонецЦикла;
	КонецЦикла;
	КонецЕсли;
	//ГТС конец
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//++ASP_Crow 11.03.2026 000029567 в расширение
	//++ASP_СмирновН 09.12.25 000028820 проверка
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
	ТекущийСклад = Объект.Склад;
	Отбор = Новый Структура("Склад",ТекущийСклад);
	НайденныеСтроки = Объект.Товары.НайтиСтроки(Отбор);
	   Если НайденныеСтроки.Количество() <> Объект.Товары.Количество() Тогда		
			Для каждого СтрТов из Объект.Товары Цикл			
			СтрТов.Склад = ?(СтрТов.Склад <> ТекущийСклад, ТекущийСклад, СтрТов.Склад);			
			КонецЦикла;	
		КонецЕсли;
	КонецЕсли;		
    //--ASP_СмирновН 09.12.25 000028820
	//--ASP_Crow 11.03.2026 000029567 в расширение
...
Процедура ЗаполнитьАдресДоставкиДляПоставщика()
	//ASP_Crow 20.03.2026 000029567 В расширение
	Если Не СкладГруппа Тогда
...
#КонецОбласти
 ...
 #Область ASP_КомандыФормы
&НаКлиенте
Процедура ASP_ПеренестиТоварВДругойЗаказ(Команда)
			Ответ = Вопрос("В выбранный заказ дозапишутся товары из текущего заказа. Вы уверены?",РежимДиалогаВопрос.ДаНет,0,КодВозвратаДиалога.Да,);
	Если Ответ = КодВозвратаДиалога.Да Тогда
	ЗаказПоставщику = ОткрытьФормуМодально("Документ.ЗаказПоставщику.ФормаВыбора");
	ASP_ПеренестиТоварВДругойЗаказНаСервере(ЗаказПоставщику);
	Иначе
	КонецЕсли;
...
&НаСервере
Процедура ASP_ПеренестиТоварВДругойЗаказНаСервере(ЗаказПоставщику)
	ОбъектЗаказ=ЗаказПоставщику.ПолучитьОбъект();
	Таблица1=ОбъектЗаказ.Товары.Выгрузить();
	Таблица2=Объект.Товары.Выгрузить();
	Для каждого Стр из Таблица2 Цикл
		Стр.Кодстроки = неопределено;
		Стр.НомерСтроки = неопределено;
	КонецЦикла;
	...
	Для a = 1 По Таблица1.Количество() Цикл ОбъединениеТаблиц.Вставить(0) 
	КонецЦикла;
	Для a = 0 По Таблица1.Колонки.Количество()-1 Цикл
	ОбъединениеТаблиц.ЗагрузитьКолонку(Таблица1.ВыгрузитьКолонку(a), a)
	КонецЦикла;

	ОбъектЗаказ.Товары.Загрузить(ОбъединениеТаблиц);
	ОбъектЗаказ.Записать(РежимЗаписиДокумента.Проведение);
КонецПроцедуры

&НаКлиенте
Процедура ASP_ЗаполнитьДатуПоступленияДляВыделенныхСтрок(Команда)
...
&НаКлиенте
Процедура ASP_ЗаполнитьНомерЦиклаПоНазначениям(Команда)
	Оповещение = Новый ОписаниеОповещения("ПослеВводаНомераЦикла", ЭтотОбъект);	
 
    ПоказатьВводЗначения(Оповещение, , "Введите номер цикла","Число"); 

КонецПроцедуры

&НаКлиенте
Процедура ПослеВводаНомераЦикла(Число, Параметры) Экспорт
 
    Если Не Число = Неопределено Тогда
        ASP_ЗаполнитьНомерЦиклаПоНазначениямНаСервере(Число);		
    КонецЕсли;
    
КонецПроцедуры

&НаКлиенте
Процедура ASP_ОповеститьОИзменениях(Команда)
	ПараметрыПисьма=ОповеститьОИзмененияхНаСервере(1);
	 	
	ОткрытьФорму("Документ.ЭлектронноеПисьмоИсходящее.Форма.ФормаДокумента", ПараметрыПисьма, , , , , );
КонецПроцедуры                                                                                          

&НаСервере
Функция ОповеститьОИзмененияхНаСервере(типписьма)
	
	ПараметрыПисьма = Новый Структура;
	Если типписьма=1 Тогда
	Текст= "В 1С изменен срок поставки по:" 
	Иначе 
	Текст= "В 1С установлен срок поставки по:"
	КонецЕсли;
	Текст= Текст + Символы.ПС +
	Объект.ссылка + Символы.ПС +
	//"Поставщик: " + Объект.партнер + Символы.ПС +
	"Склад: " + Объект.склад + Символы.ПС+"Количество, Товар, Заказ клиента, Менеджер, Дата поставки"+Символы.ПС;
	
	//"Статус: " + Объект.статус + Символы.ПС +
	//"Комментарий: " + Объект.комментарий + Символы.ПС +  Символы.ПС;
	...
	Если типписьма=1 Тогда
	ПараметрыПисьма.Вставить("Тема", "Оповещение 1С: Изменён срок поставки по " + Объект.ссылка);
	Иначе 
	ПараметрыПисьма.Вставить("Тема", "Оповещение 1С: Установлен срок поставки по " + Объект.ссылка);
	КонецЕсли;
		Менеджер=Новый Массив;
		//Текст=Текст+"БЫЛО"+Символы.ПС;
	   	Разница=РазницаТаблицЗначений(Объект.Ссылка.Товары.Выгрузить(), Объект.Товары.Выгрузить(), "Номенклатура Возр,Количество Возр,Назначение Возр,ДатаПоступления Возр");
			Для каждого СТР из Разница Цикл
			Если Стр.Знак=0 тогда
				//Текст= Текст + Стр.Количество  + Символы.Таб + "шт." + Символы.Таб + Стр.Номенклатура + Символы.Таб + Стр.Назначение + Символы.Таб + Формат(Стр.ДатаПоступления, "ДЛФ=Д") + Символы.ПС;
				если ЗначениеЗаполнено(СТР.Назначение) тогда
					попытка
					Менеджер.Добавить(Стр.Назначение.Заказ.Менеджер);
					исключение 
					конецпопытки;
				иначе
			...
			КонецЕсли
		КонецЦикла;
	//	Текст=Текст+"СТАЛО"+Символы.ПС;
		Для каждого СТР из Разница Цикл
			Если Стр.Знак=0 тогда
				ДатаПоставкиСтарая=Стр.ДатаПоступления;
				НазначениеСтарое=Стр.Назначение;
				НоменклатураСтарая=Стр.Номенклатура;  
			КонецЕсли;
			Если Стр.Знак=1 тогда
				Текст= Текст + Стр.Количество  + Символы.Таб + "шт." + Символы.Таб + Стр.Номенклатура + Символы.Таб + Символы.Таб;
				если ЗначениеЗаполнено(СТР.Назначение) тогда
					попытка
					Текст=Текст+строка(Стр.Назначение.Заказ.Номер)+Символы.Таб+строка((Стр.Назначение.Заказ.Менеджер))+Символы.Таб;
					исключение 
					конецпопытки;
				иначе
				Текст=Текст+Символы.Таб+Символы.Таб+Символы.Таб;
				КонецЕсли;                                      
				Если Стр.ДатаПоступления<>ДатаПоставкиСтарая И Стр.Назначение=НазначениеСтарое и Стр.Номенклатура= НоменклатураСтарая и ЗначениеЗаполнено(ДатаПоставкиСтарая) Тогда
				Текст=Текст+Символы.Таб + "! Было: " + Формат(ДатаПоставкиСтарая, "ДЛФ=Д") + " Стало: " + Формат(Стр.ДатаПоступления, "ДЛФ=Д") + " !" + Символы.ПС;
				Иначе
				Текст=Текст+Символы.Таб + "! " + Формат(Стр.ДатаПоступления, "ДЛФ=Д") + " !" + Символы.ПС;
				КонецЕсли;
			КонецЕсли
		КонецЦикла;
...
//Построитель.ИсточникДанных=Новый ОписаниеИсточникаДанных(Разница);       
//Построитель.Вывести(ТабДокумент);
Возврат ПараметрыПисьма;
КонецФункции

 &НаКлиенте
Процедура ASP_ОповеститьОПростановкеСроков(Команда)
	ПараметрыПисьма=ОповеститьОИзмененияхНаСервере(2);
...
Функция РазницаТаблицЗначений(Таблица0, Таблица1, Измерения) Экспорт
	...
	КонецЦикла;
	ВсеКолонки	= "Номенклатура, Количество, Назначение, ДатаПоступления";
...
&НаСервере
Процедура ASP_ЗаполнитьНомерЦиклаПоНазначениямНаСервере(НомерЦикла)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЗаказПоставщикуТовары.Назначение.Заказ ССЫЛКА Документ.ЗаказКлиента
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	Заказ=ВыборкаДетальныеЗаписи.НазначениеЗаказ.ПолучитьОбъект();
	Если НЕ значениезаполнено(Заказ.НомерЦикла) Тогда
	Заказ.НомерЦикла=НомерЦикла;
	Заказ.Записать(РежимЗаписиДокумента.Проведение);
	КонецЕсли;
	КонецЦикла;
	сообщить("Номер цикла " + строка(НомерЦикла) + " установлен по " + строка(ВыборкаДетальныеЗаписи.Количество()) + " назначениям. Если установлен был ранее - не изменен.");
	...
	Если ВыбраннаяДата <> Неопределено И ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		Если МассивВыделенныхСтрок = Неопределено Тогда
				...
				Для Каждого СтрокаТЧ Из Объект.Товары Цикл
					...
					СтрокаТЧ.ДатаОтгрузки=ДатаПоступления;
				КонецЦикла;
...
&НаКлиенте
Процедура ASP_ЗаполнитьЦеныПоВидуЦен(Команда)
		
	ВидЦен = "";
	ПоказатьВводЗначения(Новый ОписаниеОповещения("ЗаполнитьЦеныПоВидуЦенЗавершение", ЭтотОбъект, Новый Структура("ВидЦен", ВидЦен)), ВидЦен,,Тип("СправочникСсылка.ВидыЦен"));	
	
КонецПроцедуры 

&НаКлиенте
Процедура ЗаполнитьЦеныПоВидуЦенЗавершение(Значение, ДополнительныеПараметры) Экспорт
	
	ВидЦен = ?(Значение = Неопределено, ДополнительныеПараметры.ВидЦен, Значение);
	
	
	Если (Значение <> Неопределено) Тогда
		ЗаполнитьЦеныПоВидуЦенСервер(ВидЦен);
...
КонецФункции // ЗаполнитьЦеныПоВидуЦенСервер()

&НаКлиентеНаСервереБезКонтекста
Процедура РассчитатьИтогиСтрокЗаказа(Знач Товары,
	                                 Знач ЦенаВключаетНДС,
	                                 СуммаЗаказано = Неопределено,
	                                 СуммаНДСЗаказанныхСтрок = Неопределено,
	                                 СуммаОтменено = Неопределено,
	                                 СуммаНДСОтмененныхСтрок = Неопределено,
	                                 КоличествоЗаказанныхСтрок = Неопределено,
	                                 КоличествоОтмененныхСтрок = Неопределено)
	...
	Если КоличествоОтмененныхСтрок > 0 Тогда
		...
		Для Каждого ТекСтрока Из НайденныеСтроки Цикл
			...
			НоваяСумма    = НоваяСумма + ТекСтрока.Сумма;
			НоваяСуммаНДС = НоваяСуммаНДС + ТекСтрока.СуммаНДС;
			
		КонецЦикла;
		
		Если Не ЦенаВключаетНДС Тогда
			НоваяСумма = НоваяСумма + НоваяСуммаНДС;
		КонецЕсли;
		
		Если НоваяСумма <> СуммаОтменено Тогда
			СуммаОтменено = НоваяСумма;
		КонецЕсли;
		
		Если НоваяСуммаНДС <> СуммаНДСОтмененныхСтрок Тогда
			СуммаНДСОтмененныхСтрок = НоваяСуммаНДС;
		КонецЕсли;
		...
		Если Товары.Количество() <> КоличествоОтмененныхСтрок Тогда
			...
			Если Не ЦенаВключаетНДС Тогда
				СуммаЗаказано = Товары.Итог("Сумма") + Товары.Итог("СуммаНДС") - СуммаОтменено;
			Иначе
				СуммаЗаказано = Товары.Итог("Сумма") - СуммаОтменено;
			КонецЕсли;
	...
	Иначе
		...
		СуммаЗаказано             = Товары.Итог("Сумма");
		СуммаНДСЗаказанныхСтрок   = Товары.Итог("СуммаНДС");
		
		Если Не ЦенаВключаетНДС Тогда
			СуммаЗаказано = СуммаЗаказано + СуммаНДСЗаказанныхСтрок;
		КонецЕсли;
		
	КонецЕсли;
	
КонецПроцедуры // РассчитатьИтогиСтрокЗаказа()

&НаСервере
// Осуществляет заполнение видов цен и цен в тч Товары по виду цен
//
// Параметры:
// ДокументПродажи                   - ДокументОбъект, для которого необходимо заполнить цены
// ИмяТабличнойЧасти                 - Строка - имя табличной части, в которой необходимо заполнить цены
// ВидЦен                            - СправочникСсылка.ВидыЦен - Вид цен, по которому необходимо заполниь цены
// ИспользоватьАвтоматическиеСкидки  - Булево - признак, определяющий необходимость перезаполнения автоматических скидок
// ИспользоватьРучныеСкидки          - Булево - признак, определяющий необходимость перезаполнения ручных скидок
// ЗаполнятьСтавкиНДС                - Булево - признак, определяющий необходимость заполнения сумм НДС
//
// Возвращаемое значение:
// Булево.
// Истина, если цены заполнены, ложь в противном случае
//
Функция ЗаполнитьЦеныПоВидуЦенНаСервере(Знач ИмяТабличнойЧасти,
	                           Знач ВидЦен,
	                           ИспользоватьРучныеСкидки = Истина) Экспорт
	...
	// Сформируем текст запроса необходимых цен и заполним параметры запроса
	
	ТекстЗапроса     = "";
	ПараметрыЗапроса = Новый Структура();
	
	СформироватьЗапросВременнаяТаблицаТовары(ТекстЗапроса);
	СформироватьЗапросЦеныПрайсЛиста(ТекстЗапроса);
	
	ПараметрыЗапроса.Вставить("Товары", Объект[ИмяТабличнойЧасти].Выгрузить(, "НомерСтроки,Номенклатура,Характеристика,Упаковка"));
	ПараметрыЗапроса.Вставить("Дата",   ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДата()));
	ПараметрыЗапроса.Вставить("Валюта", Объект.Валюта);
	ПараметрыЗапроса.Вставить("ВидЦен", ВидЦен);
	...
	// Выполним запросы и заполним цены в табличной части документа
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	ТаблицаЦенПрайсЛиста = РезультатЗапроса[1].Выгрузить();
	ТаблицаЦенПрайсЛиста.Индексы.Добавить("НомерСтроки");
	
	Если ТаблицаЦенПрайсЛиста.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого ТекСтрока Из Объект[ИмяТабличнойЧасти] Цикл
		...
		НайденнаяСтрока = ТаблицаЦенПрайсЛиста.Найти(ТекСтрока.НомерСтроки,"НомерСтроки");
...
// ДокументПродажи  - ДокументОбъект, к таблицам которого формируется запрос
//
Процедура СформироватьЗапросВременнаяТаблицаТовары(ТекстЗапроса)
...
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса
//
Процедура СформироватьЗапросЦеныПрайсЛиста(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ВременнаяТаблицаТовары.Упаковка.Числитель / ВременнаяТаблицаТовары.Упаковка.Знаменатель  КАК КоэффициентУпаковки,
		|	ВЫБОР
		|		КОГДА
		...
		|			1
		|	КОНЕЦ
		|	* ЦеныНоменклатурыСрезПоследних.Цена/ЕстьNULL(ЦеныНоменклатурыСрезПоследних.Упаковка.Числитель/ЦеныНоменклатурыСрезПоследних.Упаковка.Знаменатель,1)
		|	* ВЫБОР
