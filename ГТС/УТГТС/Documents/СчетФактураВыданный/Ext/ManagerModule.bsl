...
Процедура ЗаполнитьПлатежноРасчетныеДокументы(ПлатежноРасчетныеДокументы, МассивОснований, Организация, ДатаСФ, АктулизироватьРасчеты = Истина) Экспорт
	...
	Если МассивАналитикУчетаПоПартнерам.Количество() > 0 Тогда 
		Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") И АктулизироватьРасчеты Тогда
...
Процедура СформироватьВременнуюТаблицуТоваровДляПечати(МенеджерВременныхТаблиц, МассивОснований, ПараметрыПечати) Экспорт
	...
	//++ASP_Crow 10.03.2026 000029567 Поместить в расширение
	//++ASP_Crow 27.11.2024 000025328 Округление до 8 символов не дает сгруппировать корректно меняем на до 2
	//|	КОНЕЦ КАК ЧИСЛО(21,8))                                   КАК Цена,                                   
	|	КОНЕЦ - 0.005 КАК ЧИСЛО(21,2))                                   КАК Цена,                                   
	//--ASP_Crow 27.11.2024 000025328    
	//--ASP_Crow 10.03.2026 000029567 Поместить в расширение
	...
	//++ASP_Crow 10.03.2026 000029567 Поместить в расширение
	//++ASP_Crow 27.11.2024 000025328 Чтобы формировалось по старой логике со свёрткой
	//ПараметрыПечати.ВыводитьНаборы = ЕстьНаборы;
	ПараметрыПечати.ВыводитьНаборы = Ложь;
	//--ASP_Crow 27.11.2024 000025328	//--ASP_Crow 10.03.2026 000029567 Поместить в расширение
...
Функция ТекстЗапросаДанныхПрослеживаемыеКомплектующие()
	...
	//ТекстЗапросаТекущийНа 09.07.2026, был с ошибкой
	...
	//Текст Запроса Типовая УТ 11.5.25.105
	ТекстЗапроса = "
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ЗаполнитьТаблицуПлатежноРасчетныхДокументовПоВыборке(ПлатежноРасчетныеДокументы, ДанныеВыборки)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.НомерСчетаФактурыНаАванс = Выборка.СчетФактураАванс.Номер; //ASP_Crow 01.06.2026 000030574
	КонецЦикла;
