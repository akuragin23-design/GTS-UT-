...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДокОснование) Тогда
		...
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
			...
			ЗаполнитьПоОперацииПоПлатежнойКарте(
				ДанныеЗаполнения,
				ДанныеЗаполнения);
		//ГТС 04.09.2017	
		//--ASP_Crow 11.03.2026 000029567 в расширение	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Альтер ЛОГО+
	Если Не Отказ Тогда
		// изменим стутус сделки
		Для каждого СтрокаТЗ Из Движения.РасчетыСКлиентамиПоДокументам Цикл
			Если ЗначениеЗаполнено(СтрокаТЗ.УдалитьЗаказКлиента) Тогда	
				Попытка
					Если ЗначениеЗаполнено(СтрокаТЗ.УдалитьЗаказКлиента.Сделка) Тогда
						обСделка = СтрокаТЗ.УдалитьЗаказКлиента.Сделка.ПолучитьОбъект();
						Если обСделка.Статус = Перечисления.СтатусыСделок.ВРаботе ИЛИ обСделка.Статус = Перечисления.СтатусыСделок.Проиграна Тогда
							обСделка.Статус = Перечисления.СтатусыСделок.Выиграна;
							обСделка.Закрыта = Истина;
		                    обСделка.ПричинаПроигрышаСделки = Справочники.ПричиныПроигрышаСделок.ПустаяСсылка();
							обСделка.Записать();
						КонецЕсли;
					КонецЕсли;
				Исключение
...
Процедура ЗаполнитьПоЗаказуКлиента(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	//++ASP_Crow 11.03.2026 000029567 в расширение
	СуммаКОплате = 0;//ASP_Dante 31/08/19
...
Процедура ЗаполнитьПоОперацииПоПлатежнойКарте(Знач ДокументОснование, ДанныеЗаполнения) 
	//++ASP_Crow 11.03.2026 000029567 в расширение
		...
		Комментарий = ДанныеЗаполнения.Комментарий + " " + строка(ДокументОснование);
		Контрагент = Справочники.Контрагенты.НайтиПоНаименованию("РЕБ",истина,,);
		...
		БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.НайтиПоНаименованию("РЕБ",истина,,);
