...
Процедура РасшифровкаПлатежаЗаявкаПриИзмененииСервер(ЗаявкаСсылка)
	...
	Если ТипЗнч(ЗаявкаСсылка) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
	   ...
	   //++ASP_Crow 10.03.2026 000029567 Поместить в расширение
	    //asp+ непп 150324
		КонтрагентДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаявкаСсылка, "Договор");
         Объект.Договор = КонтрагентДоговор;
         //asp+
		Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
	...
	//++ASP_Crow 03.06.2026 000030624 Заполняем расшифровку
	Если СтрокаРасшифровки <> Неопределено Тогда
		Если НЕ ЗначениеЗаполнено(СтрокаРасшифровки.ОбъектРасчетов) Тогда
			СтрокаРасшЗаявки = СтрокаРасшифровки.ЗаявкаНаРасходованиеДенежныхСредств.РасшифровкаПлатежа[0]; //Нет заявок где 0 строк, нет заявок больше 1 строки
			СтрокаРасшифровки.ОбъектРасчетов = СтрокаРасшЗаявки.ОбъектРасчетов;	
		КонецЕсли;
	КонецЕсли;  
	//--ASP_Crow 03.06.2026 000030624
