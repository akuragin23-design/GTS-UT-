...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	КонецЕсли;
	//++ASP_Crow 10.03.2026 000029567 Переместить в расширение
	// Альтер ЛОГО+:	Изменено для определения кассира не по пользователю, а по данным организации
	Попытка
		ЗаполнитьКассира(ДанныеЗаполнения.Организация);
	Исключение
	КонецПопытки;
	// +Альтер ЛОГО                                           
	//--ASP_Crow 10.03.2026 000029567 Переместить в расширение
...
Процедура ЗаполнитьПоЗаказуКлиента(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	//++ASP_Crow 10.03.2026 000029567 Переместить в расширение
	// Альтер ЛОГО+: заполняем поле для вывода на печать
...
// Альтер ЛОГО+:	Заполнение поля кассира при заполнении
Процедура ЗаполнитьКассира(СсылкаОрганизация) Экспорт       
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Если ЗначениеЗаполнено(СсылкаОрганизация.ТекущийКассир) Тогда
		ЗапросКассир = Новый Запрос;
		ЗапросКассир.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Пользователи.Ссылка
		|ИЗ
		...
		ЗапросКассир.УстановитьПараметр("ФизическоеЛицо", СсылкаОрганизация.ТекущийКассир);
		Выборка = ЗапросКассир.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			Кассир = Выборка.Ссылка;
		КонецЕсли;		
	Иначе
		Кассир = "";
	КонецЕсли;
