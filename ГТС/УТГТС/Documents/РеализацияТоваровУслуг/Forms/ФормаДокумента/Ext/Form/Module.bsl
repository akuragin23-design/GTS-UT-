...
Процедура ПриОткрытии(Отказ)
	...
	//ГТС 30.06.2019 ограничиваем изменения после установки отметки склада
	УстановитьДоступностьОтметкиСклада();
	
	//ГТС
	Элементы.СпособДоставки.Видимость = Истина;

	//ASP_Hlopkov 21.09.21+
	УстановитьДоступностьУстановкиФактаПолученияДокументов();
	УстановитьДоступностьASP_УбратьОтметкуСклада();
	//ASP_Hlopkov 21.09.21-  
	
	//ASP=
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеЗаполнено(Объект.ЗаказКлиента) И НЕ ЗначениеЗаполнено(Объект.ЗаявкаНаТранспорт) Тогда
		врЗаявкаНаТранспорт = НайтиЗаявкуНаТранспортНаСервере();
		Если ЗначениеЗаполнено(врЗаявкаНаТранспорт) Тогда
			//ГТС 20062019, ставим по умолчанию да, так как не всегда правильно нажимают
			//Ответ = Вопрос("Найдена заявка на транспорт, связанная с заказом клиента. Хотите 'связать' заявку с текущим документом реализации?",
			//				РежимДиалогаВопрос.ДаНет);
			//Если Ответ = КодВозвратаДиалога.Да Тогда
				Объект.ЗаявкаНаТранспорт = врЗаявкаНаТранспорт;
				ЗаполнитьПоЗаявкеНаТранспортНаСервере();
			//КонецЕсли;					
		КонецЕсли;	
	КонецЕсли;	
	//--ASP_Crow 10.03.2026 000029567 в расширение
...
Процедура СоглашениеПриИзмененииСервер()
	...
	//++ASP_Crow 10.03.2026 000029567 в расширение (закоментить)
...
Процедура ИзменениеМенеджераСервер()  
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Объект.Отпустил = Объект.Менеджер.ФизическоеЛицо;
	Объект.Подразделение = Объект.Менеджер.Подразделение;
КонецПроцедуры

// Альтер ЛОГО+
&НаСервере
Функция ПолучитьЗаявкиНаСервере()  
	//++ASP_Crow 10.03.2026 000029567 в расширение
	
	Если НЕ Объект.Проведен ИЛИ НЕ ЗначениеЗаполнено(Объект.ЗаказКлиента) Тогда
		Возврат Неопределено;
	КонецЕсли;
	// надо предложить пользователю установить документ реализации в заявке на услуги
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗаявкаНаУслуги.Ссылка,
	|	ЗаявкаНаУслуги.Услуга
	|ИЗ
	|	Документ.ЗаявкаНаУслуги КАК ЗаявкаНаУслуги
	|ГДЕ
	|	ЗаявкаНаУслуги.ЗаказКлиента = &ЗаказКлиента
	|	И (НЕ ЗаявкаНаУслуги.ПометкаУдаления)
	|	И ЗаявкаНаУслуги.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаУслуги.Запланировано)
	|	И ЗаявкаНаУслуги.ДокументРеализации = ЗНАЧЕНИЕ(Документ.РеализацияТоваровУслуг.ПустаяСсылка)";
	Запрос.УстановитьПараметр("ЗаказКлиента", Объект.ЗаказКлиента);
	
	ТЗ_ЗаявкиНаУслуги = Запрос.Выполнить().Выгрузить();
	
	// проверим есть ли такие услуги в табличной части реализации
	КолвоЗаявок = ТЗ_ЗаявкиНаУслуги.Количество();
	i = 0;
	
	Пока i < КолвоЗаявок Цикл
		Если Объект.Товары.НайтиСтроки(Новый Структура("Номенклатура", ТЗ_ЗаявкиНаУслуги[i].Услуга)).Количество() > 0 Тогда
			i = i + 1;
		Иначе
			ТЗ_ЗаявкиНаУслуги.Удалить(ТЗ_ЗаявкиНаУслуги[i]);
			КолвоЗаявок = КолвоЗаявок - 1;
		КонецЕсли;	
	КонецЦикла;
	
	СЗ_Заявки = Новый СписокЗначений;
	СЗ_Заявки.ЗагрузитьЗначения(ТЗ_ЗаявкиНаУслуги.ВыгрузитьКолонку("Ссылка"));
	Возврат СЗ_Заявки;
	
КонецФункции//ПолучитьЗаявкиНаСервере()
// +Альтер ЛОГО

// Альтер ЛОГО+
&НаСервере
Функция НайтиЗаявкуНаТранспортНаСервере()     
	//++ASP_Crow 10.03.2026 000029567 в расширение
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДокЗаявкаНаТранспорт.Ссылка КАК ЗаявкаНаТранспорт,
	|	ВЫБОР
	...
	|ИЗ
	|	Документ.ЗаявкаНаТранспорт КАК ДокЗаявкаНаТранспорт
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	|		ПО (РеализацияТоваровУслуг.ЗаявкаНаТранспорт = ДокЗаявкаНаТранспорт.Ссылка)
	|			И (РеализацияТоваровУслуг.ЗаказКлиента = &ЗаказКлиента)
	|			И (НЕ РеализацияТоваровУслуг.ПометкаУдаления)
	|ГДЕ
	|	НЕ ДокЗаявкаНаТранспорт.ПометкаУдаления
	|	И ДокЗаявкаНаТранспорт.ДокументОснование = &ЗаказКлиента
	|
	|УПОРЯДОЧИТЬ ПО
	|	Реализация,
	|	ДокЗаявкаНаТранспорт.Дата УБЫВ";
	Запрос.УстановитьПараметр("ЗаказКлиента", Объект.ЗаказКлиента);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ЗаявкаНаТранспорт;
	Иначе
		Возврат Неопределено;
	КонецЕсли;	
	
КонецФункции//НайтиЗаявкуНаТранспортНаСервере()

Процедура ЗаполнитьПоЗаявкеНаТранспортНаСервере()
	//++ASP_Crow 10.03.2026 000029567 в расширение
	//Объект.Автомобиль = Объект.ЗаявкаНаТранспорт.Автомобиль;
	//Объект.Водитель = Объект.ЗаявкаНаТранспорт.Водитель;
	//ГТС добавляем способо доставки 14.03.2021
	Объект.ДатаДоставки=Объект.ЗаявкаНаТранспорт.ПланируемаяДатаДоставки;
	Объект.СпособДоставки=перечисления.СпособыДоставки.ДоКлиента;
КонецПроцедуры//ЗаполнитьПоЗаявкеНаТранспортНаСервере	
// +Альтер ЛОГО
...
Процедура ЗаполнитьМаркетинговоеМероприятиеПоУмолчаниюСервер()
	     //++ASP_Crow 10.03.2026 000029567 в расширение
	Если НЕ (ЗначениеЗаполнено(Объект.КаналПервичногоИнтереса) 
		И Объект.КаналПервичногоИнтереса.ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.МаркетинговыеМероприятия")) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Владелец", Объект.КаналПервичногоИнтереса);
	ТЗ = Запрос.Выполнить().Выгрузить();
	Если ТЗ.Количество() = 1 Тогда
		Объект.ИсточникПервичногоИнтереса = ТЗ[0].Ссылка;
	КонецЕсли;
	
	Если Объект.КаналПервичногоИнтереса = ПланыВидовХарактеристик.КаналыРекламныхВоздействий.НайтиПоНаименованию("Старый клиент") 
		И НЕ ЗначениеЗаполнено(Объект.ИсточникПервичногоИнтереса) Тогда
		Объект.ИсточникПервичногоИнтереса = Справочники.МаркетинговыеМероприятия.НайтиПоНаименованию("Старый клиент");	
	КонецЕсли;
	
КонецПроцедуры//ЗаполнитьМаркетинговоеМероприятиеПоУмолчаниюСервер()
// +Альтер ЛОГО

Функция ПолныеПраваДоступны(ПолнПр=Неопределено)
	//++ASP_Crow 10.03.2026 000029567 в расширение
...
Процедура УстановитьОтметкуСклада(Команда)
	...
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Если ВыполнитьПроверкуНаличияГарантии() Тогда
	...
	//ASP_Hlopkov 20.09.21-  
	Иначе
	Сообщить ("Требуется занесение гарантии на данную продукцию перед установкой отметки склада")
	КонецЕсли;
...
//ГТС блокируем изменения ТЧ пользователями после установки отметки склада 30.06.2019
...
Процедура УстановитьДоступностьОтметкиСклада()
	...
	Если РольДоступна("ПолныеПрава")  ИЛИ РольДоступна("ASP_ОтметкаСкладаПерекупка") И Объект.Склад=Справочники.Склады.НайтиПоНаименованию("Склад прямой перекупки") 
		//ИЛИ строка(Пользователи.ТекущийПользователь().Подразделение) = "Отдел сервиса" 
		Тогда
		Элементы.ГруппаОтметкаСклада.Доступность = Истина;
	КонецЕсли;
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Объект.ПринятоВРаботу) Тогда
		Элементы.ГруппаТовары.ТолькоПросмотр = Истина;
		Элементы.ТоварыГруппаЗаполнить.Доступность=Ложь;
	КонецЕсли;
	
	Элементы.ОригиналыДокументовПолучены.Доступность=Истина;
...
Процедура ЗаписатьВДатеЗапретаНаСервере()
     //++ASP_Crow 10.03.2026 000029567 в расширение
	ДокументОбъект = ДанныеФормыВЗначение(Объект, Тип("ДокументОбъект.РеализацияТоваровУслуг"));		
...
&НаКлиенте
Процедура ASP_УстановитьФактПолученияДокументов(Команда)
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Объект.ОригиналыДокументовПолучены = Истина;
...
&НаСервере
Процедура УстановитьДоступностьУстановкиФактаПолученияДокументов()
    //++ASP_Crow 10.03.2026 000029567 в расширение
	Если ЭтаФорма.ТолькоПросмотр Тогда
		Элементы.ASP_УстановитьФактПолученияДокументов.Видимость	 = Истина;
		Элементы.ASP_УстановитьФактПолученияДокументов.Доступность	 = Истина;
	Иначе
		Элементы.ASP_УстановитьФактПолученияДокументов.Видимость	 = Ложь;
		Элементы.ASP_УстановитьФактПолученияДокументов.Доступность	 = Ложь;
	КонецЕсли;	

КонецПроцедуры

&НаКлиенте
Процедура ASP_УбратьОтметкуСклада(Команда)
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Объект.ОтметкаСклада=Дата("00010101");
	Объект.ПринятоВРаботу=Дата("00010101");
	//ASP_Hlopkov 20.09.21+
	Если ЭтаФорма.ТолькоПросмотр Тогда
		ЗаписатьВДатеЗапретаНаСервере();
	КонецЕсли;	
...
&НаСервере
Процедура УстановитьДоступностьASP_УбратьОтметкуСклада()
      //++ASP_Crow 10.03.2026 000029567 в расширение
	//Если ЭтаФорма.ТолькоПросмотр Тогда
		Элементы.ASP_УбратьОтметкуСклада.Видимость	 = Истина;
		//Элементы.ASP_УбратьОтметкуСклада.Доступность = Истина;
	//Иначе
		//Элементы.ASP_УбратьОтметкуСклада.Видимость	 = Ложь;
		//Элементы.ASP_УбратьОтметкуСклада.Доступность = Ложь;
	//КонецЕсли;	
...
Функция ВыполнитьПроверкуНаличияГарантии()  
	...
	Если Объект.Дата>='20231001' 
		...
		Товары=ОбъектФормы.Товары.Выгрузить();
		Запрос = Новый Запрос;
...
Функция ПроверкаГарантийногоТалона() Экспорт     
	//++ASP_Crow 10.03.2026 000029567 в расширение
		...
		Итог=ложь;
		Пока Выборка.Следующий() Цикл
		сообщить("Позиция " + Выборка.Номенклатура + " требует оформления гарантийного талона.");
		Итог=истина;
		КонецЦикла;
	Возврат	Итог;
КонецФункции	
