...
Процедура ЗаполнитьТаблицуТоваров(Документ, ВалютаДокумента)
	...
	//+++ ASP_Маркелов 30.01.2026 000029395 Проверяем на дату заказ, заполняем нужные данные
	ПроверитьНДС = Ложь;
	Если Параметры.Документ.ОснованиеДата <= Дата('2025.12.31 23:59:59') Тогда 
		ПроверитьНДС = Истина;
		Ставка20Процентов = Справочники.СтавкиНДС.НайтиПоНаименованию("20%");
		Ставка22Процентов = Справочники.СтавкиНДС.НайтиПоНаименованию("22%");		
	    ДокументОбъект = Документ.ПолучитьОбъект();
	    СтруктураПараметровПересчета = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ДокументОбъект);
	...
	КонецЕсли;
	//--- ASP_Маркелов 30.01.2026 000029395
	//--//++ASP_Crow 10.03.2026 000029567 в расширение
	...
	Для каждого СтрокаТоваров Из ТаблицаТоваров Цикл
		...
		//+++ ASP_Маркелов 30.01.2026 00002939 Проверка, замена ставки НДС и перерасчет
	    Если ПроверитьНДС И ЗначениеЗаполнено(СтрокаТоваров.СтавкаНДС) Тогда
	    	Если СтрокаТоваров.СтавкаНДС = Ставка20Процентов Тогда
	        	СтрокаТоваров.СтавкаНДС = Ставка22Процентов;
				
				СтруктураДействий = Новый Структура;
                СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПараметровПересчета);
				...
				СтруктураДействий.Вставить("ПересчитатьСуммуСверхЗаказа", Новый Структура("РеализацияПоступлениеПоЗаказу, ТребуетсяЗалогЗаТару",
					ДокументОбъект.РеализацияПоЗаказам, ДокументОбъект.ТребуетсяЗалогЗаТару));
                ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТоваров, СтруктураДействий, КэшированныеЗначения);
				
			КонецЕсли;
	    КонецЕсли;
	    //--- ASP_Маркелов 30.01.2026 00002939
	КонецЦикла; 
...
Процедура ПерезаполнитьКоличествоПодобрано(СЗаполнением = Ложь, ТоварыРеализации = Неопределено, ШтрихкодыУпаковокРеализации = Неопределено)
	...
	Если СЗаполнением Тогда
		...
		СтруктураДопПоиска = Новый Структура("Номенклатура, Характеристика, КодСтроки, ЗаказКлиента, Серия"); //ASP_Crow 22.05.2026 000029567 
	КонецЕсли;
	...
	Для каждого ТекСтрока Из ТаблицаТоваров Цикл
		...
		Если СЗаполнением Тогда
			...
			ЗаполнитьЗначенияСвойств(СтруктураДопПоиска, ТекСтрока);//ASP_Crow 22.05.2026 000029567 
			Строки = ТоварыРеализации.НайтиСтроки(СтруктураПоиска);
			СтрокиДоп = ТоварыРеализации.НайтиСтроки(СтруктураДопПоиска);//ASP_Crow 22.05.2026 000029567 	
			...
			ИначеЕсли СтрокиДоп.Количество() > 0 Тогда//ASP_Crow 22.05.2026 000029567 
				СтрокаРеализации = СтрокиДоп.Получить(0);
				ЗаполнитьЗначенияСвойств(ТекСтрока, СтрокаРеализации,,"ВидЦены, Цена, Количество, КоличествоУпаковок");
				Если ТекСтрока.КоличествоУпаковок <> СтрокаРеализации.КоличествоУпаковок Тогда
					ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекСтрока, СтруктураДействий, Неопределено);
				КонецЕсли;
