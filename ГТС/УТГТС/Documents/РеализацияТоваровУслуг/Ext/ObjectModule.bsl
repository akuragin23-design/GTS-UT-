...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтоНовый()
		...
		//++ASP_Crow 10.10.2024 000024814 Заплатка т.к. сбилась нумерация 
		Если Год(ТекущаяДата()) = 2024 и Год(Дата) = 2024 Тогда
			УстановитьПривилегированныйРежим(Истина);
			Номер = "ГТ-00019869"; // первый номер который считается корректным
			Запрос = новый Запрос;
			...
			МассивНомеровЗаГод = Рез.ВыгрузитьКолонку("Номер");
			НомерБезНулей = Число(ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ЭтотОбъект.Номер,Истина,Истина));
			Пока МассивНомеровЗаГод.Найти(Номер) <> неопределено Цикл
            	НомерБезНулей = НомерБезНулей + 1;
				Номер = "ГТ-000" + СтрЗаменить(НомерБезНулей,Символы.НПП,"");
			КонецЦикла;
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
		//--ASP_Crow 10.10.2024 000024814
	КонецЕсли;
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	//++ASP_Crow 10.03.2026 000029567 в расширение 
	//ГТС
	Если ЗначениеЗаполнено(ОтметкаСклада) И Не ДополнительныеСвойства.Свойство("ОбходОтметкиСклад") Тогда 
		Отказ = Истина;
		Сообщить("Необходимо снять отметку склада перед отменой проведения");
	КонецЕсли;
	Если ЗначениеЗаполнено(ПринятоВРаботу) И Не ДополнительныеСвойства.Свойство("ОбходОтметкиСклад") Тогда 
		Отказ = Истина;
...
Процедура ЗаполнитьДокументНаОснованииЗаказаКлиента(Знач ДокументОснование,
	...
	Если ТипОснования = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		КонецЕсли;
		// Альтер ЛОГО+:	заполнение данных для целей маркетинга
		КаналПервичногоИнтереса = ЗаказКлиента.КаналПервичногоИнтереса;
		ИсточникПервичногоИнтереса = ЗаказКлиента.ИсточникПервичногоИнтереса;
		// +Альтер ЛОГО
		//Асист KS
		КодКлиента = ЗаказКлиента.КодКлиента;
		//Асист KS
		//ГТС
		Менеджер = ЗаказКлиента.Менеджер;
		Отпустил = ЗаказКлиента.Менеджер.ФизическоеЛицо;
		ОтгрузкаБезДокументов = ЗаказКлиента.ОтгрузкаБезДокументов;
	    ОтпустилДолжность = "";
	...
	//ASP корректировка основания для заказа клиента
	...
	СтруктураОснование.Основание = "Счет на оплату №"+ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ЭтотОбъект.ЗаказКлиента.Номер,Истина,Истина)+" от "+формат(ЭтотОбъект.ЗаказКлиента.Дата,"ДЛФ=ДД") ;;
	СтруктураОснование.ОснованиеДата = ЭтотОбъект.ЗаказКлиента.Дата;
	СтруктураОснование.ОснованиеНомер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ЭтотОбъект.ЗаказКлиента.Номер,Истина,Истина);
	
	
	//ASP конец
	
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтруктураОснование); 
//++000027117 ASP_Мочалина 
	...
	СкидкиНаценкиЗаполнениеСервер.Рассчитать(ЭтотОбъект, ПараметрыРасчета);
//--000027117 ASP_Мочалина
