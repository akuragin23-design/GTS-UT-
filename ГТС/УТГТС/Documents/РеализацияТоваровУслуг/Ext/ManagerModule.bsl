...
КонецФункции
 ...
 Функция ПроверитьДокументыОтгрузкаБезДокументов(ОбъектПараметры) Экспорт 
	 //ASP_Crow 17.03.2026 000029567 в расширение
	 ...
	 МассивОбъектовОтгрузкаБезДокументов = Новый Массив;
		Для Каждого СтрокаОбъекта Из  ОбъектПараметры Цикл  
			Если СтрокаОбъекта.ЗаказКлиента.ОтгрузкаБезДокументов Тогда
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// Альтер ЛОГО+:	Добавлен параметр ВыводитьКомплектующие для печати с комплектующими в процедуру СформироватьПечатнуюФормуНакладная()
	Если ЗначениеЗаполнено(ПараметрыПечати) тогда
		ВыводитьКомплектующие = ?(ПараметрыПечати.Свойство("ВыводитьКомплектующие"), ПараметрыПечати.ВыводитьКомплектующие, Ложь);
		ВывестиГТД = ?(ПараметрыПечати.Свойство("ВыводитьГТД"), ПараметрыПечати.ВыводитьГТД, Ложь);	
		ВывестиКомплектовочныйЛист = ?(ПараметрыПечати.Свойство("ВывестиКомплектовочныйЛист"), ПараметрыПечати.ВывестиКомплектовочныйЛист, Ложь);	
	Иначе
		ВыводитьКомплектующие = Ложь;
		ВывестиГТД = Ложь;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная") Тогда
		//++ ASP_Спиряева 27.02.2025 000026255 Изменила вывод для печати табличного док. вместо одной, если есть 2 форма,
		...
		//				МассивОбъектов, 
		//				ОбъектыПечати, 
		//				ВыводитьКомплектующие, 
		//				ВывестиГТД,
		//				ВывестиКомплектовочныйЛист)
		//	//СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыПечати),//1,03
		//	,
		//	"Документ.РеализацияТоваровУслуг.ПФ_MXL_РеализацияТоваров");
		
						
		// Формируем первую печатную форму (Накладная)  000026255
		ИтоговыйТабДокумент = СформироватьПечатнуюФормуНакладная(
		МассивОбъектов, 
		ОбъектыПечати, 
		ВыводитьКомплектующие, 
		ВывестиГТД,
		...
		Если ВывестиКомплектовочныйЛист = Ложь И ЗначениеЗаполнено(МассивОбъектов) Тогда
			// Формируем вторую печатную форму (Отгрузка без документов)000026255
			ПФ_Отгрузка = СформироватьПечатнуюФормуНакладнаяОтгрузкаБезДокументов(
			МассивОбъектов, 
			ОбъектыПечати, 
			ВыводитьКомплектующие, 
			ВывестиГТД,
	...
	РеализацияТоваровУслугЛокализация.Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода); //ASP_Crow 17.03.2026 000029567 в расширение
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			Аль_СформироватьПечатнуюФормуНакладная(КомплектПечати.Объекты, ОбъектыПечати, Истина, Ложь, Ложь)); //лала
			//ASP_Dante 01/09/19
	КонецЦикла;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			Аль_СформироватьПечатнуюФормуНакладная(КомплектПечати.Объекты, ОбъектыПечати, Истина, Ложь, Ложь));
			//ASP_Dante 01/09/19
	КонецЦикла;
	...
	Для каждого Строка Из МассивОбъектов Цикл
		...
		Если ЗначениеЗаполнено(Строка.ЗаказКлиента) Тогда
			МассивДокументов.Добавить(Строка.ЗаказКлиента);
		КонецЕсли;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		КомплектПечати.Имя,
		КомплектПечати.Представление,
		Документы.РеализацияТоваровУслуг.Аль_СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ВывестиКомплектующие , ВывестиГТД, ВывестиКомплектовочныйЛист));
	КонецЦикла;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		ИтоговыйТабДокумент	= Документы.РеализацияТоваровУслуг.СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ВывестиКомплектующие , ВывестиГТД, ВывестиКомплектовочныйЛист);
		//Проверяем, есть ли объекты для второй формы  000026255
		МассивОбъектовОтгрузкаБезДок = ПроверитьДокументыОтгрузкаБезДокументов(МассивОбъектов);
		Если ЗначениеЗаполнено(МассивОбъектовОтгрузкаБезДок) Тогда
			// Формируем вторую печатную форму (Отгрузка без документов)000026255
			ПФ_Отгрузка = СформироватьПечатнуюФормуНакладнаяОтгрузкаБезДокументов(МассивОбъектовОтгрузкаБезДок,ОбъектыПечати, ВывестиКомплектующие , ВывестиГТД, ВывестиКомплектовочныйЛист);
...
КонецФункции
 ...
 Функция СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ВывестиКомплектующие , ВывестиГТД , ВывестиКомплектовочныйЛист) Экспорт
//	//ASP_Crow 10.03.2026 000029567 Каким-то чудом перенести в расширение
...
КонецФункции
Функция СформироватьПечатнуюФормуНакладнаяОтгрузкаБезДокументов(МассивОбъектов, ОбъектыПечати, ВывестиКомплектующие , ВывестиГТД , ВывестиКомплектовочныйЛист) Экспорт    
	//ASP_Crow 17.03.2026 000029567 в расширение
//++ ASP_Спиряева 000026255 21.02.2025 скопировано из функции СформироватьПечатнуюФормуНакладная
//но убраны некоторые области для вывода 000026255 
	УстановитьПривилегированныйРежим(Истина);
	//ASP_Dante 01/09/19
	//Если ПараметрыПечати <> Неопределено И ПараметрыПечати.Свойство("ОтображатьСкидки") Тогда
	//	ОтображатьСкидки = ПараметрыПечати.ОтображатьСкидки;
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РеализацияТоваровУслуг_РеализацияТоваровОтгрузкаБезДокументов";
	...
	Пока ИСТИНА Цикл
		...
		// Макет необходимо получать для каждого документа, т.к. размеры колонок изменяются динамически.
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РеализацияТоваровУслуг.ПФ_MXL_РеализацияТоваровОтгрузкаБезДокументов"); //ASP_Dante 05/09/19ПолучитьМакет("ПФ_MXL_Накладная");//ASP_Dante 01/09/19
		...
		//ГТС добавляем блок доставки 
		если ЗначениеЗаполнено(ДанныеПечати.Ссылка.ЗаявкаНаТранспорт) Тогда
		ОбластьМакета = Макет.ПолучитьОбласть("Доставка");
		ОбластьМакета.Параметры.АдресДоставки = ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.АдресДоставки;
		ОбластьМакета.Параметры.Водитель = ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.Водитель;
		Комментарий = строка(ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.ТелефонКлиента) + " " + строка(ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.Комментарий)  + " " + строка(ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.КомментарийСклад);
		ТабличныйДокумент.Вывести(ОбластьМакета);
		конецесли;
...
#КонецЕсли
// Альтер ЛОГО+:	изменен состав параметров
// Функция СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, )
Функция Аль_СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ВывестиКомплектующие , ВывестиГТД , ВывестиКомплектовочныйЛист) Экспорт 
	//ASP_Crow 17.03.2026 000029567 в расширение
//	
	УстановитьПривилегированныйРежим(Истина);
	//ASP_Dante 01/09/19
	...
	Пока ИСТИНА Цикл
		...
		// Макет необходимо получать для каждого документа, т.к. размеры колонок изменяются динамически.
		//++ASP_Crow 04.03.2024 000021639 В базе до обновления "Старая" отсутствует, есть только обычный макет
		//Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РеализацияТоваровУслуг.ПФ_MXL_РеализацияТоваров_Старая"); //ASP_Dante 05/09/19ПолучитьМакет("ПФ_MXL_Накладная");//ASP_Dante 01/09/19
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РеализацияТоваровУслуг.ПФ_MXL_РеализацияТоваров");
		//--ASP_Crow 04.03.2024 000021639
		Если Не ПервыйДокумент Тогда
			ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
		КонецЕсли;
		...
		Если ВывестиКомплектовочныйЛист=Истина Тогда 
			ТекстЗаголовка = НСтр("ru='Комплектовочный лист по накладной № %НомерДокумента% от %ДатаДокумента% г.'");
		КонецЕсли;
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%НомерДокумента%", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер));
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%ДатаДокумента%",  Формат(ДанныеПечати.Дата, "ДФ='дд ММММ гггг'"));
		ОбластьМакета.Параметры.ЗаказКлиента = ДанныеПечати.Ссылка.ЗаказКлиента;
		если ЗначениеЗаполнено(ДанныеПечати.Ссылка.ЗаказКлиента.НомерЦикла) Тогда
		ОбластьМакета.Параметры.Цикл = "Номер цикла: "+ строка(ДанныеПечати.Ссылка.ЗаказКлиента.НомерЦикла);
		...
		ОбластьМакета.Параметры.АдресДоставки = ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.АдресДоставки;
		ОбластьМакета.Параметры.Водитель = ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.Водитель;
		Комментарий = строка(ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.ТелефонКлиента) + " " + строка(ДанныеПечати.Ссылка.ЗаявкаНаТранспорт.Комментарий);
		ТабличныйДокумент.Вывести(ОбластьМакета);
		конецесли;
	    ОбластьМакета = Макет.ПолучитьОбласть("Комментарий");
		Комментарий=?(ЗначениеЗаполнено(Комментарий),Комментарий+Символы.ПС,"") + строка(ДанныеПечати.Ссылка.Комментарий);
		...
		Если ВывестиКомплектовочныйЛист=Истина Тогда 
			ДокументыДляВыдачи = НСтр("ru='Комплектовочный лист по накладной № %НомерДокумента% от %ДатаДокумента% г.'");
			ДокументыДляВыдачи = СтрЗаменить(ДокументыДляВыдачи, "%НомерДокумента%", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер));
			ДокументыДляВыдачи = СтрЗаменить(ДокументыДляВыдачи, "%ДатаДокумента%",  Формат(ДанныеПечати.Дата, "ДФ='дд ММММ гггг'"));
		КонецЕсли;
...
//ASP+++                        
Функция ПолучитьДокументыДляВыдачи(ДанныеПечати)   
	//ASP_Crow 17.03.2026 000029567 в расширение
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДокументыВыданныеКлиенту.ВыданныйДокумент КАК ВыданныйДокумент,
		|	ДокументыВыданныеКлиенту.Количество КАК Количество
		|ИЗ
		|	РегистрСведений.ДокументыВыданныеКлиенту КАК ДокументыВыданныеКлиенту
		|ГДЕ
		|	ДокументыВыданныеКлиенту.Реализация = &Реализация";
	
	Запрос.УстановитьПараметр("Реализация", ДанныеПечати.Ссылка);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Если ВыборкаДетальныеЗаписи.Количество()>0 Тогда
	Текст = "Выданы следующие дополнительные документы: " + Символы.ПС;
	КонецЕсли;
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Текст = Текст + ВыборкаДетальныеЗаписи.ВыданныйДокумент + " - " + ВыборкаДетальныеЗаписи.Количество + " шт."+Символы.ПС;
	КонецЦикла;
	Возврат Текст;
