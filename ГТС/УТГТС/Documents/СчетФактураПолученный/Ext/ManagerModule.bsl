...
Процедура ПоместитьТаблицуСуммТорговыхДокументов(МассивТорговыхДокументов, МенеджерВременныхТаблиц, ИмяВременнойТаблицы)
	...
	Для Каждого Основание Из МассивТорговыхДокументов Цикл
		...
		//++ ASP_Maskina 000029328 27.01.2026
		Если ТаблицыДляДвижений.Свойство("ТаблицаЗаполнениеСуммСФПолученный") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицыДляДвижений.ТаблицаЗаполнениеСуммСФПолученный, ТаблицаСумм);
		Иначе  			
			Строка= ТаблицаСумм.Добавить(); 
			Строка.ДокументОснование = Основание;
            Строка.ХозяйственнаяОперация = Основание.ХозяйственнаяОперация; 
			Строка.Сумма = Основание.Сумма + Основание.СуммаНДС; 
			
			Если ТипЗнч(Основание.СтавкаНДС) = Тип("ПеречислениеСсылка.СтавкиНДС") Тогда
				Строка.СтавкаНДС = Справочники.СтавкиНДС.НайтиПоНаименованию(Строка(Основание.СтавкаНДС));
			Иначе
				Строка.СтавкаНДС = Основание.СтавкаНДС; 	
			КонецЕсли;
			Строка.СуммаНДС = Основание.СуммаНДС;   
			Строка.Валюта = Основание.Валюта;
		КонецЕсли;
		//-- ASP_Maskina 000029328 27.01.2026  
		//--ASP_Crow 10.03.2026 000029567 Поместить в расширение
	КонецЦикла;
