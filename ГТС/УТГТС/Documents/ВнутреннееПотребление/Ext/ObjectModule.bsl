...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//++//ASP_Crow 20.03.2026 000029567 В расширение
	//ГТС 08.09.2023 Для операции отдела сервиса  
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОперацииОтделаСервиса") Тогда
		ДокументОснование=ДанныеЗаполнения.Ссылка;                              
		Комментарий=("Основание списания: "+ строка(ДокументОснование));
		//ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеЗаполнения.РеквизитыШапки);
		Для каждого Стр1 из ДанныеЗаполнения.ВыполненныеРаботы Цикл
			Если Стр1.Номенклатура.ТипНоменклатуры=Перечисления.ТипыНоменклатуры.Товар Тогда
				НовСтр = Товары.Добавить();
				НовСтр.Номенклатура=Стр1.Номенклатура;
				НовСтр.Количество=Стр1.Количество;
				НовСтр.КоличествоУпаковок=Стр1.Количество;
				НовСтр.Упаковка=Стр1.Номенклатура.ЕдиницаИзмерения;
				НовСтр.СтатьяРасходов=ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("0--000039");
			КонецЕсли;
		КонецЦикла;
		
		ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.ВнутреннееПотребление);
		НоменклатураСервер.ЗаполнитьСерииПоFEFO(ЭтотОбъект,ПараметрыУказанияСерий, Ложь);
	//ГТС 08.09.2023 Конец
	//--//ASP_Crow 20.03.2026 000029567 В расширение
