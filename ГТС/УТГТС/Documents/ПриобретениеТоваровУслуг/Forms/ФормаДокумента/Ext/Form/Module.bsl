...
// Заполняет таб.часть Товары по заказам клиентов из таб.части ДокументыОснования
&НаСервере
Процедура ЗаполнитьТоварыПоЗаказамПокупателейНаСервере()
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Объект.Товары.Очистить();
	///
	Текст = 
	...
	|	СУММА(ТоварыЗаказов.Количество) КАК Количество,
	|	СУММА(ТоварыЗаказов.КоличествоУпаковок) КАК КоличествоУпаковок,
	|	МАКСИМУМ(ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)) КАК СтавкаНДС
	|ИЗ
	|	Документ.ЗаказКлиента.Товары КАК ТоварыЗаказов
	|ГДЕ
	|	ТоварыЗаказов.Ссылка В(&СписокЗаказов)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("СписокЗаказов", Объект.ДокументыОснования.Выгрузить().ВыгрузитьКолонку("ДокументОснование"));
	
	ТЗ = Запрос.Выполнить().Выгрузить();
	//
	ВремТЗКомплектующие = Новый ТаблицаЗначений();
	ВремТЗКомплектующие.Колонки.Добавить("Комплект");
	ВремТЗКомплектующие.Колонки.Добавить("Номенклатура");
	ВремТЗКомплектующие.Колонки.Добавить("Количество");
	ВремТЗКомплектующие.Колонки.Добавить("КоличествоУпаковок");
	
	МассивУдаляемыхСтрок = Новый Массив();
	
	Для каждого СтрокаТЗ из ТЗ цикл
		
		Если Найти(ВРег(СтрокаТЗ.Номенклатура.Наименование), "РАМА") = 0 тогда продолжить конецесли;
		ТЗКомплектов = ТаблицаКомплектующихНоменклатуры(СтрокаТЗ.Номенклатура, СтрокаТЗ.Количество, СтрокаТЗ.КоличествоУпаковок);
		Если ТЗКомплектов.Количество()>0 тогда
			МассивУдаляемыхСтрок.Добавить(СтрокаТЗ);
			//
			Для каждого СтрокаТЗКомплекта из ТЗКомплектов цикл
				СтрВрем = ВремТЗКомплектующие.Добавить();
				Для каждого Колонка из ВремТЗКомплектующие.Колонки цикл
					СтрВрем[Колонка.Имя] = СтрокаТЗКомплекта[Колонка.Имя];
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;
	...
	КонецЦикла;
	//
	Для каждого СтрВрем из ВремТЗКомплектующие цикл
		СтрокаТЗ = ТЗ.Добавить();
		СтрокаТЗ.Номенклатура = СтрВрем.Номенклатура;
		СтрокаТЗ.Количество = СтрВрем.Количество;
		СтрокаТЗ.КоличествоУпаковок = СтрВрем.КоличествоУпаковок;
		СтрокаТЗ.СтавкаНДС = Перечисления.СтавкиНДС.НДС20;
	КонецЦикла;
	//
	ТЗ.Свернуть("Номенклатура, СтавкаНДС", "Количество, КоличествоУпаковок");
	Объект.Товары.Загрузить(ТЗ);
	
КонецПроцедуры

// Функция возвращает таблицу комплектующих номенклатуры, если таковые есть
&НаСервере
Функция ТаблицаКомплектующихНоменклатуры(Номенклатура, Количество, КоличествоУпаковок)
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Текст = 
	"ВЫБРАТЬ
	|	КомплектующиеНоменклатуры.Номенклатура КАК Комплект,
	|	КомплектующиеНоменклатуры.Комплектующая КАК Номенклатура,
	|	ВЫРАЗИТЬ(КомплектующиеНоменклатуры.Количество * &Количество КАК ЧИСЛО(15, 3)) КАК Количество,
	|	ВЫРАЗИТЬ(КомплектующиеНоменклатуры.Количество * &КоличествоУпаковок КАК ЧИСЛО(15, 3)) КАК КоличествоУпаковок
	|ИЗ
...
// Заполняет цены по указанному виду цен
&НаКлиенте
...
&НаКлиенте
Процедура УстановитьОтметкуСклада(Команда)
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Объект.ОтметкаСклада=ТекущаяДата();
КонецПроцедуры

&НаСервере
Процедура УстановитьДоступностьОтметкиСклада()  
	//++ASP_Crow 10.03.2026 000029567 в расширение
	УстановитьПривилегированныйРежим(Истина);
	//ГТС корректируем что только сотрудник текущего склада может ставить отметку.
	//РольДоступна("ASP_ОтметкаСклада") ИЛИ
	Если Строка(Объект.Склад) = "Склад временного хранения г.ТАЛДОМ" и строка(Пользователи.ТекущийПользователь().Подразделение) = "Склад Талдом" Тогда
	Элементы.ГруппаОтметкаСклада.Доступность = Истина;
	ИначеЕсли Строка(Объект.Склад) = "Склад временного хранения г.ТАЛДОМ" и строка(Пользователи.ТекущийПользователь().Подразделение) <> "Склад Талдом" Тогда
	Элементы.ГруппаОтметкаСклада.Доступность = Ложь;
	ИначеЕсли Строка(Объект.Склад) = "Склад Подольск" и строка(Пользователи.ТекущийПользователь().Подразделение) = "Склад Москва" Тогда
	Элементы.ГруппаОтметкаСклада.Доступность = Истина;
	ИначеЕсли 	Строка(Объект.Склад) = "Склад Подольск" и строка(Пользователи.ТекущийПользователь().Подразделение) <> "Склад Москва" Тогда
	Элементы.ГруппаОтметкаСклада.Доступность = Ложь;
	ИначеЕсли РольДоступна("ASP_ОтметкаСклада")  Тогда
	Элементы.ГруппаОтметкаСклада.Доступность = Истина;
	Иначе Элементы.ГруппаОтметкаСклада.Доступность = Ложь;
	КонецЕсли;
	//Элементы.ГруппаОтметкаСклада.Доступность = (РольДоступна("ПолныеПрава") ИЛИ (РольДоступна("ASP_ОтметкаСкладаПерекупка") И Объект.Склад=Справочники.Склады.НайтиПоНаименованию("Склад прямой перекупки")));
	
	Если РольДоступна("ПолныеПрава")  ИЛИ РольДоступна("ASP_ОтметкаСкладаПерекупка") И Объект.Склад=Справочники.Склады.НайтиПоНаименованию("Склад прямой перекупки") Тогда
	Элементы.ГруппаОтметкаСклада.Доступность = Истина;
	КонецЕсли;
	Элементы.ГруппаОтметкаСклада.ТолькоПросмотр = НЕ (РольДоступна("ASP_ОтметкаСклада") ИЛИ РольДоступна("ПолныеПрава"));
	Если ЗначениеЗаполнено(Объект.ОтметкаСклада) Тогда
		Элементы.ГруппаТовары.ТолькоПросмотр = Истина;
	КонецЕсли;
	//Устанавливаем доступность валютного учета
	Элементы.ВалютныйУчет.Видимость = (РольДоступна("ASP_ВалютныйУчет") или РольДоступна("ПолныеПрава"));
	УстановитьПривилегированныйРежим(Ложь);
...
&НаСервере
Процедура ASP_ПроверитьНазначенияНаСервере()
	//++ASP_Crow 10.03.2026 000029567 в расширение
	пп=0;
	...
	Запрос.Текст = 
		...
		|	И ЗаказКлиентаТовары.Ссылка.Назначение = &Назначение
	...
	Запрос.УстановитьПараметр("Назначение", Стр.Назначение);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если Стр.Количество>ВыборкаДетальныеЗаписи.Количество Тогда
			Сообщить("Под назначение " +Стр.Назначение+" требуется " + Стр.Номенклатура + Стр.Характеристика + " на "+ строка(Стр.Количество-ВыборкаДетальныеЗаписи.Количество) + " шт. меньше чем указано");
		КонецЕсли	
		КонецЦикла;
		
		Если т=0 Тогда сообщить("Под назначение " +Стр.Назначение+" не требуется товар "+ Стр.Номенклатура + Стр.Характеристика);
		КонецЕсли;
	Иначе
	КонецЕсли;
	КонецЦикла;
сообщить("Проверено" + строка(пп) + " назначений.");
КонецПроцедуры

&НаКлиенте
Процедура ASP_ПроверитьНазначения(Команда)
	//++ASP_Crow 10.03.2026 000029567 в расширение
	ASP_ПроверитьНазначенияНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ASP_УдалитьОтметкуСклада(Команда)
	//++ASP_Crow 10.03.2026 000029567 в расширение
	Объект.ОтметкаСклада=Дата("00010101");
	//ASP_Hlopkov 20.09.21+
	Если ЭтаФорма.ТолькоПросмотр Тогда
		ЗаписатьВДатеЗапретаНаСервере();
	КонецЕсли;	
	//ASP_Hlopkov 20.09.21-
КонецПроцедуры

&НаСервере
Процедура ЗаписатьВДатеЗапретаНаСервере()
    //++ASP_Crow 10.03.2026 000029567 в расширение
	ДокументОбъект = ДанныеФормыВЗначение(Объект, Тип("ДокументОбъект.ПриобретениеТоваровУслуг"));		
	
	ДокументОбъект.ОбменДанными.Загрузка = Истина;
	ДокументОбъект.ДополнительныеСвойства.Вставить("ИгнорироватьДатуЗапрета", Истина);
...
// заполняет таб.часть Товары по заказам покупателей
&НаКлиенте
Процедура ЗаполнитьТоварыПоЗаказамПокупателей(Команда)
	
	Если Объект.ДокументыОснования.Количество() = 0 тогда возврат конецесли;
	///
	Если Объект.Товары.Количество() > 0 тогда
		Ответ = Вопрос("Таблица товаров будет очищена и заполнена заново. Продолжить?", РежимДиалогаВопрос.ДаНетОтмена);
		Если НЕ Ответ = КодВозвратаДиалога.Да тогда возврат конецесли;		
	КонецЕсли;
	///
	ЗаполнитьТоварыПоЗаказамПокупателейНаСервере();
	
КонецПроцедуры  

&НаКлиенте
Процедура ПодобратьЗаказыПокупателей(Команда)
	ОткрытьФорму("Документ.ПриобретениеТоваровУслуг.Форма.ФормаПодобораЗаказов",,ЭтаФорма);
КонецПроцедуры
...
Процедура ЗаполнитьСтатьюРасходов(Команда)
	  СтатьяРасходов = "";
	Если ВвестиЗначение(СтатьяРасходов,,Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов")) Тогда
	ЗаполнитьСтатьюРасходовСервер(СтатьяРасходов);
	КонецЕсли;
	
КонецПроцедуры  

&НаСервере
Процедура ЗаполнитьСтатьюРасходовСервер(СтатьяРасходов)
	
	Для каждого Стр из Объект.Товары Цикл
		Стр.СписатьНаРасходы=Истина;
		Стр.СтатьяРасходов=СтатьяРасходов;
	Конеццикла
