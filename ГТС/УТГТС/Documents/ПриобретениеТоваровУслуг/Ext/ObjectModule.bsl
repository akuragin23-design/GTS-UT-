...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Менеджер) Тогда //ASP_Crow 16.03.2026 000029567 В расширение
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПриобретениеТоваровУслугЛокализация.asp_ОбработкаПроведения(ЭтотОбъект, Отказ, РежимПроведения);//ASP_Dante 04/12/19
	// tise добавил 2011-01-04
	Если НЕ Отказ Тогда
		
		ВЦ_Закупочная = Справочники.ВидыЦен.Закупочная;
		ВалютаЦены = Константы.ВалютаРегламентированногоУчета.Получить();
		
		ЗапросЦеныСегодня = Новый Запрос;
		ЗапросЦеныСегодня.Текст =
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ЦеныНоменклатуры.Период = &ДатаЦены
		|	И ЦеныНоменклатуры.ВидЦены = &ВЦ_Закупочная
		|	И ЦеныНоменклатуры.Номенклатура В(&СписокНоменклатуры)
		|	И ЦеныНоменклатуры.Характеристика В(&СписокХарактеристик)";
		ЗапросЦеныСегодня.УстановитьПараметр("ДатаЦены", НачалоДня(Дата));
		ЗапросЦеныСегодня.УстановитьПараметр("ВЦ_Закупочная", ВЦ_Закупочная);
		ЗапросЦеныСегодня.УстановитьПараметр("СписокНоменклатуры", Товары.ВыгрузитьКолонку("Номенклатура"));
		ЗапросЦеныСегодня.УстановитьПараметр("СписокХарактеристик", Товары.ВыгрузитьКолонку("Характеристика"));
		
		ТЗ_Цены = ЗапросЦеныСегодня.Выполнить().Выгрузить();
		
		// Альтер ЛОГО+:	добавлено для записи закупочных цен номенклатуры
		Для каждого СтрокаТЧ из Товары Цикл
			Если СтрокаТЧ.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
				Продолжить;
			КонецЕсли;
			Если ТЗ_Цены.НайтиСтроки(Новый Структура("Номенклатура,Характеристика",СтрокаТЧ.Номенклатура,СтрокаТЧ.Характеристика)).Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
			
			ЗаписьЦены = Движения.ЦеныНоменклатуры.Добавить();
			ЗаписьЦены.Период = Дата;
			ЗаписьЦены.ВидЦены = ВЦ_Закупочная;
			ЗаписьЦены.Валюта = ВалютаЦены;
			ЗаписьЦены.Номенклатура = СтрокаТЧ.Номенклатура;
			ЗаписьЦены.Характеристика = СтрокаТЧ.Характеристика;
			ЗаписьЦены.Упаковка = СтрокаТЧ.Упаковка;
			ЗаписьЦены.Цена = ?(ЦенаВключаетНДС, СтрокаТЧ.Сумма, СтрокаТЧ.Сумма + СтрокаТЧ.СуммаНДС) / СтрокаТЧ.КоличествоУпаковок;
			
			СтрокаЦенаЕсть = ТЗ_Цены.Добавить();
			СтрокаЦенаЕсть.Номенклатура = СтрокаТЧ.Номенклатура;
			СтрокаЦенаЕсть.Характеристика = СтрокаТЧ.Характеристика;
		КонецЦикла;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	//asp+ //ASP_Crow 16.03.2026 000029567 В расширение
	Менеджер                  = Пользователи.ТекущийПользователь();
    //asp-
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	ИначеЕсли ИспользоватьРаздельноеОформлениеЗакупок
		...
		ПараметрыЗаполнения = ПараметрыЗаполненияВидовЗапасов(ИспользоватьРаздельноеОформлениеЗакупок);
		//++ASP_Crow 10.07.2024 000023822 Если товары в пути то пойдем (попытаемся) по типовой логике  //ASP_Crow 16.03.2026 000029567 В расширение
		Если ЭтотОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпортуТоварыВПути Тогда  
			ЗапасыСервер.ЗаполнитьВидыЗапасовПоОстаткамКОформлению(ЭтотОбъект,
																	МенеджерВременныхТаблиц,
																	Отказ,
																	ПараметрыЗаполнения);
		Иначе
			ЗапасыСервер.ЗаполнитьВидыЗапасовПоОстаткамКОформлению_Типовой(ЭтотОбъект,МенеджерВременныхТаблиц,Отказ,ПараметрыЗаполнения);															
		КонецЕсли;
...
Функция ВременныеТаблицыДанныхДокумента(ИспользоватьРаздельноеОформлениеЗакупок, ВключатьПрослеживаемыеРаботы = Истина)
	...
	//++ASP_Crow 27.01.2025 000025949 Для учета номенклатуры набора //ASP_Crow 16.03.2026 000029567 В расширение
	|	,ИСТИНА как НоменклатураНабора	
...
// Альтер ЛОГО+:	добавлено в связи с добавлением типа основания заполнения ЗаказКлиента
Процедура ЗаполнитьДокументНаОснованииЗаказаКлиента(Знач Основание)
	...
	//|	И ЗаказКлиентаТовары.ДатаОтгрузки = &ПустаяДата
	...
	|			И ЗаказыКлиентовОстатки.КодСтроки = ТоварыВЗаказе.КодСтроки
	|ГДЕ
...
//ГТС валютный учет 
Процедура ВалютныйУчетПроведение()     
	//ASP_Crow 16.03.2026 000029567 В расширение
	Если ЭтотОбъект.ASP_ВалютныйУчет=Истина Тогда
		
		
		НаборЗаписей = РегистрыСведений.ASP_ВалютныйУчет.СоздатьНаборЗаписей();
		...
		|	ВТ.Номенклатура,
		|	ВТ.Характеристика");
		З.УстановитьПараметр("ТЧ",ЭтотОбъект.Товары);
		...
		Пока Выборка.Следующий() Цикл
			...
			|ГДЕ
			|	НоменклатураДополнительныеРеквизиты.Свойство.Наименование = ""Исключить из вал. учета""
			|	И НоменклатураДополнительныеРеквизиты.Ссылка = &Ссылка";
			
			Запрос1.УстановитьПараметр("Ссылка", Выборка.Номенклатура);
			...
			Пока ВыборкаДетальныеЗаписи1.Следующий() Цикл
				НеУчитывать=ВыборкаДетальныеЗаписи1.Значение;
			КонецЦикла;
			
			Если НеУчитывать=Ложь Тогда
				
				
				НовЗапись = НаборЗаписей.Добавить();
				НовЗапись.Период = ЭтотОбъект.Дата;
				НовЗапись.Номенклатура = Выборка.Номенклатура;
				НовЗапись.Характеристика = Выборка.Характеристика;      
				НовЗапись.ВидЦены=ЭтотОбъект.ASP_ВалютаУчета;
				НовЗапись.Коррекция=ложь;
				ДоляПозиции=Выборка.СуммаСНДС/ЭтотОбъект.СуммаДокумента;
				Если ЭтотОбъект.ASP_ВалютаУчета=Справочники.ВидыЦен.ВалютныйУчетCNY Тогда
				если ЭтотОбъект.Валюта.Код="643" Тогда
					НовЗапись.ЦенаВВалюте=((Выборка.СуммаСНДС/Выборка.КоличествоУпаковок)/ЭтотОбъект.ASP_КурсВалюты)+(((ЭтотОбъект.ASP_СуммаДопрасходов/ЭтотОбъект.ASP_КурсВалюты)*ДоляПозиции)/Выборка.КоличествоУпаковок); 
				ИначеЕсли ЭтотОбъект.Валюта.Код="156" Тогда
					НовЗапись.ЦенаВВалюте=((Выборка.СуммаСНДС/Выборка.КоличествоУпаковок))+(((ЭтотОбъект.ASP_СуммаДопрасходов/ЭтотОбъект.ASP_КурсВалюты)*ДоляПозиции)/Выборка.КоличествоУпаковок); 
				ИначеЕсли
					ЭтотОбъект.Валюта.Код="840" Тогда 
					НовЗапись.ЦенаВВалюте=((Выборка.СуммаСНДС/Выборка.КоличествоУпаковок)/ЭтотОбъект.ASP_КурсВалюты)+(((ЭтотОбъект.ASP_СуммаДопрасходов/ЭтотОбъект.ASP_КурсВалюты)*ДоляПозиции)/Выборка.КоличествоУпаковок); 
				КонецЕсли;    
				...
				КонецЕсли;                                        
				КонецЕсли;
				НовЗапись.Количество=Выборка.КоличествоУпаковок;
				НовЗапись.Курс=ЭтотОбъект.ASP_КурсВалюты;
			КонецЕсли;
		КонецЦикла;
