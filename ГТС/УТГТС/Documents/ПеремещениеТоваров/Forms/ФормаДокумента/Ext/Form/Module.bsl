...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ASP_СоздатьЗаказНаПеремещение(ТекущийОбъект); //ASP_yulia_l_14/01/2021_9412 //++ASP_Crow 11.03.2026 000029567 в расширение
...
Процедура УстановитьДоступностьОтметкиСклада()
	...
	Элементы.ГруппаПринятоВРаботу.ТолькоПросмотр = НЕ (РольДоступна("ASP_ОтметкаСклада") ИЛИ РольДоступна("ПолныеПрава"));
	Элементы.УстановитьОтметкуСклада.Доступность = (РольДоступна("ASP_ОтметкаСклада") ИЛИ РольДоступна("ПолныеПрава"));
	Элементы.ASP_УстановитьПринятоВРаботу.Доступность = (РольДоступна("ASP_ОтметкаСклада") ИЛИ РольДоступна("ПолныеПрава"));
	Если ЗначениеЗаполнено(Объект.ОтметкаСклада) ИЛИ ЗначениеЗаполнено(Объект.ПринятоВРаботу) Тогда
	...
	Элементы.ГруппаПринятоВРаботу.ТолькоПросмотр = Истина;
	Элементы.СкладПолучатель.ТолькоПросмотр = Истина;
	КонецЕсли;
	//ГТС устанавливаем права на отметки между двумя складами для доп.контроля изменений
	Если 
		Строка(Объект.СкладОтправитель) = "Склад временного хранения г.ТАЛДОМ" и строка(Пользователи.ТекущийПользователь().Подразделение) = "Склад Талдом" и
		Строка(Объект.СкладПолучатель) = "Склад Подольск" и не РольДоступна("ПолныеПрава") 
	Тогда 
		Элементы.ГруппаОтметкаСклада.ТолькоПросмотр = Истина;
	    Элементы.ГруппаПринятоВРаботу.ТолькоПросмотр = Ложь;
		Элементы.УстановитьОтметкуСклада.Доступность = Ложь;
	ИначеЕсли
		Строка(Объект.СкладОтправитель) = "Склад Подольск" и строка(Пользователи.ТекущийПользователь().Подразделение) = "Склад Москва" и
		Строка(Объект.СкладПолучатель) = "Склад временного хранения г.ТАЛДОМ" и не РольДоступна("ПолныеПрава")  
	Тогда
		Элементы.ГруппаОтметкаСклада.ТолькоПросмотр = Истина;
	    Элементы.ГруппаПринятоВРаботу.ТолькоПросмотр = Ложь;
		Элементы.УстановитьОтметкуСклада.Доступность = Ложь;
	ИначеЕсли
		Строка(Объект.СкладОтправитель) = "Склад Подольск" и строка(Пользователи.ТекущийПользователь().Подразделение) = "Склад Талдом" и
		Строка(Объект.СкладПолучатель) = "Склад временного хранения г.ТАЛДОМ" и не РольДоступна("ПолныеПрава")		
	Тогда
	    Элементы.ГруппаОтметкаСклада.ТолькоПросмотр = Ложь;
	    Элементы.ГруппаПринятоВРаботу.ТолькоПросмотр = Истина;
		Элементы.ASP_УстановитьПринятоВРаботу.Доступность = Ложь;
	ИначеЕсли
		Строка(Объект.СкладОтправитель) = "Склад временного хранения г.ТАЛДОМ" и строка(Пользователи.ТекущийПользователь().Подразделение) = "Склад Москва" и
		Строка(Объект.СкладПолучатель) = "Склад Подольск" и не РольДоступна("ПолныеПрава")		
	Тогда
	    Элементы.ГруппаОтметкаСклада.ТолькоПросмотр = Ложь;
	    Элементы.ГруппаПринятоВРаботу.ТолькоПросмотр = Истина;
...
&НаКлиенте
Процедура ASP_УстановитьПринятоВРаботу(Команда)
	Объект.ПринятоВРаботу=ТекущаяДата();
КонецПроцедуры

&НаСервере
Процедура ASP_СоздатьЗаказНаПеремещение(Объект) 
	//++ASP_Crow 11.03.2026 000029567 в расширение
	//+++ASP_yulia_l_14/01/2021_9412
	Если Объект.Статус = Перечисления.СтатусыПеремещенийТоваров.Принято И Объект.ДополнительныеСвойства.Свойство("Товары") Тогда      //yulia
		ТоварыПоступления = Объект.ДополнительныеСвойства.Товары;
		ЕстьНазначение = Ложь;
		УжеСозданЗаказ = Ложь;
		Для каждого Строка Из ТоварыПоступления Цикл 
			Если ЗначениеЗаполнено(Строка.Назначение) Тогда
				...
				|	И ЗаказНаПеремещениеТовары.Назначение = &Назначение
				...
				|	И ЗаказНаПеремещениеТовары.Ссылка.Дата = &Дата
				...
				Запрос.УстановитьПараметр("СкладОтправитель", Объект.СкладПолучатель);
				Запрос.УстановитьПараметр("СкладПолучатель", Заказ.Склад);
				
				РезультатЗапроса = Запрос.Выполнить();
				Если НЕ РезультатЗапроса.Пустой() Тогда
					УжеСозданЗаказ = Истина;
					ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
					Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
						Сообщить("По номенклатуре "+ строка.номенклатура+" с назначением " +Строка.Назначение+" уже создан документ "+ ВыборкаДетальныеЗаписи.Ссылка);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;	
		КонецЦикла;
		Если ЕстьНазначение И НЕ УжеСозданЗаказ Тогда 
			//МассивСтрок = Новый Массив;
			ТЗСтрок = Новый ТаблицаЗначений;
			ТЗСтрок.Колонки.Добавить("СтрокаТаб");
			ТЗСтрок.Колонки.Добавить("Назначение");
			ТЗСтрок.Колонки.Добавить("КолВоПерем"); //000018285
			Для каждого СтрокаТовар из ТоварыПоступления Цикл
				Если НЕ СтрокаТовар.Назначение = Справочники.Назначения.ПустаяСсылка() Тогда 
					Заказ = СтрокаТовар.Назначение.Заказ;
					ТоварыЗаказ = Заказ.Товары;
					
					НужнаяНоменклатура = ТоварыЗаказ.НайтиСтроки(Новый Структура("Номенклатура, Обособленно", СтрокаТовар.Номенклатура, Истина));
					
					Для Каждого СтрокаНоменклатура из НужнаяНоменклатура Цикл            //+++ASP_yulia_l_02/02/2021_9690
						Если СтрокаНоменклатура.Склад <> Объект.СкладПолучатель Тогда   //что бы не плодить много заказов на перемещение сделаем группировку по складам (по ним и будут отдельные заказы)
							строка = ТЗСтрок.Добавить();
							строка.СтрокаТаб = СтрокаНоменклатура;
							строка.Назначение = СтрокаТовар.Назначение; 
							//000018285+++
							строка.КолВоПерем = СтрокаТовар.Количество;
							Прервать;
							//000018285---
						КонецЕсли;	
					КонецЦикла;
			...
			//Получим склад на который будем отправлять и по нему создавать заказ на перемещение
			Пока НЕ ТЗСтрок.Количество() = 0 Цикл  
				СкладОтбор = ТЗСтрок[0].СтрокаТаб.Склад ; 
				Перемещение = Документы.ЗаказНаПеремещение.СоздатьДокумент();
				Перемещение.Дата = Объект.Дата+1 ;  
				Перемещение.СкладОтправитель = Объект.СкладПолучатель;   
				Перемещение.СкладПолучатель = СкладОтбор;   
				...
				Перемещение.Подразделение = Заказ.Подразделение;
				Перемещение.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеТоваров;
				Перемещение.СпособДоставки = Заказ.СпособДоставки;
				Перемещение.Приоритет = Справочники.Приоритеты.НайтиПоНаименованию("Средний");
				МассивКУдалению = Новый Массив();
				// индекс = 0 ;
				Для каждого СтрокаМ из ТЗСтрок Цикл 
					Если СтрокаМ.СтрокаТаб.Склад = СкладОтбор Тогда
						
						НовыйТовар = Перемещение.Товары.Добавить();
						//000018285+++
						//НовыйТовар.Количество = СтрокаМ.СтрокаТаб.Количество;
						//НовыйТовар.КоличествоУпаковок = СтрокаМ.СтрокаТаб.Количество;
						НовыйТовар.Количество = СтрокаМ.КолВоПерем;
						НовыйТовар.КоличествоУпаковок = СтрокаМ.КолВоПерем;
						//000018285---
						НовыйТовар.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада;
						НовыйТовар.НачалоОтгрузки = ТекущаяДата() + ((24*60*60)*3);
						НовыйТовар.Номенклатура = СтрокаМ.СтрокаТаб.Номенклатура;
						НовыйТовар.Назначение = СтрокаМ.Назначение;	   //изменить
						
						МассивКУдалению.Добавить(СтрокаМ); //собираем массив к удалению
					КонецЕсли;
				КонецЦикла;
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации("Проверка отказа.Создание из перемещения", УровеньЖурналаРегистрации.Ошибка, , Перемещение.Ссылка, "Ошибка проведения" + символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
					+ символы.ПС + ОписаниеОшибки());
					Сообщить("Записан и НЕ проведен документ "+Перемещение);
...
КонецПроцедуры //ASP_yulia_l_14/01/2021_9412
