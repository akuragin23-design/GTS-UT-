...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		//++ASP_Crow 25.05.2024 000021639 Возвращаем обеспечение заказов, для регламентного задания
		ТекстЗапросаТаблицаОбеспечениеЗаказов(Запрос, ТекстыЗапроса, Регистры);                      
		//--ASP_Crow 25.05.2024 000021639
		//--ASP_Crow 11.03.2026 000029567 в расширение
...
Процедура ЗаполнитьШапкуДокументаПоЗаказу(Объект, ПараметрыЗаполнения, МассивЗаказов) Экспорт
	...
	Объект.Дата = ?(МассивЗаказов.Количество() = 1,
		...
		ЗаказыСервер.ПолучитьМаксимальнуюДатуОтгрузкиЗаказа(МассивЗаказов, ПараметрыЗаполнения.ИмяРегистраЗаказ, 
			ПараметрыЗаполнения.ИмяПоляЗаказ));
	//++ASP_Crow 11.03.2026 000029567 в расширение
		//ГТС заполняем заказ клиента по основанию заказа на перемещение
		если ТипЗнч(МассивЗаказов[0].ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") тогда
			Объект.ЗаказКлиента = МассивЗаказов[0].ДокументОснование;
		КонецЕсли; 
		//--ASP_Crow 11.03.2026 000029567 в расширение
