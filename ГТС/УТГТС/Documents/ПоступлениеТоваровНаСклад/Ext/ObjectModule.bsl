...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//++ASP_Crow 30.05.2025 000027079 в тов орг заменим движение расхода потом
	ПоступлениеТоваровНаСкладЛокализация.ПередЗаписью(ЭтотОбъект, Отказ, РежимЗаписи, РежимПроведения); 
	//--ASP_Crow 30.05.2025 000027079
	//--ASP_Crow 11.03.2026 000029567 в расширение
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПоступлениеТоваровНаСкладЛокализация.asp_ОбработкаПроведения(ЭтотОбъект, Отказ, РежимПроведения); //++ASP_Crow 11.03.2026 000029567 в расширение
...
Процедура ЗаполнитьДокументНаОснованииАктаПриемкиПослеПоступления(ДанныеЗаполнения)
	...
	ОбщегоНазначенияУТ.ПроверитьВозможностьВводаНаОсновании(ДокументОснование,
															...
															Выборка.ЕстьОшибкиПроведен,
															Выборка.ЕстьОшибкиСтатус);
	Склад			= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Склад"); //++ASP_Crow 11.03.2026 000029567 в расширение
...
Процедура ЗаполнитьУпаковкиЦены(ТЗТовары, ТЗУпаковки)
	...
	Для Каждого СтрокаТоваров Из ТЗТовары Цикл
		...
		//++ASP_Crow 11.03.2026 000029567 в расширение
		//++ASP_Crow 31.05.2024 000023121 Попробуем хотя бы таким образом создать код строки, т.к. переработка регистра и формирования движений затронет больше времени
		// (это временный костыль)
		СтрокаТЧ.КодСтроки = СтрокаТЧ.НомерСтроки;
		СтрокаТЧ.ИдентификаторСтроки = новый УникальныйИдентификатор;
		//--ASP_Crow 31.05.2024 000023121   
		//--ASP_Crow 11.03.2026 000029567 в расширение
