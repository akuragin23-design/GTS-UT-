...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	// Альтер ЛОГО+: Добавлено в связи с добавлением типа заполнения на основании Сборка товаров
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.СборкаТоваров") Тогда
		...
		Характеристика = ДокументОснование.Характеристика;
		Количество = ДокументОснование.Количество;
		КоличествоУпаковок = ДокументОснование.КоличествоУпаковок;
		Упаковка = ДокументОснование.Упаковка;
		Склад = ДокументОснование.Склад;
		Организация = ДокументОснование.Организация;
		//УдалитьСтатусОтгрузки = Перечисления.УдалитьСтатусыОтгрузки.Отгружено;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	//++ASP_Crow 10.03.2026 000029567 в расширение
	//ASP_Dante 01/09/19
	Если Дата>=Дата("20121129") и ПустаяСтрока(ВариантКомплектации) Тогда
		Сообщить("Укажите вариант комплектации!");
		Отказ=Истина;
	КонецЕсли;
	//ASP_Dante 01/09/19
	...
	СборкаТоваровЛокализация.ASP_ОбработкаПроведения(ЭтотОбъект, Отказ, РежимПроведения);//+ASP ЯмаеваН 23.12.2021
...
Процедура РаспределитьНомераГТД(Отказ) Экспорт
	//++ASP_Crow 01.07.2026 Сборка изменилась, в типовом варианте прошлой версии было иначе, верну на старый т.к. он как минимум работал по нужной логике
	#Область НовыйТиповой
...
// Альтер ЛОГО+: добавлено в связи с добавлением типа заполнения по основанию Сборка товаров
Процедура ЗаполнитьПоСборкеТоваров()   
	...
	|	ВЫБОР
	|		КОГДА ЦеныНоменклатурыСрезПоследних.Упаковка ЕСТЬ NULL
	|			ТОГДА 0
	|		КОГДА ЦеныНоменклатурыСрезПоследних.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА ЦеныНоменклатурыСрезПоследних.Цена * СборкаТоваровТовары.Количество
	|		ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Цена * СборкаТоваровТовары.Количество * ЦеныНоменклатурыСрезПоследних.Упаковка.Числитель / ВЫБОР
	|				КОГДА ЦеныНоменклатурыСрезПоследних.Упаковка.Знаменатель = 0
	|					ТОГДА 1
	|				ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Упаковка.Знаменатель
	|			КОНЕЦ
	|	КОНЕЦ КАК СебестоимостьПоЗакупочнойЦене,
	|	СборкаТоваровСерии.Серия КАК Серия,
	|	СборкаТоваровСерии.Назначение КАК Назначение
	|ИЗ
	|	Документ.СборкаТоваров.Товары КАК СборкаТоваровТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаОстатков, ВидЦены = &ВЦ_Закупочная) КАК ЦеныНоменклатурыСрезПоследних
	|		ПО СборкаТоваровТовары.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|			И СборкаТоваровТовары.Характеристика = ЦеныНоменклатурыСрезПоследних.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СборкаТоваров.Серии КАК СборкаТоваровСерии
	|		ПО СборкаТоваровТовары.Номенклатура = СборкаТоваровСерии.Номенклатура
	|			И СборкаТоваровТовары.Характеристика = СборкаТоваровСерии.Характеристика
	|ГДЕ
	|	СборкаТоваровТовары.Ссылка = &Основание
	|ИТОГИ
	|	СУММА(СебестоимостьПоЗакупочнойЦене)
	|ПО
	|	ОБЩИЕ";
	Запрос.УстановитьПараметр("Основание", ДокументОснование);
	Запрос.УстановитьПараметр("ДатаОстатков", ДокументОснование.Дата);
	Запрос.УстановитьПараметр("ВЦ_Закупочная", Справочники.ВидыЦен.Закупочная);
	///
	ВыборкаИтог = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Если ВыборкаИтог.Следующий() Тогда
		СебестоимостьВсего = ВыборкаИтог.СебестоимостьПоЗакупочнойЦене;
		
		Выборка = ВыборкаИтог.Выбрать();
		Пока Выборка.Следующий() Цикл
			СтрокаТЧ = Товары.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, Выборка);
			СтрокаТЧ.ДоляСтоимости = ?(СебестоимостьВсего = 0, 0, Выборка.СебестоимостьПоЗакупочнойЦене * 100 / СебестоимостьВсего);
			//
	...
	КонецЕсли;
	//
	Если Товары.Итог("ДоляСтоимости")<>100 тогда
		Излишек = Товары.Итог("ДоляСтоимости")-100;
		ТЗ = Товары.Выгрузить();
		ТЗ.Сортировать("ДоляСтоимости Убыв");
		СтрокаМаксимума = ТЗ[0];
		СтрокаМаксимума.ДоляСтоимости = СтрокаМаксимума.ДоляСтоимости - Излишек;
		Товары.Загрузить(ТЗ);
	Конецесли;
	
КонецПроцедуры//ЗаполнитьПоСборкеТоваров()	
// +Альтер ЛОГО
