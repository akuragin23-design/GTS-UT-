...
&НаКлиенте
Процедура ПодобратьВодителя(Команда)   
	//ASP_Crow 18.03.2026 000029567 В расширение
	ПараметрыПодбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора, МножественныйВыбор", Истина, Истина, Ложь);
	ОткрытьФорму("Справочник.ВодителиГТС.ФормаВыбора", ПараметрыПодбора, ЭтаФорма);
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьДанныеВыбора(Элемент)
	Объект.Водитель = Элемент.Владелец;
	Объект.АвтомобильГосударственныйНомер = Элемент.АвтомобильНомер;
	Объект.АвтомобильМарка = Элемент.АвтомобильМарка;
	Объект.Перевозчик = Элемент.Контрагент;
	Объект.УдостоверениеСерия = Элемент.УдостоверениеСерия;
	Объект.УдостоверениеНомер = Элемент.УдостоверениеНомер;
	Объект.ВидПеревозки = "Грузовая";
	Объект.БанковскийСчетПеревозчика = Элемент.БанковскийСчетКонтрагента; 
	Объект.ASP_Водитель=Элемент.Ссылка; 
	Объект.АвтомобильВместимостьВКубическихМетрах = Элемент.Вместимость;
	Объект.АвтомобильГрузоподъемностьВТоннах=Элемент.Грузоподъемность;
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	ЗаполнитьДанныеВыбора(ВыбранноеЗначение);
КонецПроцедуры
