...
Функция СоздатьТранспортныеНакладные(МассивОбъектов, 
	...
	//ASP пересчет веса	
	#Область ASP_ПересчетВеса
	_МассивОснований	= ТранспортнаяНакладнаяОбъект.ДокументыОснования.Выгрузить().ВыгрузитьКолонку("ДокументОснование");
	_ТипыОбъектов	= ОбщегоНазначенияУТ.РазложитьМассивСсылокПоТипам(_МассивОснований);
	...
	Запрос.Текст = Документы.ЗаданиеНаПеревозку.ТекстЗапросаПолученияСпискаНакладныхИзЗаданийНаПеревозку(Истина)
					+ ОбщегоНазначения.РазделительПакетаЗапросов()
					+ Документы.ТранспортнаяНакладная.ТекстВТМассыИзОснований(_ТипыОбъектов, Запрос);
	
	ЗаданияНаПеревозку = Новый Массив;
	
	Если ЗначениеЗаполнено(ТранспортнаяНакладнаяОбъект.ЗаданиеНаПеревозку) Тогда
		ЗаданияНаПеревозку.Добавить(ТранспортнаяНакладнаяОбъект.ЗаданиеНаПеревозку);
	КонецЕсли;
	...
	Если НЕ Выборка.Количество() = 0 Тогда
		...
		ТранспортнаяНакладнаяОбъект.МассаБрутто	= 0;
		ТранспортнаяНакладнаяОбъект.МассаНетто	= 0;
...
Функция СформироватьПечатнуюФормуТранспортнойНакладной(МассивОбъектов, ОбъектыПечати, КомплектыПечати = Неопределено) Экспорт
	...
	|   ТаблицаТранспортныхНакладных.ДокументОснование.Валюта КАК Валюта,
	|   ТаблицаТранспортныхНакладных.ДокументОснование.Дата КАК ДатаОснования
	///ASP #327
	|ИЗ
	|	ТаблицаТранспортныхНакладных КАК ТаблицаТранспортныхНакладных
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
...
Процедура ЗаполнитьТабличныйДокументТНВерсияС2022(ТабличныйДокумент, ОбъектыПечати, ДанныеСсылкиДокументов, ДанныеПечати, Макет)
	...
	//ASP_Crow 06.03.2026 000029567 Требуется перенос в расширение
	...
	//++ASP_Трунькин 000028167 25/09/25
	МассивРеализаций = ДанныеПечати.Ссылка.ДокументыОснования.Выгрузить().ВыгрузитьКолонку("ДокументОснование");
	//Проверка на реализацию
	ДокументСсылка = МассивРеализаций[0];
	БеремСуммуИзРегистраСуммыДокументовВВалютахУчета = Ложь;
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		ЗапросСуммыРегл = Новый Запрос;
		ЗапросСуммыРегл.Текст = 
		"ВЫБРАТЬ
		|	СуммыДокументовВВалютахУчетаСрезПоследних.Регистратор КАК Регистратор,
		|	СУММА(СуммыДокументовВВалютахУчетаСрезПоследних.СуммаБезНДСРегл + СуммыДокументовВВалютахУчетаСрезПоследних.СуммаНДСРегл) КАК Поле1
		|ПОМЕСТИТЬ вттч
		|ИЗ
		|	РегистрСведений.СуммыДокументовВВалютахУчета.СрезПоследних КАК СуммыДокументовВВалютахУчетаСрезПоследних
		|ГДЕ
		|	СуммыДокументовВВалютахУчетаСрезПоследних.Регистратор В(&Регистратор)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(вттч.Поле1) КАК Сум
		|ИЗ
		|	вттч КАК вттч";
		
		ЗапросСуммыРегл.УстановитьПараметр("Регистратор", МассивРеализаций);
		
		РезультатЗапроса = ЗапросСуммыРегл.Выполнить();
		
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		БеремСуммуИзРегистраСуммыДокументовВВалютахУчета = Истина;
	КонецЕсли;
	//--ASP_Трунькин 000028167 25/09/25
	...
	ВалютаРегламентированогоУчета	= Константы.ВалютаРегламентированногоУчета.Получить();
   	...
   	//++ASP_Трунькин 000028167 25/09/25
	Если БеремСуммуИзРегистраСуммыДокументовВВалютахУчета Тогда
		ВыборкаДетальныеЗаписи.Следующий();
		СумМ = ВыборкаДетальныеЗаписи.Сум;
	Иначе
	...
	//++ГТС 26.02.2022 переносим заполнение перевозчика для нового пункта 8_0
	Если ЗначениеЗаполнено(ДанныеПечати.Перевозчик) Тогда 
		Если СведенияОПеревозчике.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо
		 Или СведенияОПеревозчике.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель Тогда
			ПредставлениеПеревозчика = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПеревозчике, 
