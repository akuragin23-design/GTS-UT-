...
Функция ПолучитьДанныеДляПечатнойФормыУПД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	НомераГТД.РегистрационныйНомер КАК НомерТовара,
	|	ТаблицаДокумента.НомерГТД.СуммаПоРНПТ КАК ОбщаяСуммаПоКомплекту,
	|	ТаблицаДокумента.НомерГТД.СуммаПоРНПТ КАК СуммаПоРНПТ,
	|	ТаблицаДокумента.НомерГТД.КоличествоКомплектующих КАК Количество,
	|	ТаблицаДокумента.НомерГТД.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ТаблицаДокумента.НомерГТД.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияКод,
	|	ТаблицаДокумента.НомерГТД.ЕдиницаИзмерения.Представление КАК ЕдиницаИзмеренияНаименование
	|ИЗ
	|	РеализацияТоваровУслугТаблицаТоваров КАК ТаблицаДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НомераГТД КАК НомераГТД
	|		ПО ТаблицаДокумента.НомерГТД = НомераГТД.Ссылка
	|ГДЕ
	|	НомераГТД.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)";
