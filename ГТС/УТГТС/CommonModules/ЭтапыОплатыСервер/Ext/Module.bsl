	...
	Для Каждого Этап Из ШаблонГрафика Цикл
		...
		Иначе
			...
			Для Каждого СтрокаДатыОтгрузки Из ТаблицаОтгрузок Цикл
				...
				//ASP_Dante 11/10/19 //ASP_Crow 11.03.2026 000029567 В расширение
...
Процедура ЗаполнитьЭтапыОплатыДокументаПродажиПоШаблону(Объект,
	...
	Если ЗначениеЗаполнено(Календарь) Тогда
		...
		Для Каждого Этап Из ШаблонГрафика Цикл
			...
			Если Этап.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.ПредоплатаДоОтгрузки Или
				Этап.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.АвансДоОбеспечения Тогда
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ДатаРеализации) Тогда
			...
			Для Каждого ТекЭтап Из ШаблонГрафика Цикл
				...
				Если ТекЭтап.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.ПредоплатаДоОтгрузки Или
					ТекЭтап.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.АвансДоОбеспечения Тогда
	...
	Для Каждого Этап Из ШаблонГрафика Цикл
		...
		Иначе
			ДатаПлатежа = ?(ЭтапГрафикаОплаты.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.КредитПослеОтгрузки
							ИЛИ ЭтапГрафикаОплаты.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.КредитСдвиг, ДатаРеализации, ДатаДокумента) + Этап.Сдвиг * ОдинДень;
		КонецЕсли;
		...
		//КонецЕсли;
		ЭтапГрафикаОплаты.ВариантОтсчета = Перечисления.ВариантыОтсчетаДатыПлатежа.ОтДатыЗаказа;
		//ASP_Dante 11/10/19
		
		Если ЭтапГрафикаОплаты.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.КредитСдвиг Тогда
			ЭтапГрафикаОплаты.Сдвиг           = Этап.Сдвиг;
		КонецЕсли;
				ЭтапГрафикаОплаты.ДатаПлатежа         = ДатаПлатежа;
