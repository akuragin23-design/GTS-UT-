...
//
Процедура ASP_ПроверитьНаличиеИзделийВТЧСУчетомПоГТД(ДокументОбъект, Отказ, ИмяТЧ = "ВыходныеИзделия") Экспорт
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатков(ДокументОбъект,
	...
	Если КонтролироватьИзменениеНомеровГТД Тогда
		ТаблицаНомеровГТД = ТаблицаВидыЗапасов.Скопировать(, "АналитикаУчетаНоменклатуры, НомерГТД, Количество"); // ТаблицаЗначений
	КонецЕсли;
	//000025702 АСП Лебедев 16/01/24 доработка по корректировки реализации для определения корректировки в дальнейшем
	ПараметрыЗаполненияВидовЗапасов.Вставить("ТипДок", ТипЗнч(ДокументОбъект)); //ASP_Crow 11.03.2026 000029567 В расширени
...
Процедура ОтразитьЗапасыИПотребности(ДополнительныеСвойства, Движения, Отказ) Экспорт  
	//ASP_Crow 11.03.2026 000029567 В расширени

	Таблица = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыИПотребности;
	
	Если Отказ ИЛИ Таблица.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Движения.ЗапасыИПотребности.Записывать = Истина;
	Движения.ЗапасыИПотребности.Загрузить(Таблица);
...
//	ПараметрыЗаполненияВидовЗапасов - Неопределено, Структура - См. ЗапасыСервер.ПараметрыЗаполненияВидовЗапасов().
//
Процедура ASP_СформироватьРезервыПоТоварамОрганизаций(ДокументОбъект, Отказ, ПараметрыЗаполненияВидовЗапасов) Экспорт
	//ASP_Crow 11.03.2026 000029567 В расширени
...
Функция ТаблицаОстатковТоваровПереданныхПереработчику(ДокументОбъект, ПараметрыЗаполненияВидовЗапасов, МенеджерВременныхТаблиц, ДатаОстатков)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |					Т.ВидЗапасов
	               |				ИЗ
	               |					ВтОтборыОстатковОрганизаций КАК Т)) КАК Остатки
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |				И (Движения.ВидЗапасов = НаКонецМесяца.ВидЗапасов)) КАК Товары
	               |
	               |СГРУППИРОВАТЬ ПО
...
//++ASP_Crow 10.07.2024 000023822 Взято из типовой УТ
// Заполняет виды запасов в табличной части "ВидыЗапасов" документа.
//
// Параметры:
//	ДокументОбъект - ДокументОбъект - Документ, в котором заполняются виды запасов.
...
//	ПараметрыЗаполненияВидовЗапасов - Неопределено, Структура - См. ЗапасыСервер.ПараметрыЗаполненияВидовЗапасов.
//
Процедура ЗаполнитьВидыЗапасовПоОстаткамКОформлению_Типовой(ДокументОбъект,
													МенеджерВременныхТаблиц,
													Отказ,
													ПараметрыЗаполненияВидовЗапасов) Экспорт
	//ASP_Crow 11.03.2026 000029567 В расширени
