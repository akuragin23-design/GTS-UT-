...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
	...
	// Альтер ЛОГО+ если это документ и его дата меньше определенной в константе ДатаКонтроляВыгрузкиДокументовВБП //ASP_Crow 11.03.2026 000029567 В расширени
	Если Найти(строка(ТипЗнч(Объект)), "Документ") = 1 тогда
		//Асист-Про Начало Аграновский 2013-08-18
		//Если Объект.ПометкаУдаления Тогда
			//Не переносятся пометки удаления.Документ сделали, прощел обмен, потом поставили пометку.
			//Возврат;
		//ИначеЕсли Объект.Дата <= константы.ДатаКонтроляВыгрузкиДокументовВБП.Получить() И ЗначениеЗаполнено(Объект.Ссылка) тогда
		Если Объект.Дата <= КонецДня(константы.ДатаКонтроляВыгрузкиДокументовВБП.Получить()) И ЗначениеЗаполнено(Объект.Ссылка) тогда
		//Асист-Про Конец Аграновский 2013-08-18	
			Рег = РегистрыСведений.ЗарегистрированныеДокументыКПроверке.СоздатьМенеджерЗаписи();
			Рег.Документ = Объект.Ссылка;
			Рег.Записать(Истина);
			//
			Возврат;
		КонецЕсли;
	Конецесли;		
	// +Альтер ЛОГО //ASP_Crow 11.03.2026 000029567 В расширени
	...
	Попытка
		...
		//++000026792 ASP_Мочалина проверка был ли изменен документ перед постановкой его на регистрацию в БП. //ASP_Crow 11.03.2026 000029567 В расширени	
			Если Найти(строка(ТипЗнч(Объект)), "Документ") = 1 и ЗначениеЗаполнено(Объект.Ссылка) тогда 
				ОбъектМодифицирован = ASP_Добавленный.СравнитьВерсииОбъекта(Объект);
			КонецЕсли;
...
Процедура ВыполнитьПравилоРегистрацииОбъектаДляСсылочногоТипа(МассивУзловРезультат,
	...
	// ПРОБ
	// Альтер ЛОГО+: закоментировано!!! //ASP_Crow 11.03.2026 000029567 В расширени
...
/////
//////// Альтер ЛОГО ////////////////////////
Процедура ЗарегистрироватьОбъектыПоСсылкам(УзелДляОбмена) Экспорт  
	//ASP_Crow 11.03.2026 000029567 В расширени
	
	// выполняем процедуру в транзакции на случай сбоев в регистрации связанных ссылок
	НачатьТранзакцию();
	
	Кэш = Новый СписокЗначений();
	ИмяПланаОбмена = ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(УзелДляОбмена);
	// Обрабатываем зарегистрированные ссылки так, чтобы получить полный список связанных ссылок
	Выборка = ПланыОбмена.ВыбратьИзменения(УзелДляОбмена, УзелДляОбмена.НомерОтправленного);
	Пока Выборка.Следующий() Цикл
		Данные = Выборка.Получить();
		// Пока обрабатываем зарегистрированные объекты только ссылочного типа
		Попытка
		...
		// Обработаем ссылку
		//ОбработатьСсылкуКРегистрации(Кэш, УзелДляОбмена, СсылкаНаОбъект, ИмяПланаОбмена, Истина);
	КонецЦикла;
