...
Функция ЗаполнитьЦены(ТабличнаяЧасть,
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, СтруктураЗаполнения);
			// Альтер ЛОГО+ //ASP_Crow 16.03.2026 000029567 В расширение
				Если АльтерЛого.РасчитатьЦенуПоКомплекту(СтрокаТЧ.Номенклатура) тогда
				если не ЗначениеЗаполнено(СтрокаТЧ.Цена) Тогда
				Коэффициент = КоэффициентНаценки(СтрокаТЧ.Номенклатура);
				СтрокаТЧ.Цена = АльтерЛого.ЦенаНоменклатурыПоКомплекту(СтрокаТЧ.Номенклатура, ?(ЗначениеЗаполнено(Параметры.Дата), Параметры.Дата, ТекущаяДатаСеанса()), Выборка.ВидЦены, СтрокаТЧ.Цена, Коэффициент);
				конецесли;
				КонецЕсли;
			// Альтер ЛОГО+
			Если СтруктураДействий <> Неопределено Тогда
...
Процедура АвтоматическоеЗакрытиеСделок() Экспорт 
	...
	//ДатаНачала = НачалоДня(НачалоДня(ТекущаяДата()) - 60 * 24 * 3600);    //Ковровцев А.С. Теперь статус сделки берется из заказа при изменении заказа
	...
	//	
	//	//ASP КАВ начало  (п.2.3)
	//	//Сделка.Статус = Перечисления.СтатусыСделок.Выиграна;
	//	Если Не Сделка.Статус = Перечисления.СтатусыСделок.Выиграна и Не Сделка.Закрыта Тогда
	//		Сделка.Статус = Перечисления.СтатусыСделок.ВыставленСчет;
	//	ИначеЕсли Не Сделка.Статус = Перечисления.СтатусыСделок.Выиграна и  Сделка.Закрыта и Не Сделка.ПричинаПроигрышаСделки.Пустая()  Тогда
	...
	//ASP КАВ начало	
	//Закрытие сделок на основании поступлений оплат (п. 2.2)
	//Запрос = Новый Запрос;
	//Запрос.Текст = "ВЫБРАТЬ
	//               |	ПоступлениеБезналичныхДенежныхСредствРасшифровкаПлатежа.Заказ.Сделка КАК Сделка
	//               |ИЗ
	...
	//               |	И ТИПЗНАЧЕНИЯ(ПоступлениеБезналичныхДенежныхСредствРасшифровкаПлатежа.Заказ) = ТИП(Документ.ЗаказКлиента)
	//               |	И ПоступлениеБезналичныхДенежныхСредствРасшифровкаПлатежа.Заказ.Сделка.Статус <> &Статус
	//               |
	//               |СГРУППИРОВАТЬ ПО
	//               |	ПоступлениеБезналичныхДенежныхСредствРасшифровкаПлатежа.Заказ.Сделка
	//               |
	//               |ОБЪЕДИНИТЬ ВСЕ
	//               |
	//               |ВЫБРАТЬ
	//               |	ПриходныйКассовыйОрдерРасшифровкаПлатежа.Заказ.Сделка
	//               |ИЗ
	//               |	Документ.ПриходныйКассовыйОрдер.РасшифровкаПлатежа КАК ПриходныйКассовыйОрдерРасшифровкаПлатежа
	//               |ГДЕ
	//               |	ПриходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	//               |	И ТИПЗНАЧЕНИЯ(ПриходныйКассовыйОрдерРасшифровкаПлатежа.Заказ) = ТИП(Документ.ЗаказКлиента)
	//			   |    И ПриходныйКассовыйОрдерРасшифровкаПлатежа.Заказ.Сделка.Статус <> &Статус
	//               |
	//               |СГРУППИРОВАТЬ ПО
	//               |	ПриходныйКассовыйОрдерРасшифровкаПлатежа.Заказ.Сделка
	//               |
	...
	//               |
	//               |ВЫБРАТЬ
	//               |	ОперацияПоПлатежнойКартеРасшифровкаПлатежа.Заказ.Сделка
	//               |ИЗ
	//               |	Документ.ОперацияПоПлатежнойКарте.РасшифровкаПлатежа КАК ОперацияПоПлатежнойКартеРасшифровкаПлатежа
	//               |ГДЕ
	//               |	ОперацияПоПлатежнойКартеРасшифровкаПлатежа.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	//               |	И ТИПЗНАЧЕНИЯ(ОперацияПоПлатежнойКартеРасшифровкаПлатежа.Заказ) = ТИП(Документ.ЗаказКлиента)
	//			   |    И ОперацияПоПлатежнойКартеРасшифровкаПлатежа.Заказ.Сделка.Статус <> &Статус
	//               |
	//               |СГРУППИРОВАТЬ ПО
	//               |	ОперацияПоПлатежнойКартеРасшифровкаПлатежа.Заказ.Сделка";
	//Запрос.УстановитьПараметр("ДатаНачала", ДатаНачала);
	//Запрос.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	//Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыСделок.Выиграна);
	//
	//Выборка = Запрос.Выполнить().Выбрать();
	//Пока Выборка.Следующий() Цикл
	//	Сделка = Выборка.Сделка.ПолучитьОбъект();
