...
Процедура Подключаемый_ОткрытьФормуПлатежнойСсылки(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт 
	...
	//ASP_Dante 23/12/19 + sig 14/02/20 перенес в серверную функцию
	Если ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.ЗаказКлиента") и ASP_Добавленный.ASP_ИнтеграцияСЯндексКассойОткрытьФормуПлатежнойСсылкиДоп(ПараметрКоманды) Тогда
		ПараметрыВыполненияКоманды.Форма.ОбновитьОтображениеДанных();
	КонецЕсли;
	//ASP_Dante 23/12/19
КонецПроцедуры
