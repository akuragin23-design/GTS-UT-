...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	//-- Локализация
	//++ASP_Crow 30.05.2024 000023121 Если товары в пути, то пока не будем делать резервирвоание т.к. товары еще не на складе
	МассивТоварыВПути = новый Массив;
	МассивТоварыВПути.Добавить(Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСТоварыВПути);
	МассивТоварыВПути.Добавить(Перечисления.ХозяйственныеОперации.ЗакупкаПоИмпортуТоварыВПути);
	МассивТоварыВПути.Добавить(Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаТоварыВПути);
	Если МассивТоварыВПути.Найти(Объект.ХозяйственнаяОперация) = Неопределено Тогда
		//ASP_Dante 28/11/19
		Если Не Отказ И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			...
			Для каждого Строка Из Объект.Товары Цикл
				Если ЗначениеЗаполнено(Строка.Назначение) Тогда
					...
					КонецЕсли; 
					Строка.ASP_ЗаказКлиента = Строка.Назначение.Заказ;
					Строка.Назначение = Справочники.Назначения.ПустаяСсылка();
					
				КонецЕсли;		
			КонецЦикла; 
			
			Если ЕстьНазначение И ЗначениеЗаполнено(Объект.ЗаказПоставщику) Тогда
				...
				ПараметрыПоиска = Новый Структура("Номенклатура, КодСтроки");
				Для каждого Строка Из Товары Цикл
					
					//+++ {{ ASP 01.02.2021 000009653 }}
					// имеющийся алгоритм модифицирован с целью скорректировать при разбиении строк Заказа поставщику
					//заполнение цены/суммы и количества. основная логика данного алгоритма оставлена без изменений.
					
					//прежний код закомментирован:
					...
					//			СтрокаНовая = ЗаказПоставщику.Товары.Добавить();
					...
					//			asp_ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействий, ЗаказПоставщику);	
					//			
					...
					//			СтрокаНовая.Сумма = СтрокаНовая.Сумма - СтрокаЗаказа.Сумма;
					//			СтрокаНовая.СуммаРучнойСкидки = СтрокаНовая.СуммаРучнойСкидки - СтрокаЗаказа.СуммаРучнойСкидки;
					//			СтрокаНовая.СуммаНДС = СтрокаНовая.СуммаНДС - СтрокаЗаказа.СуммаНДС;
					//			СтрокаНовая.СуммаСНДС = СтрокаНовая.СуммаСНДС - СтрокаЗаказа.СуммаСНДС;
					//		КонецЕсли;
					...
					Если ЗначениеЗаполнено(Строка.Назначение) Тогда
						...
						Если МассивСтрок.Количество() = 0 Тогда
							ASP_ДобавитьСтрокуВЗаказПоставщику(ЗаказПоставщику, Строка, Ложь);
						Иначе
...
Процедура ASP_ОбработатьПриНеобходимостиИзменениеКоличества(ЗаказПоставщику, СтрокаЗаказа, ТекущееКоличество) 
	//ASP_Crow 11.03.2026 000029567 В расширени
