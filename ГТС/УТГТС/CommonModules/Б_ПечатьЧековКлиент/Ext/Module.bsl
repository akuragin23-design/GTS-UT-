...
Процедура ОбработатьДокумент(Элемент)
	...
	Если МенеджерОборудованияВызовСервера.ДанныеФискальнойОперации(АктивныйДокумент) <> Неопределено тогда
...
Процедура НапечататьЧекДокумента(Элемент)
	...
	АктивныйДокумент = Элемент.Документ;
	СуммаДокумента = Б_ПечатьЧековСервер.ПолучитьСуммуДокумента(АктивныйДокумент);
	...
	ФормаПредпросмотрЧека.ПараметрыККТ 						= Б_ПечатьЧековСервер.ВернутьПараметрыККТ();
	ФормаПредпросмотрЧека.ПараметрыОперацииФискализацииЧека = Б_ПечатьЧековСервер.ВернутьПараметрыОперацииФискализацииЧека(АктивныйДокумент);
	
	ПараметрыЭквайринговойОперации = Новый Структура;
	ПараметрыЭквайринговойОперации.Вставить("Сумма", СуммаДокумента);
	ПараметрыЭквайринговойОперации.Вставить("ТипТранзакции", "AuthorizeSales");
	ФормаПредпросмотрЧека.ПараметрыЭквайринговойОперации = ПараметрыЭквайринговойОперации;
	ФормаПредпросмотрЧека.СуммаДокумента = СуммаДокумента;
	ФормаПредпросмотрЧека.СуммаОплаты = СуммаДокумента;
	...
	Если лВариантОтправкиЭлектронногоЧека = ПредопределенноеЗначение("Перечисление.ВариантыОтправкиЭлектронногоЧекаПокупателю.ОтправитьEmail") тогда
		...
		ФормаПредпросмотрЧека.ПараметрыОперацииФискализацииЧека.ПокупательEmail = ФормаПредпросмотрЧека.Email;	
	ИначеЕсли лВариантОтправкиЭлектронногоЧека = ПредопределенноеЗначение("Перечисление.ВариантыОтправкиЭлектронногоЧекаПокупателю.ОтправитьSMS") тогда
		...
		ФормаПредпросмотрЧека.ПараметрыОперацииФискализацииЧека.ПокупательНомер = ФормаПредпросмотрЧека.Телефон;	
	КонецЕсли;
	
	Попытка
		
		ФормаПредпросмотрЧека.Открыть();
