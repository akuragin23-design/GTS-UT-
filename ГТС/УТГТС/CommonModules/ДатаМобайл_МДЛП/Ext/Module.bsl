...
Функция ЧистаяСтрока(Знач ГрязнаяСтрока)
	...
	ГрязнаяСтрока = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(ГрязнаяСтрока, "");	
...
Процедура ОбработатьУпрСимволыВМассивМарок(МассивМарок)

	ОбновленныйМассив = Новый Массив;
	
	
	Для Каждого ЭлементМассива из МассивМарок Цикл		
		ОбновленныйМассив.Добавить(УбратьИзМаркиУпрСимволы(ЭлементМассива));	
	КонецЦикла;
	
	МассивМарок = ОбновленныйМассив; 
...
Функция ПолучитьДокументМаркировки(ИсходныйДокумент, ДокументТСД, ЭтоНовыйДокумент,СтрокаОшибок) Экспорт
	...
	ИначеЕсли Шаблон.ВидДокумента = "РеализацияТоваровУслуг" или Шаблон.ВидДокументаНового = "РеализацияТоваровУслуг" Тогда
...
Функция ОбработатьВидДокументаУведомлениеОбОтгрузкеМДЛП(ИсходныйДокумент, ДокументТСД, ЭтоНовыйДокумент) Экспорт
	...
	//ИсходныйДокумент.Комментарий = "DataMobile " + ДокументТСД.Комментарий;
	ДобавитьКомментарий(ИсходныйДокумент, ДокументТСД, Шаблон);
...
Процедура ОбработатьИзменениеТЧПодбор_Отгрузка(ДокументТСД, ИсходныйДокумент, ИмяТЧПодбор, ТЧПриЗагрузкеПодбор)
	...
	Для Каждого Стр из ТЗСобранныеДанныеПодбор Цикл
		...
		Если  ЗначениеЗаполнено(Стр.Номенклатура) тогда
			...
			Иначе
				...
				СтруктураСтроки.Цена = СтрокиПоиска[Сч].Цена;
				СтруктураСтроки.СуммаНДС = СтрокиПоиска[Сч].СуммаНДС;
				СтруктураСтроки.СтатусУказанияСерий = СтрокиПоиска[Сч].СтатусУказанияСерий; 
				СтруктураСтроки.ИдентификаторСтроки = СтрокиПоиска[Сч].ИдентификаторСтроки; 
			КонецЕсли;			
...
Процедура ОбработатьИзменениеТЧПодбор_Реализация(ДокументТСД, ИсходныйДокумент, ИмяТЧПодбор, КопияТЧ, Шаблон)
	...
	ТЗСобранныеДанныеПодбор = ДокументТСД.СобранныеДанныеПодбор.Выгрузить();
	ТЗСобранныеДанныеПодбор.Свернуть("Номенклатура, ХарактеристикаНоменклатуры,СерияСсылка,ЕдиницаИзмерения", "Количество");
	
	СерииВОтдельнойТЧ = Шаблон.СерииВОтдельнойТЧ;
	лЕстьСерия=(Шаблон.ИспользованиеСерийниковПодбор>0 и (ИсходныйДокумент.метаданные().ТабличныеЧасти.Товары.Реквизиты.Найти("Серия")<>Неопределено)) И Не СерииВОтдельнойТЧ;
	
	КопияТЧ = ?(КопияТЧ.Количество() = 0, Неопределено, КопияТЧ);
	...
	Для Каждого СтрТЧ из ТЗСобранныеДанныеПодбор Цикл
		...
		Если ИсходныйДокумент.Метаданные().Имя = "ВыводИзОборотаИСМП" И СтрТЧ.Номенклатура.ОсобенностьУчета.Пустая() тогда
			Продолжить;	
		КонецЕсли;
		...
		Если ИсходныйДокумент.Метаданные().Имя="МаркировкаТоваровИСМП" тогда 
		...
		Если КопияТЧ <> Неопределено И (ИсходныйДокумент.Метаданные().Имя = "РеализацияТоваровУслуг" Или ИсходныйДокумент.Метаданные().Имя = "ВыводИзОборотаИСМП") Тогда
		...
		Если СтрокиПоиска.Количество() <> 0 Тогда
			...
			Если КопияТЧ <> Неопределено Тогда
				...
				Для каждого СтрПоиска Из СтрокиПоиска Цикл
					...
					Если ИсходныйДокумент.Метаданные().Имя = "ВыводИзОборотаИСМП" Тогда
				...
				Если лЕстьИдентификаторСтроки И ПустаяСтрока(СтрокаТабличнойЧасти.ИдентификаторСтроки) Тогда
