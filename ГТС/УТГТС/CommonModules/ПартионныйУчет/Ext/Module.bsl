...
Процедура РаспределитьПрочиеРасходыПоБазе(НачалоПериода, ОкончаниеПериода, Организации, РассчитатьВсе = Ложь, КоличествоЗаписейВсего = 0) Экспорт
	//ASP_Crow 05.03.2026 000029567 нет в старой и новой версии, возможно устарело
	...
	////ASP_Dante 12/07/21 Задача 000011492 перенос доработокВернутьДокументыРаспределенияРасходовПоБазеКОтражениюВРеглУчете(РегистраторыПрочиеРасходы);
	////ASP_Dante 19/07/21ВернутьДокументыРаспределенияРасходовПоБазеКОтражениюВРеглУчете(РасчетныеПрочиеРасходы);
	//
...
////ASP_Dante 12/07/21 Задача 000011492 перенос доработок	ВернутьДокументыСДвижениямиПоСебестоимостиКОтражениюВРеглУчете(ДанныеДляСебестоимостиПроизводства);
//	// Завершение этапа
...
//	Комментарий = СтрЗаменить(Комментарий, "%Замер%", Формат(Замер/1000., "ЧДЦ=3; ЧН=; ЧГ="));
//	ЗаписьЖурналаРегистрации(
//		НСтр("ru = 'Партионный учет.СебестоимостьТоваров'",	ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
//		УровеньЖурналаРегистрации.Информация,,, Комментарий);
КонецПроцедуры
