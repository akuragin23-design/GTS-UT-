...
Функция ASP_ПодключитьПроизвольноеНапоминание(Пользователь, Текст, ВремяСобытия, ИнтервалДоСобытия = 0, Предмет = Неопределено, Идентификатор = Неопределено) Экспорт
	//ASP_Crow 11.03.2026 000029567 В расширени
	//++ASP_Crow 01.10.2024 000024647 Добавляем процедуру с тем, чтобы пользователя вставить
	ПараметрыНапоминания = Новый Структура;
	ПараметрыНапоминания.Вставить("Описание", Текст);
	Если ТипЗнч(ВремяСобытия) = Тип("РасписаниеРегламентногоЗадания") Тогда
		ПараметрыНапоминания.Вставить("Расписание", ВремяСобытия);
