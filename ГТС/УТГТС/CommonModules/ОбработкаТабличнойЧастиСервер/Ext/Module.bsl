...
Процедура ЗаполнитьЦенуПродажиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ЗначениеЗаполнено(Цена) Тогда 
		...
		//Альтер ЛОГО Округляем цену до рублей если вид цены - "Продажа в CNY"
		Если ТекущаяСтрока.ВидЦены = Справочники.ВидыЦен.НайтиПоНаименованию("Продажа в CNY") Тогда
			ТекущаяСтрока.Цена = Окр(ТекущаяСтрока.Цена, 0);
		КонецЕсли;	
		//Альтер ЛОГО
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСерверЛокализация.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВСтрокеТЧ(СтруктураДействий, СтруктураДействийЗаполнения);
...
Процедура ПроверитьЗаполнитьОбеспечениеВСтрокеТЧДокументаПродажи(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВДокументеПродажи", ПараметрыДействия) И не СтруктураДействий.Свойство("ASP_ОбменЗаказами") Тогда
		...
		ОбеспечениеВДокументахСервер.ПроверитьЗаполнитьОбеспечениеВСтрокеТЧДокументаПродажи(ТекущаяСтрока,
		                                                                                    ...
		                                                                                    ПараметрыДействия);
	//ASP_Crow 11.03.2026 000029567 В расширени
	ИначеЕсли СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВДокументеПродажи", ПараметрыДействия) и СтруктураДействий.Свойство("ASP_ОбменЗаказами") Тогда
		...
		КонецЕсли;
		Если Реквизиты.ОбособленнаяЗакупкаПродажа Тогда
		...
		Если ПараметрыДействия.ЖелаемаяДатаОтгрузки <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ТекущаяСтрока.ДатаОтгрузки)
					Или ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.КОбеспечению Тогда
...
Функция МетодыКВыполнению(СтруктураДействий)
	...
	ПакетнаяОбработкаТабличнойЧастиСерверЛокализация.СоответствиеКомандМетодам(Соответствие);
