...
Процедура ПересчитатьЦенуСкидкуПоСуммеВПродажахВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	#КонецЕсли
		//ASP_Dante 29/08/19
		//ASP+++
		Если СтруктураДействий.Свойство("ASP_ОсобыйСлучай") И СтруктураДействий.ASP_ОсобыйСлучай Тогда
		...
		Иначе
			//ASP_Dante 29/08/19 типовой функ
		// Альтер ЛОГО+: закоментировано с целью сохранить пересчет цены от суммы не зависимо от флага ручной скидки
		...
		//	КонецЕсли;
		//Иначе
				// Альтер ЛОГО+:	Если процент ручной скидки слишком маленький, то обнуляем скидку
