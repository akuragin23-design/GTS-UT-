...
Процедура ПолучитьДополненныйЗапросДинамическогоСписка(ТекстЗапроса) Экспорт
	//++ Локализация
	ТекстЗапроса =
	//ASP_Dante 09/07/21 Задача 000011492 Перенос доработок //ASP_Crow 11.03.2026 000029567 В расширени
	"ВЫБРАТЬ
	...
	|	КОНЕЦ КАК Цена,
	|	СпрНоменклатура.ASP_АрхивнаяПозиция КАК ASP_АрхивнаяПозиция
	|ИЗ
	...
	|			МИНИМУМ(ЕСТЬNULL(ЗапасыИПотребностиОстатки.ВНаличииОстаток, 0) / ЕСТЬNULL(ВариантыКомплектацииНоменклатурыТовары.Количество, 1)) КАК Остаток,
	|			ВариантыКомплектацииНоменклатурыТовары.Ссылка.Владелец КАК Владелец,
	|			СУММА(ЦеныНоменклатурыСрезПоследних.Цена * ЕСТЬNULL(ВариантыКомплектацииНоменклатурыТовары.Количество, 1)) КАК Цена
	|		ИЗ
	...
	|				ПО ВариантыКомплектацииНоменклатурыТовары.Номенклатура = ЗапасыИПотребностиОстатки.Номенклатура
	|					И ВариантыКомплектацииНоменклатурыТовары.Характеристика = ЗапасыИПотребностиОстатки.Характеристика
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ВариантыКомплектацииНоменклатурыТовары.Ссылка.Владелец) КАК ВложенныйЗапрос
	|		ПО СпрНоменклатура.Ссылка = ВложенныйЗапрос.Владелец
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыИПотребности.Остатки(, Склад В (&Склады)) КАК ЗапасыИПотребностиОстатки
	|		ПО СпрНоменклатура.Ссылка = ЗапасыИПотребностиОстатки.Номенклатура
	|ГДЕ
