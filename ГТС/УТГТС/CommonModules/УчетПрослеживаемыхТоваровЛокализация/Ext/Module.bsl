...
Процедура ПроверитьКорректностьКомплекта(Объект, Отказ, ПараметрыПроверки) Экспорт
	...
	Если ОшибкиПроверки.Следующий() Тогда
		...
		//ГТС 23.11.2023 Если не основной вариант комплектации, то разрешаем сохранить
			Если Объект.Основной=Ложь тогда
				ЕстьОшибкиПроверки=Ложь;
				отказ=ложь;
			КонецЕсли;
			//ГТС 
...
Функция ПрослеживаемыеКомплектующиеДляПечатиДанных(ПрослеживаемыеКомплектующие) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		//++ASP_Crow 06.07.2026 000030907 Учитываем ситуацию .если ед изм не заполнен
		Если ЗначениеЗаполнено(Выборка.ЕдиницаИзмеренияКод) Тогда
		...
		Иначе
			НоваяСтрока.ЕдиницаИзмеренияКод				= СокрЛП("796");
			НоваяСтрока.ЕдиницаИзмеренияНаименование	= СокрЛП("шт");	
		КонецЕсли;
		//--ASP_Crow 06.07.2026 000030907	
	КонецЦикла;
