...
Процедура ОтразитьДвиженияПодчиненныхРегистров(Механизмы, Документ, ТаблицыДляДвижений, Отказ)
	...
	Если СвойстваДокумента(Документ).РежимЗаписи = РежимЗаписиДокумента.Проведение И ЗначениеЗаполнено(ТаблицыДляДвижений) Тогда
		...
		//++ASP_Crow 25.03.2024 000021639 Заставляем делать движение по обеспечению заказов для регламентного задания 
		Если ТаблицыДляДвижений.Свойство("ТаблицаОбеспечениеЗаказов") Тогда
			Документ.Движения.ОбеспечениеЗаказов.Очистить();
			Документ.Движения.ОбеспечениеЗаказов.Записывать = Истина;
			Документ.Движения.ОбеспечениеЗаказов.Загрузить(ТаблицыДляДвижений.ТаблицаОбеспечениеЗаказов);
			Документ.Движения.ОбеспечениеЗаказов.Записать();
		КонецЕсли;
