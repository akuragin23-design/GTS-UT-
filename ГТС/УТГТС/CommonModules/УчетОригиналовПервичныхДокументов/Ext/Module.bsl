...
Процедура УстановитьУсловноеОформлениеВФормеСписка(Форма, Список) Экспорт
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветГиперссылки);
	ЭлементОформления.Использование = Истина;
...
Процедура ОбработатьШтрихкод(Штрихкод) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина); //ASP_Crow 16.03.2026 000029567 В расширение
	СсылкаПоШтрихкоду = СсылкаПоШтрихкодуТабличногоДокумента(Штрихкод);
	Если СсылкаПоШтрихкоду.Количество() = 0 Тогда
	...
	Структура.Вставить("ОбщееСостояние",Истина);
	Структура.Вставить("Ссылка",СсылкаПоШтрихкоду[0]);
	Структура.Вставить("СостояниеОригиналаПервичногоДокумента",Справочники.СостоянияОригиналовПервичныхДокументов.ФормаНапечатана);
	Массив.Добавить(Структура);
	
	ЕстьРольASP_ОригиналВОфисе = Ложь;
	Если Пользователи.ЭтоПолноправныйПользователь(,,Ложь) Тогда
		ГруппаОригинал = Справочники.ГруппыДоступа.НайтиПоНаименованию("Отметка Оригинал в офисе");
		Если ЗначениеЗаполнено(ГруппаОригинал) Тогда
			СтрокаТЧ = ГруппаОригинал.Пользователи.Найти(Пользователи.ТекущийПользователь(), "Пользователь");
			Если СтрокаТЧ <> Неопределено Тогда
				ЕстьРольASP_ОригиналВОфисе = Истина;
			КонецЕсли;	
		КонецЕсли;
		
	Иначе
		ЕстьРольASP_ОригиналВОфисе = РольДоступна("ASP_ОригиналВОфисе");
	КонецЕсли;
	
	Если ЕстьРольASP_ОригиналВОфисе Тогда
		СостояниеВОфисе = Справочники.СостоянияОригиналовПервичныхДокументов.НайтиПоКоду("000000008");
		СообщениеВОфисе = строка(СсылкаПоШтрихкоду[0].Ссылка) + " в офисе!";
		Если ТипЗнч(СсылкаПоШтрихкоду[0]) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			СообщениеВОфисе = СообщениеВОфисе + Символы.ПС + "Ответственный: " + СсылкаПоШтрихкоду[0].Менеджер.Наименование  + Символы.ПС + "Подразделение: " + СсылкаПоШтрихкоду[0].Менеджер.Подразделение;
		ИначеЕсли ТипЗнч(СсылкаПоШтрихкоду[0]) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
			//Ищем наличие скан-образов УПД, добавлено 21.12.2022	
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СчетФактураВыданныйПрисоединенныеФайлы.Ссылка) КАК Ссылка
			|ИЗ
			|	Справочник.СчетФактураВыданныйПрисоединенныеФайлы КАК СчетФактураВыданныйПрисоединенныеФайлы
			|ГДЕ
			|	СчетФактураВыданныйПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла";
			Запрос.УстановитьПараметр("ВладелецФайла", СсылкаПоШтрихкоду[0]);
			//РезультатЗапроса = Запрос.Выполнить();
			//ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
			Если Запрос.Выполнить().Пустой() Тогда
				СообщениеВОфисе = СообщениеВОфисе + Символы.ПС + "СКАН-ОБРАЗ НЕ НАЙДЕН!!!"; 
			КонецЕсли;
			СообщениеВОфисе = СообщениеВОфисе + Символы.ПС + "Ответственный: " + СсылкаПоШтрихкоду[0].Ответственный.Наименование + Символы.ПС + "Подразделение: "  + СсылкаПоШтрихкоду[0].Ответственный.Подразделение;
		ИначеЕсли ТипЗнч(СсылкаПоШтрихкоду[0]) = Тип("ДокументСсылка.ТранспортнаяНакладная") Тогда
			Если ЗначениеЗаполнено(СсылкаПоШтрихкоду[0].Отпустил) Тогда
				ОтпустилПользователь = Справочники.Пользователи.НайтиПоРеквизиту("ФизическоеЛицо", СсылкаПоШтрихкоду[0].Отпустил); 	
			КонецЕсли;
			СообщениеВОфисе = СообщениеВОфисе + Символы.ПС + "Ответственный: " + СсылкаПоШтрихкоду[0].Отпустил.Наименование  + Символы.ПС + "Подразделение: " + ОтпустилПользователь.Подразделение;
		КонецЕсли;
		УчетОригиналовПервичныхДокументовВызовСервера.УстановитьНовоеСостояниеОригинала(Массив,СостояниеВОфисе);
		сообщить(СообщениеВОфисе);
	Иначе
		УчетОригиналовПервичныхДокументовВызовСервера.УстановитьНовоеСостояниеОригинала(Массив,Справочники.СостоянияОригиналовПервичныхДокументов.НайтиПоНаименованию("Помещен в архив"));
		Если ТипЗнч(СсылкаПоШтрихкоду[0]) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
			//Ищем наличие скан-образов УПД, добавлено 21.12.2022	
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			...
			//ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
			Если Запрос.Выполнить().Пустой() Тогда
				Сообщить("СКАН-ОБРАЗ НЕ НАЙДЕН!!! Документ будет помещен в архив."); 
			КонецЕсли;
		КонецЕсли;
		сообщить(строка(СсылкаПоШтрихкоду[0].Ссылка) + " помещен в архив!");	
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
	//ГТС доработано
	//ASP доработано 000016074   
	//--//ASP_Crow 16.03.2026 000029567 В расширение
