...
Функция ЦенаПродажиИОстаткиТовара(Номенклатура, Характеристика, Соглашение, Валюта, Склады, ВидыЦен) Экспорт
	...
	// Альтер ЛОГО+ для пустого соглашения сделаем принудительное отражение цены продажи //ASP_Crow 11.03.2026 000029567 В расширени
	Иначе
		
		Если Соглашение.Пустая() Тогда
			ВидЦеныПродажи = Справочники.ВидыЦен.ПолучитьСсылку(Новый УникальныйИдентификатор("c801ed00-1722-11e0-b697-5cff35022aee"));
			Цена = ЦенаПродажиНоменклатуры(Номенклатура, ВидЦеныПродажи);
			СтруктураЦены = Новый Структура("ВидЦены, Цена, Упаковка, ЕдиницаИзмерения, Описание", ВидЦеныПродажи, Цена, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), Номенклатура.ЕдиницаИзмерения, 2);
		Иначе
			СтруктураЦены = Новый Структура("ВидЦены, Цена, Упаковка, ЕдиницаИзмерения, Описание, СрокПоставки", 
				Справочники.ВидыЦен.ПустаяСсылка(), 0, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), Справочники.ЕдиницыИзмерения.ПустаяСсылка(), 0, 0);
		КонецЕсли;		
	КонецЕсли;
...
// Альтер ЛОГО+ Функция добывает цену номенклатуры для случая без соглашения
Функция ЦенаПродажиНоменклатуры(Номенклатура, ВидЦены) Экспорт  
	//ASP_Crow 11.03.2026 000029567 В расширени
