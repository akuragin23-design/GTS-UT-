// Регистрация Правила регистрации для конфигурации ERP 2.5.25 от 07.11.2025 11:02:47
...
Функция ПараметрыРегистрации() Экспорт
	...
	Наименование = НСтр("ru = 'Правила регистрации для конфигурации ERP 2.5.25'");
	Результат.Вставить("Наименование", Наименование);
	Результат.Вставить("ДатаВремяСоздания", Дата(2025,11,7,11,2,0));
	...
	Результат.Вставить("ВерсияКонфигурации", "2.5.25.32");
...
Процедура ПРО_Документ_СчетФактураВыданныйАванс_ПередОбработкой(ПРО, Отказ, Объект, ОбъектМетаданных, Выгрузка)
	...
	Если НЕ ЗначениеЗаполнено(Основание) 
		ИЛИ ИсключаемыеТипыДокументовОснований.Найти(ТипЗнч(Основание)) <> Неопределено Тогда
		Отказ = Истина;
	КонецЕсли;
