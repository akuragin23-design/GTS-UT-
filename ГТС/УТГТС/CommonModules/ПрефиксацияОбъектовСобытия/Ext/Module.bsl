...
Процедура УстановитьПрефикс(Источник, Префикс, УстановитьПрефиксИнформационнойБазы, УстановитьПрефиксОрганизации)
	...
	МассивТипов.Добавить(Тип("ДокументОбъект.РеализацияТоваровУслуг"));
	МассивТипов.Добавить(Тип("ДокументОбъект.ЗаказКлиента"));
	МассивТипов.Добавить(Тип("ДокументОбъект.ПриходныйКассовыйОрдер"));
	МассивТипов.Добавить(Тип("ДокументОбъект.ПоступлениеБезналичныхДенежныхСредств"));
	МассивТипов.Добавить(Тип("ДокументОбъект.СчетФактураВыданный"));	
	
	Если Не МассивТипов.Найти(ТипЗнч(Источник)) = Неопределено  Тогда
		Если Год(Источник.Дата) > 2015 Тогда
			Префикс = "ГТ-";
		КонецЕсли;
	КонецЕсли;
	//ASP КАВ конец 
	
	//++ASP_Crow 01.06.2026 000030574 Изменяем шаблон, если это СФ на аванс
	МассивТиповСФ = новый Массив; 
	МассивТиповСФ.Добавить(Тип("ДокументСсылка.СчетФактураВыданныйАванс"));
	МассивТиповСФ.Добавить(Тип("ДокументСсылка.СчетФактураПолученныйАванс"));
	МассивТиповСФ.Добавить(Тип("ДокументОбъект.СчетФактураВыданныйАванс"));
	МассивТиповСФ.Добавить(Тип("ДокументОбъект.СчетФактураПолученныйАванс"));
	
	Если МассивТиповСФ.Найти(ТипЗнч(Источник)) <> Неопределено Тогда
		Если Год(Источник.Дата) >= 2026 Тогда
			Префикс = "А";
		КонецЕсли;	
	КонецЕсли;
	//--ASP_Crow 01.06.2026 000030574
...
Функция РеквизитОрганизацияДоступен(Объект)
	...
	МетаданныеОбъекта = Объект.Метаданные();
	//++Непп //ASP_Crow 11.03.2026 000029567 В расширени
