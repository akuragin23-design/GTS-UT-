...
Функция СчетаФактурыВыданныеНаОсновании(ПараметрыРегистрации) Экспорт
	...
	Если СчетФактураТребуется Тогда
		...
		Если ТаблицаСчетаФактуры.Количество() > 0 Тогда
			...
			КонецЕсли;
		//++ASP_Crow 22.02.2024 000021639 п.104 ошибок для физ лица с/ф нужна
...
Функция РассчитатьНДСЗаписиКнигиПокупокПоАвансуПолученному(ПараметрыРасчета)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ РАЗЛИЧНЫЕ   // Двухходовки
	|	НДСАвансыПолученныеЗачеты.Организация          КАК Организация,
	|	НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса КАК ДокументЗачетаАванса,
	|	НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса КАК СчетФактура
	|ИЗ
	|	ВТ_НДСАвансыПолученныеЗачеты КАК НДСАвансыПолученныеЗачеты
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.СчетФактураВыданный КАК СчетФактураВыданный
