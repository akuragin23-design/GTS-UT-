...
Функция СформироватьЗаголовокДокумента(Шапка, знач НазваниеДокумента = "", УдалитьТолькоЛидирующиеНулиИзНомераОбъекта = Ложь) Экспорт
	...
	Иначе 
		...
		КонецЕсли;
	КонецЕсли;
	//ASP КАВ начало //ASP_Crow 11.03.2026 000029567 В расширени
	Если НазваниеДокумента = "Счет на оплату" или НазваниеДокумента = "Поставка по заказу" Тогда  //000017447   Поставка по заказу
		НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(НомерНаПечать, Истина, Истина);
	КонецЕсли;
	//ASP КАВ конец //ASP_Crow 11.03.2026 000029567 В расширени
