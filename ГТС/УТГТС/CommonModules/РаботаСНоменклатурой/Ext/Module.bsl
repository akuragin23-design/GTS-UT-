...
Процедура ВывестиГиперссылкуХарактеристик(ТабличныйДокумент, Макет)
	...
	ОбластьСвойстваСтрока.Области.СвойстваСтрока.ЦветТекста  = ЦветаСтиля.ЦветГиперссылки;
...
Процедура УстановитьПараметрыДинамическогоСписка(ПараметрыЗапроса)
	...
	//ГТС добавляем отбор по складам 17/10/2020 //ASP_Crow 11.03.2026 000029567 В расширени
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	ПараметрыЗапроса.УстановитьЗначениеПараметра("Склады", Склады);
	ПараметрыЗапроса.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
	ПараметрыЗапроса.УстановитьЗначениеПараметра("ВидЦены", Справочники.ВидыЦен.НайтиПоНаименованию("Цена продажи"));
