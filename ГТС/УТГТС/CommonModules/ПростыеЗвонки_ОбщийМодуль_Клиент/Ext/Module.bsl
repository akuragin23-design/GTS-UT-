...
Процедура ПриЗавершенииЗвонка(callID, src, dst, duration, start, end, direction, record, line)
	...
	Если ПоказыватьОкноВходящегоВызова = "По завершению разговора" Тогда
		...
		Если direction = 0 Тогда
			ПоказатьОкноВходящегоВызова(src,dst,,,,line);
		КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьОкноВходящегоВызова(НомерС,НомерНа,ПараметрыНаправление=Неопределено,ДатаВремяНачала=Неопределено,ДатаВремяОкончания=Неопределено,line)
	                                                                                                                                                	
	
	//СтруктураКлиентов = ПростыеЗвонки_ОбщийМодуль_Сервер.ПолучитьКлиентаПоНомеруТелефона(НомерС);
	
	
	ПараметрФормыВходящегоЗвонка = Новый Структура("НомерТелефона,НомерНа,ПараметрыНаправление,ДатаВремяНачала,ДатаВремяОкончания,line", НомерС,НомерНа,ПараметрыНаправление,ДатаВремяНачала,ДатаВремяОкончания,line);
	//ASP__A.Zhel 04.06.2019 добавил уникальность - истина
	ПолучитьФорму("ОбщаяФорма.ПростыеЗвонки_ВходящийЗвонок", ПараметрФормыВходящегоЗвонка,,Истина).Открыть();
	//__ASP
	//
...
Процедура Позвонить(НомерТелефона) Экспорт
	...
	Исключение
		Если НЕ ЗначениеЗаполнено(НомерПользователя) Тогда
			Сообщить("Не заполнен номер пользователя");
		Иначе
			Сообщить("Ошибка компоненты");
		КонецЕсли;
	КонецПопытки;
