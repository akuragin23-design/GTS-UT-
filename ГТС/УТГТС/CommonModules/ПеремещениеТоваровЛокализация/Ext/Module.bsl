...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт 
	...
	//ASP_Dante 05/11/19
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И Объект.Дата > ТекущаяДата() Тогда
		Объект.Дата = НачалоДня(Объект.Дата);
	КонецЕсли;         
	//ASP_Dante 05/11/19	
	//ASP_Dante 03/12/19
	Если Не Отказ И РежимЗаписи = РежимЗаписиДокумента.Проведение И Объект.Статус = Перечисления.СтатусыПеремещенийТоваров.Принято Тогда
		ЕстьНазначение = Ложь; ЕстьЗаказНаПеремешениеВНазначении = ложь;
		Для каждого Строка Из Объект.Товары Цикл
			Если ЗначениеЗаполнено(Строка.Назначение) Тогда
				//ASP_Dante 22/09/20
				Заказ = Строка.Назначение.Заказ;
				Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаПеремещение") Тогда
					//asp sig 15052021 №11070 теперь заказы на перемещ обрабатываем
					//ВызватьИсключение "В назначении есть перемещение строка " + Строка.НомерСтроки + ", проверьте документ.
					//					|Провести не удалось!";
					//КонецЕсли;
					////ASP_Dante 22/09/20
					//ASP_Dante 28/05/21 Задача 000011227 ЕстьНазначение = истина; 
					ЕстьЗаказНаПеремешениеВНазначении = истина;
					//Строка.Назначение = Справочники.Назначения.ПустаяСсылка();
					//Объект.ДополнительныеСвойства.Вставить("Товары",Объект.Товары.ВыгрузитьКолонки( "Номенклатура, Назначение, КодСтроки, Количество"));
				ИначеЕсли ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
					
					Если Заказ.Склад = Объект.СкладПолучатель Тогда
						Если Не ЕстьНазначение Тогда
							Объект.ДополнительныеСвойства.Вставить("Товары", Объект.Товары.Выгрузить(, "Номенклатура, Назначение, КодСтроки, Количество, ЗаказНаПеремещение"));
							ЕстьНазначение = Истина;
						КонецЕсли;
						Строка.ASP_ЗаказКлиента = Заказ;
						Строка.Назначение = Справочники.Назначения.ПустаяСсылка();
						//ASP_Hlopkov 17.11.2020+
					ИначеЕсли Заказ.Склад <> Объект.СкладПолучатель И Заказ.Склад.ЭтоГруппа Тогда
						Запрос = Новый Запрос;
						Запрос.Текст = 
						"ВЫБРАТЬ
						...
						Запрос.УстановитьПараметр("Родитель", Заказ.Склад);
						...
						Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
							Если ВыборкаДетальныеЗаписи.Ссылка = Объект.СкладПолучатель Тогда
								Если Не ЕстьНазначение Тогда
									Объект.ДополнительныеСвойства.Вставить("Товары", Объект.Товары.Выгрузить(, "Номенклатура, Назначение, КодСтроки, Количество, ЗаказНаПеремещение"));
									ЕстьНазначение = Истина;
								КонецЕсли;
						...
						КонецЦикла;
						//ASP_Hlopkov 17.11.2020-
						//+++ASP_yulia_l_14/01/2021_9412
					ИначеЕсли Заказ.Склад <> Объект.СкладПолучатель И НЕ Заказ.Склад.ЭтоГруппа Тогда   
						Если Не ЕстьНазначение Тогда
							Объект.ДополнительныеСвойства.Вставить("Товары", Объект.Товары.Выгрузить(, "Номенклатура, Назначение, КодСтроки, Количество, ЗаказНаПеремещение"));
							ЕстьНазначение = Истина;
						КонецЕсли;
					...
					КонецЕсли;
				КонецЕсли;
				//ASP_Hlopkov 13.08.2020+
				Если ЗначениеЗаполнено(Строка.НазначениеОтправителя) И Строка.НазначениеОтправителя.Заказ.Склад = Объект.СкладПолучатель Тогда
					Строка.НазначениеОтправителя = Справочники.Назначения.ПустаяСсылка();
					//ASP_Hlopkov 18.11.2020+	
				ИначеЕсли ЗначениеЗаполнено(Строка.НазначениеОтправителя) И Строка.НазначениеОтправителя.Заказ.Склад.ЭтоГруппа Тогда
					Запрос = Новый Запрос;
					Запрос.Текст = 
					"ВЫБРАТЬ
					|	Склады.Ссылка КАК Ссылка
					...
					Запрос.УстановитьПараметр("Родитель", Строка.НазначениеОтправителя.Заказ.Склад);
		...
		Для каждого Строка Из Объект.ВидыЗапасов Цикл
			Если ЗначениеЗаполнено(Строка.Назначение) И ТипЗнч(Строка.Назначение.Заказ) = Тип("ДокументСсылка.ЗаказКлиента") и Строка.Назначение.Заказ.Склад = Объект.СкладПолучатель Тогда
				Строка.Назначение = Справочники.Назначения.ПустаяСсылка();
				//ASP_Hlopkov 18.11.2020+	
			ИначеЕсли ЗначениеЗаполнено(Строка.Назначение) И ТипЗнч(Строка.Назначение.Заказ) = Тип("ДокументСсылка.ЗаказКлиента") И Строка.Назначение.Заказ.Склад.ЭтоГруппа Тогда
				Запрос = Новый Запрос;
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	Склады.Ссылка КАК Ссылка
				...
				Запрос.УстановитьПараметр("Родитель", Строка.Назначение.Заказ.Склад);
		...
		//ASP_Dante 28/05/21 Задача 000011227 Удаляю строки с назначением по заказу на перемещение
		//(Как понял они не обрабатываются дальше при проведении)
		Если Объект.ДополнительныеСвойства.Свойство("Товары") Тогда
			МассивСтрокУдаления = Новый Массив;
			Для каждого СтрокаДопСвойств Из Объект.ДополнительныеСвойства.Товары Цикл	
				Если ТипЗнч(СтрокаДопСвойств.Назначение.Заказ) = Тип("ДокументСсылка.ЗаказНаПеремещение") ИЛИ НЕ ЗначениеЗаполнено(СтрокаДопСвойств.Назначение) Тогда 
					МассивСтрокУдаления.Добавить(СтрокаДопСвойств);
				КонецЕсли;
			КонецЦикла;
			
			Для каждого СтрокаУдаления Из МассивСтрокУдаления Цикл		
				Объект.ДополнительныеСвойства.Товары.Удалить(СтрокаУдаления);		
			КонецЦикла;
		КонецЕсли;
		//ASP_Dante 28/05/21
		
		//ASP_Hlopkov 11.08.2020+ sig 180820 добавил проверку "ЗначениеЗаполнено(Объект.ЗаказНаПеремещение)"
		Если ЕстьНазначение И НЕ Отказ и ЗначениеЗаполнено(Объект.ЗаказНаПеремещение) и не ЕстьЗаказНаПеремешениеВНазначении Тогда
			//ASP_Hlopkov 14.08.2020+
			ЗаказНаПеремещение = Объект.ЗаказНаПеремещение.ПолучитьОбъект();
			
			Для Каждого Строка Из Объект.Товары Цикл
				
				МассивСтрок = ЗаказНаПеремещение.Товары.НайтиСтроки(Новый Структура("Номенклатура, КодСтроки", Строка.Номенклатура, Строка.КодСтроки));
				
				Для Каждого СтрокаТЧ Из МассивСтрок Цикл
					СтрокаТоваров = СтрокаТЧ; //+++ {{ ASP 10.09.2020 000008403 }} было: ЗаказНаПеремещение.Товары[СтрокаТЧ.НомерСтроки];
					
					//ASP_Hlopkov 30.09.2020+ добавил условие на заполненность назначения
					Если СтрокаТоваров.Назначение <> Справочники.Назначения.ПустаяСсылка() Тогда
						//++ASP_Crow 21.02.2024 000021639 Обособленно теперь галка
						Если (СтрокаТоваров.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить и СтрокаТоваров.Обособленно) ИЛИ СтрокаТоваров.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить Тогда 
							//СтрокаТоваров.Обособленно = Истина;
							СтрокаТоваров.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.КОбеспечению
						КонецЕсли;                                                
						//--ASP_Crow 21.02.2024 000021639
					КонецЕсли;	
				КонецЦикла;	
				
			КонецЦикла;	
			
			//ASP_Hlopkov 18.08.2020+
			...
			////ASP_Hlopkov 09.09.2020+
			//БылоНесколькоПеремещений = Ложь;
			//
			//Запрос = Новый Запрос;
			//Запрос.Текст = 
			//	"ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			//	|ГДЕ
			//	|	ЗаказНаПеремещениеТовары.Ссылка = &СсылкаЗаказа
			//	|	И НЕ ПеремещениеТоваровТовары.Ссылка = &СсылкаПеремещения";
			//
			//Запрос.УстановитьПараметр("СсылкаЗаказа", ЗаказНаПеремещение.Ссылка);
			//Запрос.УстановитьПараметр("СсылкаПеремещения", Объект.Ссылка);
			//
			//РезультатЗапроса = Запрос.Выполнить();
			//
			//Если НЕ РезультатЗапроса.Пустой() Тогда
			//	
			//	БылоНесколькоПеремещений = Истина;
			//	
			//	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
			//	
			...
			//	//ТЗДокументов.Колонки.Добавить("ОтметкаСклада");
			//	//ТЗДокументов.Колонки.Добавить("ПринятоВРаботу");
			//	
			//	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			//		
			...
			//		
			//		НоваяСтрока.ДокументСсылка = ВыборкаДетальныеЗаписи.Ссылка;
			//		//НоваяСтрока.ОтметкаСклада  = ДокументОбъект.ОтметкаСклада;
			//		//НоваяСтрока.ПринятоВРаботу = ДокументОбъект.ПринятоВРаботу;
			//		
			//		//ДокументОбъект.ОтметкаСклада  = '00010101';
			//		//ДокументОбъект.ПринятоВРаботу = '00010101';
			//		
			...
			//	КонецЦикла;
			//	
			//Объект.ДополнительныеСвойства.Вставить("ТЗДокументов", ТЗДокументов);	
			//	
			//КонецЕсли;	
			//ASP_Hlopkov 09.09.2020-
			...
			ЗаказНаПеремещение.ДополнительныеСвойства.Вставить("БезКонтроляОстатков");
			
			Попытка
				ЗаказНаПеремещение.Записать(РежимЗаписиДокумента.Проведение);
			Исключение
				//Отказ = Истина;
				//Сообщить("Не удалось провести заказ поставщику " + Объект.ЗаказНаПеремещение);
			КонецПопытки;
			//РегистрыНакопления.ЗаказыНаПеремещение.Остатки(ТекущаяДата(), Новый Структура("Номенклатура", ЗаказНаПеремещение.Товары[2].Номенклатура))
			ЗаказНаПеремещение.ДополнительныеСвойства.Очистить();
			...
			|		ПО ПеремещениеТоваровТовары.Ссылка.СкладОтправитель = СборкаТоваров.Ссылка.Склад
			|			И ПеремещениеТоваровТовары.Назначение = СборкаТоваров.Назначение
			|			И (СборкаТоваров.Номенклатура = ПеремещениеТоваровТовары.Номенклатура)
			|ГДЕ
			|	ПеремещениеТоваровТовары.Ссылка = &Ссылка
			|	И СборкаТоваров.Проведен
			|	И ПеремещениеТоваровТовары.Назначение <> &Назначение
			|	И СборкаТоваров.Статус = &Статус";
			
			Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
			Запрос.УстановитьПараметр("Назначение", Справочники.Назначения.ПустаяСсылка());
			Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыСборокТоваров.СобраноРазобрано);
			...
			Пока Выборка.Следующий() Цикл
				...
				ОбъектЗаказНаСборку = Выборка.ЗаказНаСборку.ПолучитьОбъект();
				
				ОбъектСборка 		= Выборка.Ссылка.ПолучитьОбъект();
				
				Если ОбъектСборка.Статус = Перечисления.СтатусыСборокТоваров.СобраноРазобрано Тогда
					
					ОбъектЗаказНаСборку.Назначение = Справочники.Назначения.ПустаяСсылка();
					ОбъектСборка.Назначение 	   = Справочники.Назначения.ПустаяСсылка();
					
					Попытка
						ОбъектЗаказНаСборку.Записать(РежимЗаписиДокумента.Проведение);
						Сообщить("Очищено назначение в документе " + ОбъектЗаказНаСборку);
					Исключение
						Отказ = Истина;
						Сообщить("Не удалось провести заказ на сборку " + ОбъектЗаказНаСборку);
					КонецПопытки;
					
					Попытка
						ОбъектСборка.Записать(РежимЗаписиДокумента.Проведение);
						Сообщить("Очищено назначение в документе " + ОбъектСборка);
					Исключение
						Отказ = Истина;
						Сообщить("Не удалось провести сборку " + ОбъектСборка);
					КонецПопытки;
		...
		КонецЕсли;	
		//ASP_Hlopkov 11.08.2020-
		
		
		Если ЕстьНазначение И ЗначениеЗаполнено(Объект.ЗаказНаПеремещение) и не ЕстьЗаказНаПеремешениеВНазначении Тогда
			ЗаказНаПеремещение = Объект.ЗаказНаПеремещение.ПолучитьОбъект();
			Товары = ЗаказНаПеремещение.Товары.Выгрузить();
			ЗаказНаПеремещение.Товары.Очистить();
			
			ПараметрыПоиска = Новый Структура("Номенклатура, КодСтроки, ЗаказНаПеремещение");
			Для каждого Строка Из Товары Цикл
				
				СтрокаЗаказа = ЗаказНаПеремещение.Товары.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаЗаказа, Строка); 	
				
				Если ЗначениеЗаполнено(Строка.Назначение) Тогда
					
					ПараметрыПоиска.Номенклатура = Строка.Номенклатура;
					ПараметрыПоиска.КодСтроки = Строка.КодСтроки; 
					ПараметрыПоиска.ЗаказНаПеремещение = ЗаказНаПеремещение.Ссылка; //ASP_yulia_l_20/05/2021_000011125
					...
					Для каждого СтрокаПоступления Из МассивСтрок Цикл
						...
						Если Не Строка.Количество = СтрокаПоступления.Количество Тогда
							СтрокаНовая = ЗаказНаПеремещение.Товары.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаНовая, СтрокаЗаказа,,"КодСтроки");
							СтрокаЗаказа.Количество = СтрокаПоступления.Количество;
							СтрокаЗаказа.КоличествоУпаковок = СтрокаПоступления.Количество;
							
							СтруктураДействий = Новый Структура;
							
							СтрокаНовая.Количество = СтрокаНовая.Количество - СтрокаЗаказа.Количество;
							СтрокаНовая.КоличествоУпаковок = СтрокаНовая.КоличествоУпаковок - СтрокаЗаказа.КоличествоУпаковок;
							
						КонецЕсли;
						СтрокаЗаказа.ASP_ЗаказКлиента = Строка.Назначение.Заказ;
						СтрокаЗаказа.Назначение = Справочники.Назначения.ПустаяСсылка();
						
						//ASP_Hlopkov 13.08.2020+
						//Если СтрокаЗаказа.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно Тогда 
		...
		ИначеЕсли ЕстьНазначение и не ЕстьЗаказНаПеремешениеВНазначении Тогда  //ASP_yulia_l_10/03/2021_10122
			//построчно с проверкой  свернуть и обойти по той же логике
			ТЗЗаказыНаПеремещение = Объект.Товары.Выгрузить();
			ТЗЗаказыНаПеремещение.Свернуть("ЗаказНаПеремещение");
			Для каждого Стр из ТЗЗаказыНаПеремещение Цикл
				Если ЗначениеЗаполнено(Стр.ЗаказНаПеремещение) Тогда
					ЗаказНаПеремещение = Стр.ЗаказНаПеремещение.ПолучитьОбъект();
					Товары = ЗаказНаПеремещение.Товары.Выгрузить();
					ЗаказНаПеремещение.Товары.Очистить();
					
					ПараметрыПоиска = Новый Структура("Номенклатура, КодСтроки, Назначение, ЗаказНаПеремещение");
					Для каждого Строка Из Товары Цикл
					...
					//Исключение
					//	Отказ = Истина;
					//	Сообщить("Не удалось провести " + ЗаказНаПеремещение);
					//КонецПопытки;
				КонецЕсли;	
			КонецЦикла;
		ИначеЕсли ЕстьЗаказНаПеремешениеВНазначении Тогда //ASP_Dante 28/05/21 Задача 000011227 Убрал ЕстьНазначение и ЕстьЗаказНаПеремешениеВНазначении Тогда //№11070 15/05/21
			
			//ставим "резервировать" в заказе назначения
			ТЗЗаказыНаПеремещение = Объект.Товары.Выгрузить(,"Назначение, Номенклатура"); //ASP_Трунькин 000030610 03/06/2026 добавил номенклатуру
			ТЗЗаказыНаПеремещение.Свернуть("Назначение,Номенклатура");
			Для каждого Стр из ТЗЗаказыНаПеремещение Цикл
				Если ЗначениеЗаполнено(Стр.Назначение) Тогда
					Заказ = Стр.Назначение.Заказ;
					Номенклатура = Стр.Номенклатура;//ASP_Трунькин 000030610 03/06/2026
					Если ЗначениеЗаполнено(Заказ) и ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаПеремещение")  Тогда
						ЗаказНаПеремещение = Заказ.ПолучитьОбъект();
						Для каждого Строка Из ЗаказНаПеремещение.Товары Цикл
							//++ASP_Трунькин в условие добавил номенклатуру
							Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.КОбеспечению и Строка.Обособленно и Строка.Номенклатура = Номенклатура  Тогда//11070
								Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада;
								//ASP_Chin Снятие обособления перед проведением
				...
				КонецЕсли;
			КонецЦикла;
			//очищаем назначения в перемещении
			Для каждого Строка из Объект.Товары Цикл
				Если ЗначениеЗаполнено(Строка.Назначение) Тогда
					//ASP_Dante 22/09/20
					Заказ = Строка.Назначение.Заказ;
			...
			Для каждого Строка из Объект.ВидыЗапасов Цикл
				...
				КонецЕсли;
			КонецЦикла;
			//ASP_Dante 28/05/21 Задача 000011227 Добавил блок была не доработка задачи 11070
			Если ЕстьНазначение Тогда
				//построчно с проверкой  свернуть и обойти по той же логике
				ТЗЗаказыНаПеремещение = Объект.Товары.Выгрузить(Новый Структура("Назначение",Справочники.Назначения.ПустаяСсылка()));
				ТЗЗаказыНаПеремещение.Свернуть("ЗаказНаПеремещение");
				Для каждого Стр из ТЗЗаказыНаПеремещение Цикл
					Если ЗначениеЗаполнено(Стр.ЗаказНаПеремещение) Тогда
						ЗаказНаПеремещение = Стр.ЗаказНаПеремещение.ПолучитьОбъект();
						...
						Для каждого Строка Из Товары Цикл
							...
							Если ЗначениеЗаполнено(Строка.Назначение) Тогда
								...
								ПараметрыПоиска.КодСтроки = Строка.КодСтроки;
								ПараметрыПоиска.Назначение = Строка.Назначение;
								ПараметрыПоиска.ЗаказНаПеремещение = ЗаказНаПеремещение.Ссылка;
		...
		КонецЕсли;
		//Если ЕстьНазначение И НЕ Отказ и ЗначениеЗаполнено(Объект.ЗаказНаПеремещение) Тогда
		//	Если БылоНесколькоПеремещений Тогда
		//	
		//		Для Каждого СтрокаДокумента Из ТЗДокументов Цикл
		//		
		...
		//	КонецЕсли;	
		//КонецЕсли;
		//ASP_Dante 19/07/21 Задача 000011788
		Если Не Объект.ВидыЗапасовУказаныВручную Тогда
			Альтерлого.ИнициироватьПерезаполнениеВидовЗапасов(Объект);
			Объект.ЗаполнитьАналитикиУчетаНоменклатуры();
			Объект.ЗаполнитьВидыЗапасов(Отказ);
		КонецЕсли;
		//ASP_Dante 19/07/21
	КонецЕсли;	
	//ASP_Dante 03/12/19
...
Процедура ASP_ОбработкаПроведения(Объект, Отказ, РежимПроведения) Экспорт
	...
	Если Не Отказ И ДополнительныеСвойства.Свойство("Товары") Тогда
		ТоварыПоступления = ДополнительныеСвойства.Товары;
		...
		Пока ВыборкаЗаказ.Следующий() Цикл
			...
			//ОбъектДокумент.ДополнительныеСвойства.Вставить("БезКонтроля");
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Для каждого Строка Из МассивСтрок Цикл
					...
					Для каждого СтрокаЗаказ Из МассивСтрокЗаказ Цикл
						Склад = ?(ТипЗнч(ОбъектДокумент) = Тип("ДокументОбъект.ЗаказНаСборку"), ОбъектДокумент.Склад, СтрокаЗаказ.Склад);
						//+++добавлено условие проверки перемещение на склад заказа ASP_yulia_l_14/01/2021_9412
						Если Склад = Объект.СкладПолучатель Тогда
							...
							Если КоличествоСписать = СтрокаЗаказ.Количество Тогда
								СтрокаЗаказ.Обособленно = ЛОЖЬ; //++ASP_Crow 21.02.2024 000021639 снимаем обособленность
							...
							ИначеЕсли КоличествоСписать < СтрокаЗаказ.Количество Тогда
								...
								КоличествоСписать = 0;
							КонецЕсли; //--- ASP_yulia_l_14/01/2021_9412
						КонецЕсли;
					КонецЦикла; 					
				КонецЦикла;
			КонецЦикла; 
			...
			//++ГТС 29.08.2025 Добавляем запись в регистр сроков резервирования дл перемещений 
			Если ТипЗнч(ОбъектДокумент) = Тип("ДокументОбъект.ЗаказКлиента") и Объект.СкладПолучатель<>Справочники.Склады.НайтиПоНаименованию("Склад временного хранения г.ТАЛДОМ") Тогда
				СрокРезерваЗаказ = РегистрыСведений.СрокиРезервирования.СоздатьМенеджерЗаписи();
				СрокРезерваЗаказ.ЗаказКлиента = ВыборкаЗаказ.Заказ;
				СрокРезерваЗаказ.Прочитать();    
				ДатаОкончанияРезерва=КалендарныеГрафики.ДатаПоКалендарю(КалендарныеГрафики.ОсновнойПроизводственныйКалендарь(), ТекущаяДата(), 3);
				Если СрокРезерваЗаказ.Выбран() Тогда
					СрокРезерваЗаказ.ОкончаниеРезервированияПеремещение = ДатаОкончанияРезерва;
					Если СрокРезерваЗаказ.ОкончаниеРезервирования<ДатаОкончанияРезерва Тогда
						СрокРезерваЗаказ.ОкончаниеРезервирования=ДатаОкончанияРезерва;
					КонецЕсли;
					СрокРезерваЗаказ.Записать(); 
				Иначе
					СрокРезерваЗаказ.ЗаказКлиента = ВыборкаЗаказ.Заказ;
					СрокРезерваЗаказ.ОкончаниеРезервированияПеремещение = ДатаОкончанияРезерва;
					СрокРезерваЗаказ.ОкончаниеРезервирования = ДатаОкончанияРезерва;
					СрокРезерваЗаказ.Продлено=1;
					СрокРезерваЗаказ.НачальнаяДатаРезерва=ТекущаяДата();
					СрокРезерваЗаказ.Записать();
				КонецЕсли;
			КонецЕсли;
			//--ГТС 29.08.2025
			...
			//++ASP_Crow 16.10.2024 000024647 Добавление заказа для отправки уведомления
			
			Если МассивЗаказовДляУведомлений.Найти(ОбъектДокумент) = Неопределено Тогда
	...
	//ASP_Hlopkov 09.09.2020+
	//Если Не Отказ И ДополнительныеСвойства.Свойство("ТЗДокументов") Тогда
	//	
	//	Для Каждого СтрокаДокумента Из ДополнительныеСвойства.ТЗДокументов Цикл
	//		
	//		ДокументОбъект = СтрокаДокумента.ДокументСсылка.ПолучитьОбъект();	
	//		
	//		//ДокументОбъект.ОтметкаСклада  = СтрокаДокумента.ОтметкаСклада;
	//		//ДокументОбъект.ПринятоВРаботу = СтрокаДокумента.ПринятоВРаботу;
	//		
	//		ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение);
	//		
	//	КонецЦикла;	
