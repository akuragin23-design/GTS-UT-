...
Процедура ПроверитьКорректностьЗаполненияКонтрагента(Объект, Контрагент, Отказ) Экспорт
	...
	КонецЕсли;
	Возврат; //ASP - в базе много задвоений, нет смысла делать проверки  //ASP_Crow 11.03.2026 000029567 В расширени
...
// Альтер ЛОГО+:	Определяет, есть ли в базе контрагент с таким же набором ИНН/КПП
Функция ПолучитьСписокДублейПоИННКПП(ИНН,КПП,ИсключаяСсылку = Неопределено) Экспорт
...
КонецФункции//ПолучитьСписокДублейПоИННКПП
