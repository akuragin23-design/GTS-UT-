...
Процедура ПриОкончанииРедактированияНабора(Форма, ИмяТЧ = "Товары", Параметры) Экспорт
	...
	Для Каждого СтрокаТЧ Из Данные.Комплектующие Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			//++ASP_Crow 01.04.2024 000021639 Разница в ставке НДС (перечисление и справочник) заменяем
			Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.СтавкаНДС) Тогда
				ТекущаяСтрока.СтавкаНДС = СтрокаТЧ.СтавкаНдс.ПеречислениеСтавкаНДС;
			КонецЕсли;
			//--ASP_Crow 01.04.2024 000021639
			//--//ASP_Crow 11.03.2026 000029567 В расширени
...
Функция АдресНабораВоВременномХранилище(Форма, Параметры, ИмяТЧ = "Товары") Экспорт
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		//++ASP_Crow 29.03.2024 000021639 Ставку ндс заполняем если она перечисление
		Если ТипЗнч(НайденнаяСтрока.СтавкаНдс) = ТИП("ПеречислениеСсылка.СтавкиНДС") Тогда
			НоваяСтрока.СтавкаНдс = Справочники.СтавкиНДС.НайтиПоРеквизиту("ПеречислениеСтавкаНДС", НайденнаяСтрока.СтавкаНДС);	
		КонецЕсли;                                                                                                             
		//--ASP_Crow 29.03.2024 000021639
