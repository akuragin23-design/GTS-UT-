...
Процедура ЗарегистрироватьИзменения(Объект, Замещение = Ложь)
	...
	ИначеЕсли ТипОбъекта = Тип("РегистрНакопленияНаборЗаписей.ТоварыКОтгрузке")
		...
		Если Замещение Тогда
			...
			//ASP Mike блокируем номенклатуру, чтобы не было взаимоблокировок
				БлокировкаДанных = Новый БлокировкаДанных;
				ЭлементБлокировки = БлокировкаДанных.Добавить("Справочник.Номенклатура");
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				ЭлементБлокировки.ИсточникДанных = СтарыйНаборЗаписей.Выгрузить(, "Номенклатура");
				ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Ссылка", "Номенклатура");
		...
		//ASP Mike блокируем номенклатуру, чтобы не было взаимоблокировок
		    лИсточник = Новый ТаблицаЗначений;
			лИсточник.Колонки.Добавить("Номенклатура");
			
			Для каждого Запись Из Объект Цикл
				Если ЗначениеЗаполнено(Запись.Номенклатура) тогда
					ЗаполнитьЗначенияСвойств(лИсточник.Добавить(), Запись)
				КонецЕсли;
			КонецЦикла;
			...
			ЭлементБлокировки = БлокировкаДанных.Добавить("Справочник.Номенклатура");
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			ЭлементБлокировки.ИсточникДанных = лИсточник;
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Ссылка", "Номенклатура");
			БлокировкаДанных.Заблокировать();
		//ASP Mike -
...
Функция ПолучитьАдресXDTO(ПараметрыОбмена, ПредставлениеАдреса) Экспорт
	...
	XDTOАдрес.Представление 	= СокрЛП(ПредставлениеАдреса.Представление);//ASP_Lebedev есть адреса с большим количеством пробелов, из-за этого не проходит по длине
...
Процедура ЗаданиеОбменССайтом(КодНастройки) Экспорт
	...
	Если ПараметрыОбмена.ПометкаУдаления Тогда
		Возврат;
	КонецЕсли;
	//ГТС пробуем включить и выключить полный обмен для работы только ночью, а днем делать частичный.
	Если ПараметрыСеанса.ТекущийПользователь = Справочники.Пользователи.НайтиПоНаименованию("ROBOT") и (ТекущаяДата()>НачалоДня(ТекущаяДата())+20*60*60 или ТекущаяДата()<НачалоДня(ТекущаяДата())+6*60*60) тогда
	ТекОбмен=ПараметрыОбмена.ПолучитьОбъект();
	ТекОбмен.ВыгружатьТолькоИзменения=Ложь;
	ТекОбмен.ДеактивироватьТоварыЕслиПолнаяВыгрузка=Истина;
	ТекОбмен.Записать();
	Иначе
	ТекОбмен=ПараметрыОбмена.ПолучитьОбъект();
	ТекОбмен.ВыгружатьТолькоИзменения=Истина;
	ТекОбмен.ДеактивироватьТоварыЕслиПолнаяВыгрузка=Ложь;
	ТекОбмен.Записать();
	КонецЕсли;
