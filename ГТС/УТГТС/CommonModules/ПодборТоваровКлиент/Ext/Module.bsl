...
Процедура ПолучитьИнформациюОТовареПриПродаже(Форма) Экспорт
	...
	Для Каждого СтрокаТбл Из ИнформацияОТоваре.ТекущиеОстатки Цикл
		...
		// Альтер ЛОГО+ тут и далее по плановым остаткам проверяем, если список складов пуст, то склад доступен для выбора в любом случае 
		//ASP_Crow 11.03.2026 000029567 В расширени
		Если Форма.Склады.Количество() = 0 Тогда
			СтрокаОстаткиПоСкладам.СкладДоступенДляВыбора = Истина;
		Иначе
			СтрокаОстаткиПоСкладам.СкладДоступенДляВыбора = СкладДоступенДляВыбора(Форма, СтрокаОстаткиПоСкладам.Склад);
		КонецЕсли;
		// +Альтер ЛОГО
		...
		Для Каждого СтрокаТбл Из ИнформацияОТоваре.ПланируемыеОстатки Цикл
			...
			//ASP_Crow 11.03.2026 000029567 В расширени
			Если Форма.Склады.Количество() = 0 Тогда
				СтрокаПланируемыеОстаткиПоДатам.СкладДоступенДляВыбора = Истина;
			Иначе
				СтрокаПланируемыеОстаткиПоДатам.СкладДоступенДляВыбора = СкладДоступенДляВыбора(Форма, СтрокаПланируемыеОстаткиПоДатам.Склад);
			КонецЕсли;
			// +Альтер ЛОГО
...
// Альтер ЛОГО+ Функция проверяет, если по данному товару текущие или планируемые остатки
Функция ЕстьОстатки(СтруктураИнформацииОТоваре, СтрокаТЧ) 
	//ASP_Crow 11.03.2026 000029567 В расширени
	ТекОстаток = СтрокаТЧ.Свободно;
	//
	Склад = СтрокаТЧ.Склад;
	ПланируемыеОстатки = СтруктураИнформацииОТоваре.ПланируемыеОстатки;
	ПлановыйОстаток = 0;
	Для Каждого СтрокаПлановыхОстатковПоСкладу Из ПланируемыеОстатки Цикл
		Если СтрокаПлановыхОстатковПоСкладу.Склад <> Склад Тогда
			Продолжить;
		КонецЕсли;
		ПлановыйОстаток = ПлановыйОстаток + СтрокаПлановыхОстатковПоСкладу.Доступно;
	КонецЦикла;
	Возврат (ТекОстаток + ПлановыйОстаток) > 0;
КонецФункции
