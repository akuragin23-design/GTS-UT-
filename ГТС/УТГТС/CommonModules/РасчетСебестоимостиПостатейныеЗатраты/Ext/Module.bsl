...
Функция ДанныеДляРасшифровкиДопРасходовМеждуПартиямиИТоварами(Период, МассивОрганизаций, Регистратор, РасчетСебестоимостиВыполнен) Экспорт
	...
	СостояниеРасчетаСебестоимости = РасчетСебестоимостиПрикладныеАлгоритмы.ТекущееСостояниеРасчета(
		Период, МассивОрганизаций, Справочники.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости);
...
Функция ПодготовитьБазуПоПрямымЗатрат(Параметры) Экспорт
	...
	Если Не РасчетСебестоимостиПрикладныеАлгоритмы.ВременнаяТаблицаСуществует(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтОВЗ") Тогда
		...
		Запрос.Выполнить();
	КонецЕсли;
	Если Не РасчетСебестоимостиПрикладныеАлгоритмы.ВременнаяТаблицаСуществует(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТУчетныеПолитикиФинУчета") Тогда
...
Процедура ПодготовитьПроизводственныеРасходыКРаспределению(ПараметрыРасчета)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(Т.СтатьяРасходов, ДД.СтатьяРасходов) КАК СтатьяРасходов,
		|	ДД.АналитикаРасходов,
		|	Т.ЭтоРасходОВЗ,
		|	ВЫБОР 
		...
		|			И ДД.СтатьяРасходов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РасходыКРаспределению КАК Т
		|		ПО (Т.Организация = ДД.Организация)
		|			И (Т.Подразделение = ДД.Подразделение)
		...
		|	И (ДД.Ссылка = &Регистратор
		|			ИЛИ &Регистратор = НЕОПРЕДЕЛЕНО)
		|	И (Т.СтатьяРасходов ЕСТЬ НЕ NULL
		|		ИЛИ ДД.СтатьяРасходов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.СторноПроизводственныхЗатрат))
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Регистратор
		|;
		...
		|	ДД.ВариантРаспределенияРасходовНУ,
		|	ДД.ВариантРаспределенияРасходовНДД,
		|	ВЫБОР КОГДА ДД.РаспределятьПоПартиям И ДД.ДоляСтоимостиНаПроизводство = 0
		|		ТОГДА 0
		|		ИНАЧЕ ДД.ДоляСтоимостиНаНЗП / ВЫБОР 
		|			КОГДА НЕ ДД.РаспределятьПоПартиям ТОГДА
		|				1
		|			ИНАЧЕ
		|				ДД.ДоляСтоимостиНаПроизводство
...
Функция ТекстДанныеПоФактическимМатериальнымЗатратам(ПараметрыРасчета = Неопределено)
	...
	ТекстЗапроса = 
		...
		|	ДД.ВременнаяРазница,
		|	ДД.СтоимостьНДД
		|ПОМЕСТИТЬ ВТКэшРасчетныеОборотыСебестоимость_Индексы
		|ИЗ
		|	ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК ДД
		|
		...
		|	И (НЕ ДД.Количество = 0 
		|		ИЛИ НЕ (ДД.Стоимость + ДД.ДопРасходы) = 0
		|		ИЛИ НЕ (ДД.СтоимостьБезНДС + ДД.ДопРасходыБезНДС) = 0
		|		ИЛИ НЕ (ДД.СтоимостьРегл + ДД.ДопРасходыРегл) = 0
		|		ИЛИ НЕ (ДД.СтоимостьУпр + ДД.ДопРасходыУпр) = 0
		//++ Локализация
		|		ИЛИ НЕ (ДД.СтоимостьРегл + ДД.ДопРасходыРегл - ДД.ПостояннаяРазница - ДД.ВременнаяРазница) = 0
		//-- Локализация
		|		ИЛИ НЕ ДД.СтоимостьНДД = 0
		|		)
		|	И ТИПЗНАЧЕНИЯ(ДД.КорАналитикаУчетаНоменклатуры) = ТИП(Справочник.КлючиАналитикиУчетаНоменклатуры)
		|	И ДД.КорАналитикаУчетаНоменклатуры <> ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаНоменклатуры.ПустаяСсылка)
		|	И ТИПЗНАЧЕНИЯ(ДД.КорПартия) = ТИП(Справочник.ПартииПроизводства)
		|	И ДД.КорПартия <> ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
		// Исключаем документы по которым введена корректировка или сторнирование в текущем периоде
		...
		//-- Устарело_Переработка24
		|		                 КАК ЭтоПолуфабрикатПереработчика,
		...
		|ИЗ
		|	ВТКэшРасчетныеОборотыСебестоимость_Индексы КАК ДД
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	...
	ТекстЗапроса = ОбъединитьТексты(ТекстыЗапросов)
		...
		|	НаправлениеДеятельности, ГруппаПродукции)
		|)"
		+ ОбщегоНазначения.РазделительПакетаЗапросов();
	...
	|УНИЧТОЖИТЬ ВТКэшРасчетныеОборотыСебестоимость_Индексы
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|";
...
Функция ТекстБазаРаспределенияДолейПоЭтапам(ПараметрыРасчета) Экспорт
	...
	ШаблонТекстаЗапроса = 
		...
		|		ПО ПартииПроизводства.Ссылка = Т.ПартияПроизводства
		|	ЛЕВОЕ СОЕДИНЕНИЕ НастройкиПризнанияРасходовНДД КАК НастройкиПризнанияРасходовНДД
		|		ПО ПартииПроизводства.Организация = НастройкиПризнанияРасходовНДД.Организация
	...
	ТекстРаспределяемыеВручную = 
		...
		|	И Аналитики.РаспределениеЗатратНаВыпуск
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ НастройкиПризнанияРасходовНДД КАК НастройкиПризнанияРасходовНДД
...
Функция ТекстМатериальныеЗатратыНЗП()
	...
	ТекстЗапроса =
		...
		// Исключаем приход услуг внешнего переработчика
		|	И НЕ ИСТИНА В(
		|		ВЫБРАТЬ ПЕРВЫЕ 1
		|			ИСТИНА
		|		ИЗ
		|			ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК УслугиПереработчика
		|		ГДЕ
		|			УслугиПереработчика.АналитикаУчетаНоменклатуры = ДД.АналитикаУчетаНоменклатуры
		|			И УслугиПереработчика.РазделУчета = ДД.РазделУчета
		|			И УслугиПереработчика.ВидЗапасов = ДД.ВидЗапасов
		|			И УслугиПереработчика.Организация = ДД.Организация
		|			И УслугиПереработчика.Партия = ДД.Партия
		|			И УслугиПереработчика.АналитикаУчетаПартий = ДД.АналитикаУчетаПартий
		|			И УслугиПереработчика.АналитикаФинансовогоУчета = ДД.АналитикаФинансовогоУчета
		|			И УслугиПереработчика.ВидДеятельностиНДС = ДД.ВидДеятельностиНДС
		|			И УслугиПереработчика.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщика))
		// Исключаем документы по которым введена корректировка или сторнирование в текущем периоде
		|	И ИсправленныеДокументы.СторнируемыйДокумент ЕСТЬ NULL
		|
		|СГРУППИРОВАТЬ ПО
...
Функция ТекстТрудозатратыНЗП()
	...
	ТекстЗапроса =
		...
		|	
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПартииПроизводстваСводно КАК ПартииПроизводстваСводно
		|	ПО ПартииПроизводстваСводно.ПартияПроизводства = ДД.ПартияПроизводства
		|	И ПартииПроизводстваСводно.Организация = ДД.Организация
		|	
		// Исключаем документы по которым введена корректировка или сторнирование в текущем периоде
		|	ЛЕВОЕ СОЕДИНЕНИЕ ИсправленныеДокументы КАК ИсправленныеДокументы
...
Функция ТекстВыпущеннаяПродукция()
	...
	ТекстЗапроса =
		...
		|		ПО СпрПартииПроизводства.Документ =
		|			ВЫБОР
		...
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТУчетныеПолитикиФинУчета КАК ВТУчетныеПолитикиФинУчета
		|		ПО СпрВидыЗапасов.Организация = ВТУчетныеПолитикиФинУчета.Организация
		|
...
Функция ТекстРасходыПрочихРасходовНЗППоМатериалам() // использует ПартииДляОтнесенияРасходов, МатериальныеЗатратыПроизводства, ОстаткиПриходыРасходы, РаботыДляДавальца
	...
	|	ЕСТЬNULL(РегистраторыРасчета.Ссылка, Материальные.РегистраторРасхода) КАК Регистратор,
	|	ДД.Организация                                      КАК Организация,
	|	ДД.Подразделение                                    КАК Подразделение,
	|	ДД.ПартияПроизводства                               КАК ПартияПроизводства,
	|	ДД.АналитикаПартииВыпуска                           КАК АналитикаПартииВыпуска,
...
Функция ТекстРасходыПрочихРасходовНЗППоТрудозатратам() // использует ПартииДляОтнесенияРасходов, ТрудозатратыПроизводства, ОстаткиПриходыРасходы, РаботыДляДавальца
	...
	|	ЕСТЬNULL(РегистраторыРасчета.Ссылка, Трудозатраты.РегистраторРасхода) КАК Регистратор,
	|	ДД.Организация                                      КАК Организация,
	|	ДД.Подразделение                                    КАК Подразделение,
	|	ДД.ПартияПроизводства                               КАК ПартияПроизводства,
	|	ДД.АналитикаПартииВыпуска                           КАК АналитикаПартииВыпуска,
...
Функция ТекстРасходыПрочихРасходовНЗППоМатериальнымИТрудозатратам() // использует ПартииДляОтнесенияРасходов, МатериальныеИТрудозатратыПроизводства, ОстаткиПриходыРасходы, РаботыДляДавальца
	...
	|	ЕСТЬNULL(РегистраторыРасчета.Ссылка, Затраты.РегистраторРасхода) КАК Регистратор,
	|	ДД.Организация                                      КАК Организация,
	|	ДД.Подразделение                                    КАК Подразделение,
	|	ДД.ПартияПроизводства                               КАК ПартияПроизводства,
	|	ДД.АналитикаПартииВыпуска                           КАК АналитикаПартииВыпуска,
...
Функция ТекстРасходыПрочихРасходовНЗППоПродукции() // использует ПартииДляОтнесенияРасходов, ВыпущеннаяПродукция, РаботыДляДавальца
	Возврат "
			...
			|	ЕСТЬNULL(РегистраторыРасчета.Ссылка, Продукция.РегистраторРасхода) КАК Регистратор,
			|	ДД.Организация                                      КАК Организация,
			|	ДД.Подразделение                                    КАК Подразделение,
			|	ДД.ПартияПроизводства                               КАК ПартияПроизводства,
			|	ДД.АналитикаПартииВыпуска                           КАК АналитикаПартииВыпуска,
			...
			|		ИНАЧЕ Продукция.КорРазделУчета
			|	КОНЕЦ                                               КАК РазделУчета,
			|	ВЫБОР
			|		КОГДА ИСТИНА
