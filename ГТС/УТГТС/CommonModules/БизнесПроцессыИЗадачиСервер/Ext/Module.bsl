...
//Альтер ЛОГО Проверка установки причин закрытия заказа
Функция НеУстановленыПричиныЗакрытия(БизнесПроцесс) Экспорт
	ПричиныНеУстановлены = Ложь;
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЗаказыИОповещения.Заказ
	               |ИЗ
	               |	РегистрСведений.ЗаказыИОповещения КАК ЗаказыИОповещения
	               |ГДЕ
	               |	ЗаказыИОповещения.Задача = &Задача";
	Запрос.УстановитьПараметр("Задача", БизнесПроцесс);
	Результат = Запрос.Выполнить().Выбрать();
	Пока Результат.Следующий() Цикл
		ТЗ = Результат.Заказ.Товары.Выгрузить();
		ТЗ.Свернуть("ПричинаОтмены");
		Если ТЗ.Количество() > 1 ИЛИ ТЗ[0].ПричинаОтмены = Справочники.ПричиныОтменыЗаказовКлиентов.ПросроченГрафикПредоплаты Тогда
			ПричиныНеУстановлены = Истина;	
		КонецЕсли;		
	КонецЦикла;	
	Возврат ПричиныНеУстановлены
КонецФункции
