...
Процедура ОтразитьЗаданияКЗакрытиюМесяца(ИсточникСвойствДокумента, МенеджерВременныхТаблиц) Экспорт
	...
	Если ТипЗнч(ИсточникСвойствДокумента) = Тип("ДокументСсылка.ОтчетПереработчика") или ТипЗнч(ИсточникСвойствДокумента) = Тип("ДокументСсылка.ЗаказПереработчику") или ТипЗнч(ИсточникСвойствДокумента) = Тип("ДокументСсылка.ПередачаСырьяПереработчику") или ТипЗнч(ИсточникСвойствДокумента) = Тип("ДокументСсылка.ПоступлениеОтПереработчика")  Тогда
		Запрос.МенеджерВременныхТаблиц =  МенеджерВременныхТаблиц.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Иначе
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	КонецЕсли;
	//--ASP_Crow 03.03.2026 00002956
