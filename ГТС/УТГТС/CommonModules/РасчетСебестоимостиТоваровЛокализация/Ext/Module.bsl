...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА Передача.Контрагент ЕСТЬ НЕ NULL 
	|				ТОГДА Передача.Контрагент
	|			КОГДА ПередачаТоваровХранителю.Контрагент ЕСТЬ НЕ NULL 
	|				ТОГДА ПередачаТоваровХранителю.Контрагент
	|			КОГДА Возврат.Контрагент ЕСТЬ НЕ NULL 
	|				ТОГДА Возврат.Контрагент
	|			КОГДА ПоступлениеТоваровОтХранителя.Контрагент ЕСТЬ НЕ NULL 
	|				ТОГДА ПоступлениеТоваровОтХранителя.Контрагент
	|			КОГДА ОтчетПереработчика.Контрагент ЕСТЬ НЕ NULL 
	|				ТОГДА ОтчетПереработчика.Контрагент
	|			ИНАЧЕ Погрешность.Контрагент
	|		КОНЕЦ) КАК Контрагент
	|ПОМЕСТИТЬ ПередачиПереработчику
	|ИЗ
	|	ВТДвиженияСебестоимостиДокумента КАК Погрешность
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК Движения
	|		ПО (Движения.Организация = Погрешность.Организация)
	...
	|					ТОГДА ОтчетПереработчика.Контрагент
	|				ИНАЧЕ Погрешность.Контрагент
	|			КОНЕЦ В (НЕОПРЕДЕЛЕНО, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	|
	|СГРУППИРОВАТЬ ПО
	|	Погрешность.Организация,
	|	Погрешность.АналитикаУчетаНоменклатуры,
