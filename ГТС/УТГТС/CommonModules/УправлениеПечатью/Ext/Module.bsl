...
Функция СформироватьОфисныйДокумент(Макет, МассивОбъектов, ОбъектыПечати, КодЯзыка, ПараметрыПечати, ИмяМакета = Неопределено) Экспорт
	...
	//++ASP_Crow 02.04.2024 000022712 Получаем данные несколько иначе    
	
		Если ASP_ЭтоМакетНовоеКПWord(ИмяМакета) Тогда
	    	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		        Если ТипЗнч(ОбъектПечати) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		            ASP_СвернутьТоварыЗаказаДляПечати(ДанныеПечати[ОбъектПечати]);
		        КонецЕсли;
	    	КонецЦикла;
		КонецЕсли;
	
	
	Если ИмяМакета = "ЗаказПоЦиклу" Тогда                            
	#Область Для_ЗаказПоЦиклу 		                                     
		Первый = Истина;
		Для Каждого ОбъектПечати Из МассивОбъектов Цикл
			...
			ЗначенияПараметров["[Номер]"] = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ЗначенияПараметров["[Номер]"], Истина, Истина) + ?(ОбъектПечати.КомментарийКЦиклу<>"", ": "+ОбъектПечати.КомментарийКЦиклу,""); 
			...
			Для ИндексОбласти = 0 По СтруктураДокумента.Области.Количество()-1 Цикл
				...
				Если ЗначениеЗаполнено(ТекущаяОбласть.Коллекция) Тогда
					...
					МассивКоллекции = ПолучитьДанныеОбласти_ЗаказПоЦиклу(ДанныеПечати, ОбъектПечати, ТекущаяОбласть, ДеревоМакетаДляЗаполнения, КодЯзыка, ПараметрыПечати);
					Для Каждого ЭлементКоллекции Из МассивКоллекции Цикл
						
						Если ЭлементКоллекции["[Товары.Упаковка]"]	= "" тогда
							ЭлементКоллекции["[Товары.Упаковка]"] = "шт";
						Иначе
							ЭлементКоллекции["[Товары.Упаковка]"] = Строка(ЭлементКоллекции["[Товары.Упаковка]"]);
						КонецЕсли;
						ТекстКоличества = СтрЗаменить(Формат(ЭлементКоллекции["[Товары.КоличествоУпаковок]"], "ЧРГ="), Символы.НПП, "");
						ЭлементКоллекции["[Товары.КоличествоУпаковок]"] = ТекстКоличества;
		...
		КонецЦикла;
		//--ASP_Crow 02.04.2024 000022712
	#КонецОбласти
	Иначе
	#Область ДляОстальныхДокументов
		Для Каждого ОбъектПечати Из МассивОбъектов Цикл
			//++ASP_Crow 03.09.2024 000024400 добавим получение доп параметров	
			Если ТипЗнч(ОбъектПечати) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				ДопПараметры = ASP_ПолучитьДополнительныеПараметры(ОбъектПечати, ПараметрыДляЗаполненияШапки);
				ЗначенияПараметров = ПолучитьЗначениеПараметровОбъекта(ДанныеПечати, ОбъектПечати, ДеревоМакетаДляЗаполнения, КодЯзыка, ПараметрыПечати, ПараметрыДляЗаполненияШапки);	
				Для каждого ПараметрУдаления из ДопПараметры Цикл
					ЗначенияПараметров.удалить(ПараметрУдаления.Ключ);
				КонецЦикла;
			Иначе
				ЗначенияПараметров = ПолучитьЗначениеПараметровОбъекта(ДанныеПечати, ОбъектПечати, ДеревоМакетаДляЗаполнения, КодЯзыка, ПараметрыПечати, ПараметрыДляЗаполненияШапки);		
			КонецЕсли;
			//--ASP_Crow 03.09.2024 000024400
			ДеревоМакетаДляЗаполнения = КопияДереваМакета(ДеревоМакета);
			//ЗначенияПараметров = ПолучитьЗначениеПараметровОбъекта(ДанныеПечати, ОбъектПечати, ДеревоМакетаДляЗаполнения, КодЯзыка, ПараметрыПечати, ПараметрыДляЗаполненияШапки);
			ДеревоДокумента = ДеревоМакетаДляЗаполнения.СтруктураДокумента.ДеревоДокумента; 
			ДанныеОбъекта = ДанныеПечати[ОбъектПечати];
			...
			Для ИндексОбласти = 0 По СтруктураДокумента.Области.Количество()-1 Цикл
				...
				Иначе
					...
					ЗаполнитьКолонтитулы = Ложь;
					УстановитьПараметры(ДеревоМакетаДляЗаполнения, ЗначенияПараметров, ПараметрыЗаполненияОбласти);
					//++ASP_Crow 03.09.2024 000024400 заполнение документа доп параметрами
					Если ТипЗнч(ОбъектПечати) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
						УстановитьПараметры(ДеревоМакетаДляЗаполнения, ДопПараметры, ПараметрыЗаполненияОбласти);	
					КонецЕсли;     
					//--ASP_Crow 03.09.2024 000024400
					ДеревоМакетаДляЗаполнения.ОбластиДляВывода.Добавить(СтруктураОбластиСтроки.ДеревоОбласти);
				КонецЕсли;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		...
		Если ТипЗнч(Макет) = Тип("ДвоичныеДанные") Тогда
			ПечатнаяФорма.ОфисныеДокументы = СформироватьОфисныйДокумент(Макет, МассивОбъектов, ОбъектыПечати, КодЯзыка, ПараметрыПечати, ПечатнаяФорма.ПолныйПутьКМакету);
		Иначе			
...
Функция ASP_ПолучитьДополнительныеПараметры(Ссылка, ПараметрыДляЗаполненияШапки)
	//ASP_Crow 16.03.2026 000029567 В расширение
	
	КартинкаШтрихкода = ASP_СгенерироватьШтрихкод(Ссылка);
	ДанныеРук = ASP_ПолучитьДанныеРуководителя(Ссылка);
	СоответствиеВозвр = новый Соответствие;
	Для каждого Параметр из ПараметрыДляЗаполненияШапки Цикл
		
		Если Параметр = "[ASP_Штрихкод]" Тогда
			
			СтруктураШК = новый Структура();
			АдресКартинки = ПоместитьВоВременноеХранилище(КартинкаШтрихкода.ДвоичныеДанные);
			СтруктураШК.Вставить("АдресКартинки", АдресКартинки);
			СтруктураШК.Вставить("Ширина", 100);
			СтруктураШК.Вставить("Высота", КартинкаШтрихкода.Картинка.Высота());

			СоответствиеВозвр.Вставить(Параметр, СтруктураШК);	
		ИначеЕсли Параметр = "[ASP_ДоверенностьРуководителя]" тогда
			Если ЗначениеЗаполнено(ДанныеРук) тогда
				СоответствиеВозвр.Вставить(Параметр, ДанныеРук.Доверенность); 
			КонецЕсли;		
		ИначеЕсли Параметр = "[ASP_РуководительМенеджера]" Тогда
			Если ЗначениеЗаполнено(ДанныеРук) тогда
				СоответствиеВозвр.Вставить(Параметр, ДанныеРук.Руководитель);	
			КонецЕсли;	
		КонецЕсли;
		
	КонецЦикла;
	Возврат СоответствиеВозвр
КонецФункции

Функция ASP_СгенерироватьШтрихкод(Ссылка) 
	//ASP_Crow 16.03.2026 000029567 В расширение
	
	ПараметрыШтрихкода = Новый Структура;
	ПараметрыШтрихкода.Вставить("Ширина",           10);
...
Функция ASP_ПолучитьДанныеРуководителя(Ссылка)    
	//ASP_Crow 16.03.2026 000029567 В расширение
	Запрос = новый Запрос;
	Запрос.текст = "ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ первые 1
	               |	ВТ_Руководитель.ТекущийРуководитель КАК ТекущийРуководитель,
	               ...
	               |		ПО ВТ_Руководитель.ТекущийРуководитель = ОтветственныеЛицаОрганизаций.ФизическоеЛицо
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ОтветственныеЛицаОрганизаций.ДатаНачала УБЫВ";
	Запрос.УстановитьПараметр("Подразделение", Ссылка.Менеджер.Подразделение);
	...
	Иначе
		...
		СтруктураВозврата.Вставить("Руководитель", ПервРез.ТекущийРуководитель);
		СтруктураВозврата.Вставить("Доверенность", ПервРез.ОснованиеПраваПодписи);
...
Функция ПолучитьДанныеОбласти_ЗаказПоЦиклу(ДанныеПечати, ОбъектПечати, Область, ДеревоМакета, КодЯзыка, ПараметрыПечати);
	//ASP_Crow 16.03.2026 000029567 В расширение
	...
	ТаблицаОбработанного = новый таблицаЗначений;
	ТаблицаОбработанного.Колонки.Добавить("Номенклатура");
	ТаблицаОбработанного.Колонки.Добавить("Ключ");
	
	СоответствиеСтрок = новый Соответствие;
	Счетчик = 1;
	Для каждого СтрокаТовара из ТаблицаТоваров Цикл 
		
		Если СтрокаТовара.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
			Продолжить
		КонецЕсли;
		
		Если ТаблицаОбработанного.Найти(СтрокаТовара.Номенклатура) = Неопределено тогда
			СоответствиеНоменклатуры = новый Соответствие;
			СоответствиеНоменклатуры.Вставить("[Товары.Номенклатура]", Строка(СтрокаТовара.Номенклатура)+Строка(?(СтрокаТовара.Комментарий<>""," - " + СтрокаТовара.Комментарий,"")));
			СоответствиеНоменклатуры.Вставить("[Товары.Упаковка]", Строка(СтрокаТовара.Номенклатура.ЕдиницаИзмерения));
			СоответствиеНоменклатуры.Вставить("[Товары.КоличествоУпаковок]", СтрокаТовара.КоличествоУпаковок);
			СоответствиеНоменклатуры.Вставить("[Товары.НомерСтроки]", Счетчик);
			СоответствиеСтрок.Вставить(Счетчик, СоответствиеНоменклатуры); 
			
			НовСтрока = ТаблицаОбработанного.Добавить();
			НовСтрока.Номенклатура = СтрокаТовара.Номенклатура;
			новСтрока.Ключ = Счетчик;
			
			Счетчик = Счетчик +1;
		Иначе
			ОбработаннаяСтрока = ТаблицаОбработанного.Найти(СтрокаТовара.Номенклатура);
			
			СтрокаСоответствия = СоответствиеСтрок[ОбработаннаяСтрока.Ключ];
			СтрокаСоответствия["[Товары.КоличествоУпаковок]"] = СтрокаСоответствия["[Товары.КоличествоУпаковок]"] + СтрокаТовара.КоличествоУпаковок;
		КонецЕсли;
	КонецЦикла;
	
	Для НомерИтерации = 1 По СоответствиеСтрок.Количество() Цикл 
		СтрокаДобавления = СоответствиеСтрок[Номеритерации];
		Результат.Добавить(СтрокаДобавления);
	КонецЦикла;
	//СоответствиеСтрок = ДанныеПечати[Ссылка][ИмяТаблицы];	
	//Для НомерСтрокиТабличнойЧасти = 1 По СоответствиеСтрок.Количество() Цикл
	//	ИсточникДанных = Новый Соответствие;
...
Процедура ASP_СвернутьТоварыЗаказаДляПечати(ДанныеЗаказа)
	...
	// Рекомендуемый вариант: считаем одинаковыми строки с одинаковыми
    // номенклатурой, характеристикой, упаковкой, ценой и ставкой НДС.
    ПоляГруппировки = Новый Массив;
    ПоляГруппировки.Добавить("Номенклатура");
    ПоляГруппировки.Добавить("Характеристика");
    ПоляГруппировки.Добавить("Упаковка");
    ПоляГруппировки.Добавить("ЦенаСоСкидкойДляГруппировки");
    ПоляГруппировки.Добавить("СтавкаНДС");

    ПоляСуммирования = Новый Массив;
    ПоляСуммирования.Добавить("Количество");
    ПоляСуммирования.Добавить("КоличествоУпаковок");
    ПоляСуммирования.Добавить("Сумма");
	ПоляСуммирования.Добавить("СуммаСНДС");
    ПоляСуммирования.Добавить("СуммаНДС");
    ПоляСуммирования.Добавить("СуммаРучнойСкидки");
    ПоляСуммирования.Добавить("СуммаАвтоматическойСкидки");

    СвернутыеСтроки = Новый Массив;
	
	ТаблицаПорядка = Новый ТаблицаЗначений;
	ТаблицаПорядка.Колонки.Добавить("Ключ");
	ТаблицаПорядка.Колонки.Добавить(
    "НомерСтроки",
    ...
    СтрокаПорядка.Ключ = ЭлементТоваров.Ключ;

    ИсходнаяСтрока = ЭлементТоваров.Значение;
    ИсходныйНомерСтроки = ИсходнаяСтрока["НомерСтроки"];

    Если ТипЗнч(ИсходныйНомерСтроки) = Тип("Число") Тогда
        СтрокаПорядка.НомерСтроки = ИсходныйНомерСтроки;
    ИначеЕсли ТипЗнч(ЭлементТоваров.Ключ) = Тип("Число") Тогда
        // В стандартной структуре ключ соответствует номеру строки.
        СтрокаПорядка.НомерСтроки = ЭлементТоваров.Ключ;
    Иначе
        СтрокаПорядка.НомерСтроки =
            ТаблицаПорядка.Количество();
    КонецЕсли;

КонецЦикла;

ТаблицаПорядка.Сортировать("НомерСтроки Возр");

Для Каждого СтрокаПорядка Из ТаблицаПорядка Цикл

    ИсходнаяСтрока = Товары[СтрокаПорядка.Ключ];
	...
	ИсходнаяЦена =
			...
			СуммаСНДС =
			    ИсходнаяСтрока["СуммаСНДС"];

			Если ТипЗнч(Количество) = Тип("Число")
			    И Количество <> 0
			    И ТипЗнч(СуммаСНДС) = Тип("Число") Тогда

			    ЦенаСоСкидкой = Окр(
			        СуммаСНДС / Количество,
			        2);

			ИначеЕсли ТипЗнч(ИсходнаяЦена) = Тип("Число") Тогда

			    // Не обнуляем ранее полученную цену,
			    // если для расчёта не загрузились необходимые поля.
			    ЦенаСоСкидкой = ИсходнаяЦена;

			Иначе

			    ВызватьИсключение
			        "Не удалось рассчитать цену с НДС: отсутствуют "
			        + "СуммаСНДС, Количество и исходная Цена.";

			КонецЕсли;

			ИсходнаяСтрока.Вставить(
			    "ЦенаСоСкидкойДляГруппировки",
			    ЦенаСоСкидкой);
    ...
    ИндексИтоговойСтроки = ASP_НайтиОдинаковуюСтрокуТоваров(
        СвернутыеСтроки,
        ИсходнаяСтрока,
        ПоляГруппировки);
    ...
    Если ИндексИтоговойСтроки = -1 Тогда
        ...
        СвернутыеСтроки.Добавить(ИтоговаяСтрока);

    Иначе

        ИтоговаяСтрока =
            СвернутыеСтроки[ИндексИтоговойСтроки];

        Для Каждого ИмяПоля Из ПоляСуммирования Цикл

            ДобавляемоеЗначение = ИсходнаяСтрока[ИмяПоля];

            Если ТипЗнч(ДобавляемоеЗначение) <> Тип("Число") Тогда
                Продолжить;
            КонецЕсли;

            ТекущееЗначение = ИтоговаяСтрока[ИмяПоля];

            Если ТипЗнч(ТекущееЗначение) <> Тип("Число") Тогда
                ТекущееЗначение = 0;
            КонецЕсли;

            ИтоговаяСтрока.Вставить(
                ИмяПоля,
                ТекущееЗначение + ДобавляемоеЗначение);

        КонецЦикла;

        // Цена не суммируется и остаётся равной ключу группировки.
        ИтоговаяСтрока.Вставить(
            "Цена",
            ИтоговаяСтрока[
                "ЦенаСоСкидкойДляГруппировки"]);
...
КонецЦикла;
    // ПолучитьДанныеОбласти обращается к строкам по ключам 1..Количество.
    // Поэтому после свёртки ключи обязательно нужно перенумеровать.
НовыеТовары = Новый Соответствие;

Для Индекс = 0 По СвернутыеСтроки.ВГраница() Цикл

    ИтоговаяСтрока = СвернутыеСтроки[Индекс];
    ...
    НовыеТовары.Вставить(
        Индекс + 1,
        ИтоговаяСтрока);

КонецЦикла;

ДанныеЗаказа.Вставить("Товары", НовыеТовары);

КонецПроцедуры

Функция ASP_НайтиОдинаковуюСтрокуТоваров(
    СвернутыеСтроки,
    ИсходнаяСтрока,
    ПоляГруппировки)
    ...
    Для Индекс = 0 По СвернутыеСтроки.ВГраница() Цикл
        ...
        Для Каждого ИмяПоля Из ПоляГруппировки Цикл
            Если ИтоговаяСтрока[ИмяПоля] <> ИсходнаяСтрока[ИмяПоля] Тогда
...
Функция ASP_ЭтоМакетНовоеКПWord(ИмяМакета)

    СсылкаМакета =
        Справочники.МакетыПечатныхФорм.СсылкаМакета(
            ИмяМакета);

    Если СсылкаМакета = Неопределено
        Или Не ЗначениеЗаполнено(СсылкаМакета) Тогда
    ...
    НаименованиеМакета =
        СокрЛП(СсылкаМакета.Наименование);

    Возврат СтрНачинаетсяС(
        ВРег(НаименованиеМакета),
        ВРег("Новое КП"));
