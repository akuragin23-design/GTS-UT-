...
Процедура ОбработкаПроведения(Объект, Отказ, РежимПроведения) Экспорт 
	//ASP_Crow 16.03.2026 000029567 В расширение
	Движения = Объект.Движения;
	ДополнительныеСвойства = Объект.ДополнительныеСвойства;
	//ASP_Hlopkov 14.08.2020+
	Если Не Отказ И ДополнительныеСвойства.Свойство("Назначение") Тогда
		НазначениеСборки = ДополнительныеСвойства.Назначение;
	
		Если Объект.Статус = Перечисления.СтатусыСборокТоваров.СобраноРазобрано И ЗначениеЗаполнено(Объект.ЗаказНаСборку) И НазначениеСборки.Заказ.Склад = Объект.Склад Тогда
			Для каждого СтароеДвижение из Движения.ЗапасыИПотребности Цикл
			//СтароеДвижение = Движения.ЗапасыИПотребности[0];
			
			СтароеДвижение.Период = НазначениеСборки.Заказ.Дата - 1;
			
			Движения.Записать();
			КонецЦикла;
			ОбъектЗаказ = НазначениеСборки.Заказ.ПолучитьОбъект();
				
			МассивСтрок = ОбъектЗаказ.Товары.НайтиСтроки(Новый Структура("Номенклатура", Объект.Номенклатура));
				
			Для Каждого Строка Из МассивСтрок Цикл
				Если Строка.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.СоСклада Тогда
	...
	//+++ {{ ASP 21.09.2020 000008461 }}
	НазначениеСборки = Неопределено;
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров И Не Отказ И ДополнительныеСвойства.Свойство("Назначение") Тогда
		НазначениеСборки = ДополнительныеСвойства.Назначение;
	КонецЕсли;
	
	ЗаказКлиента = Неопределено;
	Если ЗначениеЗаполнено(НазначениеСборки) Тогда
		ЗаказКлиента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НазначениеСборки, "Заказ");
	КонецЕсли;
	
	СкладПродажи = Неопределено;
	Если ЗначениеЗаполнено(ЗаказКлиента) И Объект.Статус = Перечисления.СтатусыСборокТоваров.СобраноРазобрано И ЗначениеЗаполнено(Объект.ЗаказНаСборку) Тогда
		ЗапросСуществующихЗаказовНаПеремещение = Новый Запрос;
		ЗапросСуществующихЗаказовНаПеремещение.Текст =
			"ВЫБРАТЬ
			|	ЗаказНаПеремещение.Ссылка КАК Ссылка
			|ИЗ
			|	Документ.ЗаказНаПеремещение КАК ЗаказНаПеремещение
			|ГДЕ
			|	ЗаказНаПеремещение.ДокументОснование = &ПроводимыйОбъектСсылка
			|	И НЕ ЗаказНаПеремещение.ПометкаУдаления";
		ЗапросСуществующихЗаказовНаПеремещение.УстановитьПараметр("ПроводимыйОбъектСсылка", Объект.Ссылка);
		
		Если ЗапросСуществующихЗаказовНаПеремещение.Выполнить().Пустой() Тогда
			ЗапросСтрокЗаказа = Новый Запрос;
			ЗапросСтрокЗаказа.Текст =
				"ВЫБРАТЬ
				...
				//++ASP_Crow 08.04.2024 000021639 Обособленного более нет оно теперь несколько иначе работает
				|	И (ЗаказКлиентаТовары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.КОбеспечению) и ЗаказКлиентаТовары.Обособленно)
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				...
				|	И ЗаказКлиентаТовары.Номенклатура = &Номенклатура
				|	И ЗаказКлиентаТовары.Характеристика = &Характеристика
				|	И (ЗаказКлиентаТовары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить) и ЗаказКлиентаТовары.Обособленно)";
				//--ASP_Crow 08.04.2024 000021639
			ЗапросСтрокЗаказа.УстановитьПараметр("ЗаказКлиента", ЗаказКлиента);
			ЗапросСтрокЗаказа.УстановитьПараметр("Номенклатура", Объект.Номенклатура);
			ЗапросСтрокЗаказа.УстановитьПараметр("Характеристика", Объект.Характеристика);
			ВыборкаСкладовИзСтрокЗаказа = ЗапросСтрокЗаказа.Выполнить().Выбрать();
			
			Пока ВыборкаСкладовИзСтрокЗаказа.Следующий() Цикл
				Если ЗначениеЗаполнено(ВыборкаСкладовИзСтрокЗаказа.Склад) Тогда
					СкладПродажи = ВыборкаСкладовИзСтрокЗаказа.Склад;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если Не ЗначениеЗаполнено(СкладПродажи) Тогда
				СкладПродажи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказКлиента, "Склад");
			КонецЕсли;
		КонецЕсли; //Если ЗапросСуществующихЗаказовНаПеремещение.Выполнить().Пустой()
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СкладПродажи) И ЗначениеЗаполнено(Объект.Склад) И Объект.Склад <> СкладПродажи Тогда
		//СтароеДвижение = Движения.СвободныеОстатки[0];
		//
		//СтароеДвижение.Период = НазначениеСборки.Заказ.Дата - 1;
		//
		//Движения.Записать();
		...
		Перемещение.СкладПолучатель = СкладПродажи;
		...
		Перемещение.Подразделение = Объект.Подразделение;
		...
		НовыйТовар.Количество = Объект.Количество;
		НовыйТовар.КоличествоУпаковок = Объект.Количество;
		НовыйТовар.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада;
		НовыйТовар.НачалоОтгрузки = ТекущаяДата() + ((24*60*60)*3);
		НовыйТовар.Номенклатура = Объект.Номенклатура;
		НовыйТовар.Назначение = НазначениеСборки;
		НовыйТовар.Характеристика = Объект.Характеристика;
		
		Перемещение.ДополнительныеСвойства.Вставить("БезКонтроляОстатков");
		Перемещение.Записать(РежимЗаписиДокумента.Проведение);
		
		ОбщегоНазначения.СообщитьПользователю("Создан документ " + Строка(Перемещение.Ссылка), Перемещение.Ссылка);
	КонецЕсли;
	//--- {{ ASP 21.09.2020 000008461 }}	
КонецПроцедуры
...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	 //ASP_Crow 16.03.2026 000029567 В расширение
	//ASP_Hlopkov 14.08.2020+
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И Объект.Статус = Перечисления.СтатусыСборокТоваров.СобраноРазобрано И ЗначениеЗаполнено(Объект.ЗаказНаСборку) И ЗначениеЗаполнено(Объект.Назначение) Тогда
		
		Объект.ДополнительныеСвойства.Вставить("Назначение", Объект.Назначение);
		
		ОбъектЗаказНаСборку = Объект.ЗаказНаСборку.ПолучитьОбъект();
		...
		Если Не Объект.ВидыЗапасовУказаныВручную Тогда
			...
			Иначе
				...
				Объект.ВидыЗапасовСписание.Очистить();
			КонецЕсли;
			//ASP_Dante 06/10/21
	...
	//+ASP_Ямаева  23/12/21
	Если Не Отказ И РежимЗаписи = РежимЗаписиДокумента.Проведение И Объект.Статус = Перечисления.СтатусыСборокТоваров.СобраноРазобрано и Объект.ХозяйственнаяОперация=Перечисления.ХозяйственныеОперации.РазборкаТоваров Тогда
		...
		Для каждого Строка Из Объект.Товары Цикл
			Если ЗначениеЗаполнено(Строка.Назначение) Тогда
				...
				Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
					...
					Если Заказ.Склад = Объект.Склад Тогда
						Если Не ЕстьНазначение Тогда
							Объект.ДополнительныеСвойства.Вставить("Товары", Объект.Товары.Выгрузить(, "Номенклатура, Назначение, КодСтроки, Количество"));
		...
		Пока ВыборкаЗаказ.Следующий() Цикл
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Для каждого Строка Из МассивСтрок Цикл
					...
					МассивСтрокЗаказ = ТоварыЗаказ.НайтиСтроки(Новый Структура("Номенклатура, ВариантОбеспечения", Строка.Номенклатура, Перечисления.ВариантыОбеспечения.Обособленно));
					Для каждого СтрокаЗаказ Из МассивСтрокЗаказ Цикл
						...
						Если СтрокаЗаказ.Склад = Объект.Склад Тогда
