...
Процедура ЗаполнитьНомераГТД(Объект)
	...
	//++ASP_Мочалина 000024094 Для исключения дублирования записей с одинаковым кодом, но различными номенклатурами 
	|	и ВЫБОР
	|		КОГДА Товары.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)  Тогда
	|			УказанныеГТД.Номенклатура = Товары.Номенклатура   иначе истина
	...
	//+++ASP_Мочалина 000024168 Для исключения дублирования записей с одинаковым кодом и номенклатурами  при перепроведении документа
	|			И выбор когда Товары.НомерДляСФ подобно &РегНомерСФ и Товары.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)Тогда
	|				ПодобранныеГТД.РегистрационныйНомер подобно (&РегНомерСФ)
	|			иначе
	|			ПодобранныеГТД.РегистрационныйНомер = (&РегистрационныйНомер) 
	|			конец    
	//---ASP_Мочалина 000024168
	//|			И ПодобранныеГТД.РегистрационныйНомер = &РегистрационныйНомер
	|			И ПодобранныеГТД.ТипНомераГТД = Товары.ТипНомераГТД 
	//+++ASP_Мочалина 000024094 Для иселючения дублирования записей с одинаковым кодом, но различными номенклатурами 
	|	и ВЫБОР
	|		КОГДА Товары.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)Тогда
	|			ПодобранныеГТД.Номенклатура = Товары.Номенклатура   иначе истина

	|	КОНЕЦ 
	//---ASP_Мочалина 000024094 Для иселючения дублирования записей с одинаковым кодом, но различными номенклатурами 
	|			И НЕ ПодобранныеГТД.ПометкаУдаления
	// РНПТ заполняется только вручную: не создаём и не перезаписываем элементы справочника по строкам прослеживаемых товаров.
	|ГДЕ
	|	Товары.ТипНомераГТД <> ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)
	|;
	//++ASP_СмирновН 08.11.24 000025157
	|/////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	НомераГТД.Код ПОДОБНО &КодПоиска
	...
	//+++ASP_Мочалина 000024168 Для исключения дублирования записей с одинаковым кодом и номенклатурами  при перепроведении документа
	Запрос.УстановитьПараметр("РегистрационныйНомер", Объект.НомерДекларации); 
	Запрос.УстановитьПараметр("РегНомерСФ", СокрЛП(Объект.НомерДекларации)+"/%");
	//++ASP_СмирновН 08.11.24 000025157
	Запрос.УстановитьПараметр("КодПоиска", Объект.НомерДекларации+"%"); 
	//--ASP_СмирновН 08.11.24 000025157
	//---ASP_Мочалина 000024168 Для исключения дублирования записей с одинаковым кодом и номенклатурами  при перепроведении документа
	Запрос.УстановитьПараметр("ИспользоватьУчетПрослеживаемыхИмпортныхТоваров",
								ИспользоватьУчетПрослеживаемыхИмпортныхТоваров);
	...
	ВыгрузкаГТД = Пакеты[1].Выгрузить();
	
	Выборка = Пакеты[0].Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		Если Выборка.ТипНомераГТД = Перечисления.ТипыНомеровГТД.НомерРНПТ Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Выборка.НомерГТД) Тогда
			Если Выборка.ТипНомераГТД = Перечисления.ТипыНомеровГТД.НомерГТД Тогда
				Если ВыгрузкаГТД.Количество() > 0 Тогда
					НомерГТД = ВыгрузкаГТД[0].Ссылка.ПолучитьОбъект();	
				Иначе	
		...
		Иначе
			Если выборка.номергтд.получитьобъект() = неопределено Тогда 
				НомерГТД = Справочники.НомераГТД.СоздатьЭлемент();
			Иначе
		...
		ПараметрыДляЗаполнения.Вставить("Владелец", Выборка.Номенклатура);
		ПараметрыДляЗаполнения.ЗаполнитьПорядковыйНомерТовараАвтоматически = Истина;
		
		НомерГТД.Заполнить(ПараметрыДляЗаполнения);
		НомерГТД.Записать();                        
		Если ВыгрузкаГТД.Количество() = 0 и Выборка.ТипНомераГТД = Перечисления.ТипыНомеровГТД.НомерГТД Тогда
			НовСтр = ВыгрузкаГТД.Добавить();
			НовСтр.Ссылка = НомерГтд.Ссылка;
		КонецЕсли;
	КонецЦикла;
	//--ASP_СмирновН 08.11.24 000025157
	...
	|ГДЕ
	|	УказанныеГТД.Ссылка ЕСТЬ NULL
	// РНПТ заполняется только вручную и не синхронизируется между строками одной номенклатуры.
	|	И Товары.ТипНомераГТД <> ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Товары.НомерСтроки";
