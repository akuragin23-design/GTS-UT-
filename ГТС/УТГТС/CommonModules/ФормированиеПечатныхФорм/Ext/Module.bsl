...
//непп 130324
Функция НайтиЗаказГТС(ИНН,Номер)  Экспорт  
	//ASP_Crow 16.03.2026 000029567 В расширение
	   Заказ="";
	Запрос = Новый Запрос;
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЗаказКлиента.Контрагент.ИНН = &ИНН
		|	И ЗаказКлиента.НомерПоДаннымКлиента = &НомерПоДаннымКлиента";
	
	Запрос.УстановитьПараметр("ИНН", ИНН);
	Запрос.УстановитьПараметр("НомерПоДаннымКлиента", Номер);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Заказ="Заказ клиента №"+ВыборкаДетальныеЗаписи.Ссылка.Номер+" от "+Формат(ВыборкаДетальныеЗаписи.Ссылка.Дата, "ДЛФ=DD");
	КонецЦикла;
