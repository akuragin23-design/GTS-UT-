...
Процедура УстановитьВидимостьКомандВводаНаОсновании(Форма, ТаблицаКоманд) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ОтображатьВнеоборотныеАктивы2_2") Тогда
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			//Организация = УчетОСВызовСервера.ОрганизацияВКоторойОСПринятоКУчету(Объект.Ссылка); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		...
		Если ПравоДоступа("Добавление", Метаданные.Документы.ПринятиеКУчетуОС) Тогда
			...
			//	Элементы, "ПринятиеКУчетуОС", ТаблицаКоманд, НЕ ПринятКУчету); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		КонецЕсли;

		Если ПравоДоступа("Добавление", Метаданные.Документы.ИзменениеСостоянияОС) Тогда
			//ВнеоборотныеАктивыСлужебный.УстановитьВидимостьКомандыВводаНаОсновании(
			//	Элементы, "ИзменениеСостоянияОС", ТаблицаКоманд, ПринятКУчету); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		КонецЕсли;
		
		Если ПравоДоступа("Добавление", Метаданные.Документы.ИзменениеПараметровОС) Тогда
			//ВнеоборотныеАктивыСлужебный.УстановитьВидимостьКомандыВводаНаОсновании(
			//	Элементы, "ИзменениеПараметровОС", ТаблицаКоманд, ПринятКУчету); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		КонецЕсли;
		
		Если ПравоДоступа("Добавление", Метаданные.Документы.СписаниеОС) Тогда
			//ВнеоборотныеАктивыСлужебный.УстановитьВидимостьКомандыВводаНаОсновании(
			//	Элементы, "СписаниеОС", ТаблицаКоманд, ПринятКУчету); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		КонецЕсли;
...
Процедура ЗаполнитьСведенияОбУчете(Форма, ПредставлениеСведений) Экспорт
	...
	Элементы = Форма.Элементы;
	//ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
...
Процедура ДополнитьСведения2_4(ВнеоборотныйАктив, СведенияОбУчете, СтоимостьИАмортизация, МассивСумм, Сведения2_4) Экспорт
	...
	//Если ВнеоборотныеАктивыСлужебный.ВедетсяРегламентированныйУчетВНА() Тогда //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
	...
	//КонецЕсли;  //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
...
Функция ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	Если ВидФормы = "ФормаВыбора" Тогда
		...
		Если Параметры.Свойство("ДоговорЛизинга") И ЗначениеЗаполнено(Параметры.ДоговорЛизинга) Тогда
			//Параметры.Отбор.Вставить("НаправлениеДеятельности", ВнеоборотныеАктивыВызовСервера.НаправлениеДеятельности(Параметры.ДоговорЛизинга)); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		КонецЕсли;
...
Функция ОписаниеЗапросаДляВыбора(Параметры, УстановитьВсеОтборы, МассивОбъектов) Экспорт
	...
	ДоступенВыборУчета =  ЛОЖЬ;
	//	(ОтборСписка.Свойство("ОтражатьВРеглУчете") ИЛИ ОтборСписка.Свойство("ОтражатьВУпрУчете"))
	//	И ВнеоборотныеАктивыСлужебный.ВедетсяРегламентированныйУчетВНА(); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
...
Функция ТекстЗапросаДоступныхДляВыбораОбъектов() Экспорт
	...
	//Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4() Тогда //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
	//	
	//	ТекстЗапроса =
	//	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Функция ПечатьОС6_2003(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		//ШапкаРазделов1и2.Параметры.СрокПолезногоИспользования = ВнеоборотныеАктивыКлиентСервер.РасшифровкаСрокаПолезногоИспользования(СрокИспользования, Ложь); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		Если ЗначениеЗаполнено(ДатаПринятия) Тогда
			...
			//		ДатаПринятия, ?(ЗначениеЗаполнено(ДатаСписания), ДатаСписания, ТекущаяДатаСеанса())), Ложь); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
		КонецЕсли;
	...
	//ВнеоборотныеАктивы.ДобавитьПредупреждениеЕслиСведенияНеАктуальны(СведенияАктуальны, ТабДок); //ASP_Crow 05.03.2026 000029567 нет в старой и новой версии
