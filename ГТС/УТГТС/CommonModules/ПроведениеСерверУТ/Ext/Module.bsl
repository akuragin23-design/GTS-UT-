...
Процедура ЗаписатьНаборыЗаписей(Объект) Экспорт
	...
	Если Объект.Движения.Найти("РасчетыСКлиентами") <> Неопределено Тогда
		Для Каждого Движение ИЗ Объект.Движения.РасчетыСКлиентами Цикл
			Если Движение.Период < Дата('00010102') Тогда Движение.Период = Дата('20090101'); КонецЕсли;
		КонецЦикла; 
	КонецЕсли;
	//??-
...
Процедура ВыполнитьКонтрольРезультатовПроведения(Объект, Отказ) Экспорт
	...
	Если ЕстьИзмененияВТаблице(ДанныеТаблиц,"ОбеспечениеЗаказовИзменение") Тогда
		...
		Если ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров = 0 Тогда
			//ASP_Dante 09/07/21 Задача 000011492 перенос доработок	
			//МассивКонтролей.Добавить(Врег("ОбеспечениеЗаказов"));
			Если Не ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда	
				МассивКонтролей.Добавить(Врег("ОбеспечениеЗаказов"));
			КонецЕсли;
			//ASP_Dante 09/07/21
			ТекстЗапроса = ТекстЗапроса + 
	...
	Если ДанныеТаблиц.Свойство("ДвиженияЗаявкиНаВозвратТоваровОтКлиентовИзменение") Тогда
		//ASP_Dante 30/08/19
	...
	Если ЕстьИзмененияВТаблице(ДанныеТаблиц,"ДвиженияРасчетыСКлиентамиИзменение") Тогда
		//АСП ++
		Если ТипЗнч(Объект)=Тип("ДокументОбъект.РасходныйКассовыйОрдер") и Объект.ХозяйственнаяОперация=Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту Тогда
			Иначе
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструТоварыНаСкладах(Объект, Отказ, РезультатЗапроса)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%НоменклатураХарактеристика%",
									НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Выборка.Номенклатура, Выборка.Характеристика,)); //ASP_Dante 30/08/19//, Выборка.Серия));
...
// Альтер ЛОГО+ ДОПОЛНИТЕЛЬНЫЙ КОТРОЛЬ ОСТАТКОВ ПОСЛЕ ПРОВЕДЕНИЯ, ОТМЕНЫ ИЛИ УДАЛЕНИЯ ПРОВЕДЕНИЯ
Процедура ДополнительныйКонтрольОстатковНаСкладе(Объект, Отказ) Экспорт
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Запрос.УстановитьПараметр("ГраницаПериода", Новый Граница(Объект.Дата, ВидГраницы.Включая));
	Текст = ТекстЗапросаТаблицыТоваров(Объект.Метаданные().Имя, РеквизитСклад(Объект), СкладВТабЧасти(Объект));
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПеремещениеТоваров") тогда
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(&ГраницаПериода, Склад = &СкладОтправитель) КАК Остатки
		|			ПО ТаблицаТоваров.Номенклатура = Остатки.Номенклатура
		|				И ТаблицаТоваров.Склад = Остатки.Склад
		|				И ТаблицаТоваров.Характеристика = Остатки.Характеристика
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		ЕСТЬNULL(Остатки.ВНаличииОстаток, 0),
		|		Остатки.Склад,
		|		Остатки.Помещение,
		|		Остатки.Характеристика
		|	ИЗ
		|		ТаблицаТоваров КАК ТаблицаТоваров
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(, Склад = &СкладПолучатель) КАК Остатки
		|			ПО ТаблицаТоваров.Номенклатура = Остатки.Номенклатура
		|				И ТаблицаТоваров.Склад = Остатки.Склад
		|				И ТаблицаТоваров.Характеристика = Остатки.Характеристика) КАК Таблица
		|ГДЕ
	...
	//	|						ВТ.Склад
	//	|					ИЗ
	//	|						ТаблицаТоваров КАК ВТ)) КАК Остатки
	//	|		ПО ТаблицаТоваров.Номенклатура = Остатки.Номенклатура
	//	|			И ТаблицаТоваров.Склад = Остатки.Склад
	//	|			И ТаблицаТоваров.Характеристика = Остатки.Характеристика
	//	|ГДЕ
	...
	Иначе
		...
		|				"+?(ЭтоДокументПоступления(Объект), "", "&ГраницаПериода")+",
		|					(Номенклатура, Склад) В
		|						(ВЫБРАТЬ
		|							ВТ.Номенклатура,
		|							ВТ.Склад
	...
	СообщитьОбОшибкахПроведенияПоРегиструТоварыНаСкладах(Объект, Отказ, Выборка);
	
КонецПроцедуры

Процедура КонтрольИзмененияСоставаДокументовОприходования(Объект, Отказ, Удаление) Экспорт
	// прочитаем набор записей регистра ТоварыНаСкладах, согласно которому необходимо проверить изменение состава
	НЗ = РегистрыНакопления.ТоварыНаСкладах.СоздатьНаборЗаписей();
	НЗ.Отбор.Регистратор.Установить(Объект.Ссылка);
	НЗ.Прочитать();
	//
	Если НЗ.Количество() = 0 тогда возврат конецесли;//если документ не был проведен, тогда проверять нечего.
	//
	ТаблицаДвижений = НЗ.Выгрузить();
	//
	
	МассивДвиженийПоступления = ТаблицаДвижений.НайтиСтроки(Новый Структура("ВидДвижения", ВидДвиженияНакопления.Приход));
	
	Если МассивДвиженийПоступления.Количество() = 0 тогда возврат конецесли;//если не было проводок по приходу товаров, тогда проверять нечего
	
	// запрос сверки
	Текст = "";
	
	// Таблица товаров табличной части
	Если Не Удаление тогда // надо контролировать изменение табличной части
		Текст = ТекстЗапросаКонтроляИзмененияТабличнойЧасти(Объект);		
	Иначе// смотрим, что будет с остатками если заминусовать поступление по данному документу
		Текст = ТекстЗапросаКонтроляУдаленияДокумента(Объект);
	Конецесли;
	
	//
	Запрос = Новый Запрос(Текст);
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ТабЧасть", ТабЧастьОбъекта(Объект));
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПеремещениеТоваров") тогда
		Запрос.УстановитьПараметр("Склад", Объект.СкладПолучатель);
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.СборкаТоваров")
		ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.ПересортицаТоваров")
	...
	///
	ЗП = Новый Запрос;
	ЗП.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
	
	ЗП.Текст = 
	"Выбрать * ИЗ ОстаткиТоваров КАК ВТ
	|;
	|/////////////////////////
	|Выбрать * ИЗ ТаблицаИзменений КАК ВТ
	|;
	|/////////////////////////
	|Выбрать * ИЗ ТаблицаТоваров КАК ВТ";
	
	Массив = ЗП.ВыполнитьПакет();
...
///////
Функция ТекстЗапросаТаблицыТоваров(ИмяОбъекта, РеквизитСклад, СкладВТабЧасти)
	...
	Если ИмяОбъекта = "ПеремещениеТоваров" тогда
		...
		|	ТабЧасть.Характеристика,
		|	ТабЧасть.Количество,
		|	ТабЧасть.Ссылка.СкладПолучатель
		|ИЗ
		|	Документ.ПеремещениеТоваров.Товары КАК ТабЧасть
		|ГДЕ
		|	ТабЧасть.Ссылка = &Ссылка";
	//ИначеЕсли ИмяОбъекта = "ЗаказКлиента" тогда
	//	Текст = "ВЫБРАТЬ
	...
	//	
	Иначе
		Если ИмяОбъекта = "ПересортицаТоваров" Тогда
			ПолеНоменклатуры = "НоменклатураОприходование";
		Иначе
		...
		Текст =
		"ВЫБРАТЬ
		|	ТабЧасть."+ПолеНоменклатуры+" КАК Номенклатура,
		|	ТабЧасть.Характеристика,
		|	ТабЧасть.Количество,
		|	ТабЧасть."+?(СкладВТабЧасти, "", "Ссылка.")+РеквизитСклад+" КАК Склад
		|ПОМЕСТИТЬ ТаблицаТоваров
		|ИЗ
		|	Документ."+ИмяОбъекта+".Товары КАК ТабЧасть
		|ГДЕ
		|	ТабЧасть.Ссылка = &Ссылка";
	Конецесли;
...
Функция ТекстЗапросаТаблицыТоваровОбъекта(ИмяОбъекта, СкладВТабЧасти)
	...
	Иначе
		...
		|	ТабЧасть.Характеристика,
		|	ТабЧасть.Количество,
		|	"+?(СкладВТабЧасти, "ТабЧасть.Склад", "&Склад")+" КАК Склад
		|ПОМЕСТИТЬ ТаблицаТоваров
		|ИЗ
		|	&ТабЧасть КАК ТабЧасть";
	Конецесли;
...
Функция ТекстЗапросаКонтроляИзмененияТабличнойЧасти(Объект)
	Текст = ТекстЗапросаТаблицыТоваровОбъекта(Объект.Метаданные().Имя, СкладВТабЧасти(Объект))+
	";
	|
	|/////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаПоступления.Номенклатура,
	|	ТаблицаПоступления.Склад,
	|	ТаблицаПоступления.ВНаличии КАК Количество
	|ПОМЕСТИТЬ ТаблицаДвижения
	|ИЗ
	|	РегистрНакопления.ТоварыНаСкладах КАК ТаблицаПоступления
	|ГДЕ
	|	ТаблицаПоступления.Регистратор = &Ссылка
	|	И ТаблицаПоступления.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Таблица.Номенклатура,
	|	Таблица.Склад,
	|	СУММА(Таблица.ИзменениеПрихода) КАК ИзменениеПрихода
	|ПОМЕСТИТЬ ТаблицаИзменений
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЕСТЬNULL(ТаблицаДвижения.Номенклатура, ТаблицаТоваров.Номенклатура) КАК Номенклатура,
	|		ЕСТЬNULL(ТаблицаДвижения.Склад, ТаблицаТоваров.Склад) КАК Склад,
	|		ЕСТЬNULL(ТаблицаТоваров.Количество, 0) - ЕСТЬNULL(ТаблицаДвижения.Количество, 0) КАК ИзменениеПрихода
	|	ИЗ
	|		ТаблицаДвижения КАК ТаблицаДвижения
	|			ПОЛНОЕ СОЕДИНЕНИЕ ТаблицаТоваров КАК ТаблицаТоваров
	|			ПО ТаблицаДвижения.Номенклатура = ТаблицаТоваров.Номенклатура
	|				И ТаблицаДвижения.Склад = ТаблицаТоваров.Склад) КАК Таблица
	|
	|СГРУППИРОВАТЬ ПО
	|	Таблица.Номенклатура,
	|	Таблица.Склад
	...
	|							ВТ.Склад
	|						ИЗ
	|							ТаблицаИзменений КАК ВТ)) КАК ОстаткиТоваров
	|
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Таблица.ОстатокПослеЗаписи КАК Количество,
	|	Таблица.Номенклатура,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЕСТЬNULL(ОстаткиТоваров.ВНаличииОстаток, 0) + ТаблицаИзменений.ИзменениеПрихода КАК ОстатокПослеЗаписи,
	...
	|			ПО ТаблицаИзменений.Номенклатура = ОстаткиТоваров.Номенклатура
	|				И ТаблицаИзменений.Склад = ОстаткиТоваров.Склад) КАК Таблица
	|ГДЕ
	|	Таблица.ОстатокПослеЗаписи < 0";
	возврат Текст;
КонецФункции

Функция ТекстЗапросаКонтроляУдаленияДокумента(Объект)
	Текст = 
	"ВЫБРАТЬ
	|	ТаблицаПоступления.Номенклатура,
	|	ТаблицаПоступления.Склад,
	|	ТаблицаПоступления.ВНаличии КАК Количество
	|ПОМЕСТИТЬ ТаблицаПоступления
	|ИЗ
	|	РегистрНакопления.ТоварыНаСкладах КАК ТаблицаПоступления
	|ГДЕ
	|	ТаблицаПоступления.Регистратор = &Ссылка
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЕСТЬNULL(ОстаткиТоваров.ВНаличииОстаток, 0) - ТаблицаИзменений.Количество КАК ОстатокПослеЗаписи,
	|		ТаблицаИзменений.Номенклатура КАК Номенклатура,
	|		ТаблицаИзменений.Склад КАК Склад,
	|		ОстаткиТоваров.Характеристика КАК Характеристика,
	|		ОстаткиТоваров.Помещение КАК Помещение
	|	ИЗ
	|		ТаблицаПоступления КАК ТаблицаИзменений
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(
	|					,
	|					(Номенклатура, Склад) В
	|						(ВЫБРАТЬ
	...
	|							ВТ.Склад
	|						ИЗ
	|							ТаблицаПоступления КАК ВТ)) КАК ОстаткиТоваров
	|			ПО ТаблицаИзменений.Номенклатура = ОстаткиТоваров.Номенклатура
	|				И ТаблицаИзменений.Склад = ОстаткиТоваров.Склад) КАК Таблица
	|ГДЕ
	|	Таблица.ОстатокПослеЗаписи < 0";
...
Функция СкладВТабЧасти(Объект)
	возврат НЕ (ТипЗнч(Объект) = Тип("ДокументОбъект.ПеремещениеТоваров")
					ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.ПересортицаТоваров")
					ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.СписаниеНедостачТоваров")
					ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.СборкаТоваров")
					ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.ВозвратТоваровОтКлиента")
					ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.ОприходованиеИзлишковТоваров")); //ASP__A.Zhel 12.03.2019 добавил тип оприходование__ASP 
КонецФункции

Функция СтрокаОтбораВТ(Объект)
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПеремещениеТоваров") тогда
		Текст = "Номенклатура В
				|	(ВЫБРАТЬ
				|		ВТ.Номенклатура
				|	ИЗ
...
Функция СкладыПеремещения(Объект)
	Список = Новый СписокЗначений();
	Если НЕ ТипЗнч(Объект) = Тип("ДокументОбъект.ПеремещениеТоваров") тогда возврат неопределено конецесли;
	Список.Добавить(Объект.СкладОтправитель);
	Список.Добавить(Объект.СкладПолучатель);
	возврат Список;
КонецФункции

Функция ЭтоДокументПоступления(Объект)
	возврат ТипЗнч(Объект) = Тип("ДокументОбъект.ПриобретениеТоваровУслуг");
КонецФункции

Функция ТабЧастьОбъекта(Объект)
	Если Объект.Метаданные().Имя = "СборкаТоваров" тогда
		ТабЧасть = Новый ТаблицаЗначений();
		ТабЧасть.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		ТабЧасть.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
		ТабЧасть.Колонки.Добавить("Склад", Новый ОписаниеТипов("СправочникСсылка.Склады"));
		ТабЧасть.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10,3)));
		//
		СтрокаТЧ = ТабЧасть.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТЧ, Объект, "Номенклатура, Характеристика, Склад, Количество");
		//
		Для каждого СтрокаТЗ из Объект.Товары цикл
			СтрокаТЧ = ТабЧасть.Добавить();
			СтрокаТЧ.Склад = Объект.Склад;
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, СтрокаТЗ, "Номенклатура, Характеристика, Количество");			
		Конеццикла;		
		//
	Иначе
