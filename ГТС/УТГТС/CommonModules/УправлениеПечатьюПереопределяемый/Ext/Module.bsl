...
Процедура ПриПечати(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// ГТС фиксируем печать документов(ну точнее предпросмотр)
	Попытка	
		Для Каждого Стр1 из ОбъектыПечати Цикл
			ЗаписьЖурналаРегистрации("Печать документа",УровеньЖурналаРегистрации.Информация,
			ТипЗнч(Стр1.Значение),Стр1.Значение,"Печать документа "+Стр1.Значение,РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
		КонецЦикла;	
	Исключение
	КонецПопытки;  
	//--//ASP_Crow 16.03.2026 000029567 В расширение
