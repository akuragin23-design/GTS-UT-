...
Процедура ЗаписатьОбщееСостояниеОригиналаДокумента(Документ, Состояние) Экспорт
	...
	//ГТС убираем возможность сменить статус обратно с архива на другой статус в результате повторной печати документов 07.09.2022
	ТекущееСостояние= УчетОригиналовПервичныхДокументов.СведенияОСостоянииОригиналаПоСсылке(Документ);
	Если ЗначениеЗаполнено(ТекущееСостояние) Тогда
		Если ТекущееСостояние.СостояниеОригиналаПервичногоДокумента=Справочники.СостоянияОригиналовПервичныхДокументов.НайтиПоНаименованию("Помещен в архив") Тогда
