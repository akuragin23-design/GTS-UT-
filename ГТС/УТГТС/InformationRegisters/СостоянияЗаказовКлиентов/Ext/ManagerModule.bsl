...
Процедура ДобавитьЗаданияКОтражениюСостоянияЗаказов(
	...
	// ГТС быстро-фикс: при расчетах по договорам заказ может не попасть
// в задания через типовой анализ движений РасчетыСКлиентами.
	Если ИсточникСсылка <> Неопределено Тогда
	ASP_ДополнитьЗаказыПоРасшифровкеПлатежа(Источник, Заказы, ТипыЗаказов);
	КонецЕсли;
...
//ГТС Фикс для договоров

Процедура ASP_ДополнитьЗаказыПоРасшифровкеПлатежа(Источник, Заказы, ТипыЗаказов)
	...
	Если ТипИсточника <> Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств")
		И ТипИсточника <> Тип("ДокументСсылка.ПриходныйКассовыйОрдер")
		И ТипИсточника <> Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаПлатежа Из Источник.РасшифровкаПлатежа Цикл
		...
		Если ТипыЗаказов.Найти(ТипЗнч(СтрокаПлатежа.ОснованиеПлатежа)) <> Неопределено Тогда
			
			Заказ = СтрокаПлатежа.ОснованиеПлатежа;

		ИначеЕсли ЗначениеЗаполнено(СтрокаПлатежа.ОбъектРасчетов)
			И ТипыЗаказов.Найти(ТипЗнч(СтрокаПлатежа.ОбъектРасчетов.Объект)) <> Неопределено Тогда
			
			Заказ = СтрокаПлатежа.ОбъектРасчетов.Объект;
...
#КонецОбласти
 ...
 Процедура ASP_ИзменитьСтатусЗаказа(ТаблицаСостоянийЗаказов)  
	 //ASP_Crow 18.03.2026 000029567 В расширение
	 
	ОтборРезерв = Новый Структура("ВариантОбеспечения", Перечисления.ВариантыОбеспечения.СоСклада);
	ОтборОбеспеч = Новый Структура("Обособленно", Истина);
	
	Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Статуса заказа ИД (Список заказов клиентов)", Истина);
	
	Для каждого СтрокаТабл из  ТаблицаСостоянийЗаказов Цикл     
		
		ТаблицаТоваров = СтрокаТабл.Заказ.Товары;
		СтрокиРезерва = ТаблицаТоваров.НайтиСтроки(ОтборРезерв);
		Обеспечивается = ТаблицаТоваров.НайтиСтроки(ОтборОбеспеч); 
		ТаблицаОтмены = ТаблицаТоваров.Выгрузить(,"Отменено,ПричинаОтмены");
		ТаблицаОтмены.Свернуть("Отменено,ПричинаОтмены");  
		ЕстьОтмена = Ложь;
		Для каждого СтрОтм из ТаблицаОтмены Цикл
			Если СтрОтм.Отменено и ЗначениеЗаполнено(СтрОтм.ПричинаОтмены) Тогда
				ЕстьОтмена = Истина;
				Прервать;
			КонецЕсли;	
		КонецЦикла;	
		
		Если не СтрокаТабл.ПроцентОтгрузки = 100 и не СтрокаТабл.ПроцентОплаты = 100 и СтрокиРезерва.Количество()>0 и Не ЕстьОтмена  Тогда  
			ЗначениеКода = Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("NR", Истина); 
		ИначеЕсли  Обеспечивается.Количество()>0 и Не ЕстьОтмена  Тогда 
			ЗначениеКода = Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("OO", Истина);
		ИначеЕсли не СтрокаТабл.ПроцентОтгрузки = 100 и СтрокаТабл.ПроцентОплаты = 100 и Не ЕстьОтмена  Тогда 
			ЗначениеКода = Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("P", Истина);
		ИначеЕсли СтрокаТабл.ПроцентОтгрузки = 100 и не СтрокаТабл.ПроцентОплаты = 100 и Не ЕстьОтмена Тогда 
			ЗначениеКода = Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("NP", Истина);
		ИначеЕсли СтрокаТабл.ПроцентОтгрузки = 100 и  СтрокаТабл.ПроцентОплаты = 100 и Не ЕстьОтмена Тогда 
			ЗначениеКода = Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("F", Истина);
		ИначеЕсли ЕстьОтмена Тогда 
			ЗначениеКода = Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("C", Истина);
		Иначе
			ЗначениеКода = Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("NM", Истина);
		КонецЕсли;
		
		Запись = РегистрыСведений.ДополнительныеСведения.СоздатьМенеджерЗаписи();
		Запись.Объект = СтрокаТабл.Заказ;
		Запись.Свойство = Свойство;
		Запись.Прочитать();
		//++ASP_СмирновН 04.06.25 000027146 возможная проблема - значениекода было ссылкой, запись.значение - строка
		//возможное решение - брать наименование
		Если Запись.Значение <> ЗначениеКода.Наименование Тогда 
		//--ASP_СмирновН 04.06.25 00002714
			Запись.Объект = СтрокаТабл.Заказ;
			Запись.Свойство = Свойство;
			Запись.Значение = ЗначениеКода; 
			Запись.Записать(); 
			Если  Запись.Объект.Проведен Тогда         
				узел_ЛК = ПланыОбмена.Б_ОбменССайтом.НайтиПоКоду("000000004");
				ПланыОбмена.ЗарегистрироватьИзменения(узел_ЛК, Запись.Объект);
			КонецЕсли;
...
Процедура ОтразитьСостояниеЗаказа(Заказы, УдалениеПроведения = Ложь, Ошибки = Неопределено)
	...
	//ASP_Мочалина 000026488 изменение статуса заказ клиента и его постановка на регистрацию для обмена  с ЛКД 
	//ASP_Crow 20.03.2026 000029567 Перенести в расширение
	 ASP_ИзменитьСтатусЗаказа(ТаблицаСостоянийЗаказов);
	 // 000026488
КонецПроцедуры
