...
Процедура ПроизвестиЧтениеДанных(ЧтениеСообщения)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Объект" Тогда
			...
			// АльтерЛОГО +Троненко: данные объектного типа не загружаем			
			//ASP. Пропускать правильно так
			одПропустить(ФайлОбмена, "Объект");
			продолжить; 
			//--//ASP_Crow 18.03.2026 000029567 В расширение
		...
		ИначеЕсли ИмяУзла = "УдалениеОбъекта" Тогда
			// АльтерЛОГО +Троненко: ничего не удаляем
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	//ASP_Crow 18.03.2026 000029567 В расширение
	//ASP. Обмен с БП КОРП 
	Если ПолеИмяПланаОбмена= "ОбменУправлениеТорговлейБухгалтерияПредприятияКОРП30" Тогда 
		ПолеИмяПланаОбмена="ОбменУправлениеТорговлейБухгалтерияПредприятия30";
	КонецЕсли;
...
Процедура ЗаписатьИнформациюОбОбменеДаннымиЧерезПланыОбмена(Знач НомерОтправленного)
	...
	//ASP_Crow 18.03.2026 000029567 В расширение
	//ASP. Обмен с БП КОРП 
	Если ПолеИмяПланаОбмена="ОбменУправлениеТорговлейБухгалтерияПредприятия30" Тогда 
		УстановитьАтрибут(Приемник, "ПланОбмена", "ОбменУправлениеТорговлейБухгалтерияПредприятияКОРП30");
	Иначе
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	// Альтер ЛОГО+ //ASP_Crow 18.03.2026 000029567 В расширение
	ОбменДаннымиСобытия.ЗарегистрироватьОбъектыПоСсылкам(УзелДляОбмена);
	// +Альтер ЛОГО
	ИнициализироватьКомментарииПриВыгрузкеИЗагрузкеДанных();
	...
	Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
		...
		Если Отказ Тогда // Отказ от выгрузки данных  
			//ASP_Crow 18.03.2026 000029567 В расширение
			//Константы.ASP_ОшибкаОбмена.Установить(Истина); // ASP_Lebedev 08/12/2023 Установка истина при ошибке обмена
			ЗавершитьВедениеПротоколаОбмена();
			Возврат;
		КонецЕсли;
