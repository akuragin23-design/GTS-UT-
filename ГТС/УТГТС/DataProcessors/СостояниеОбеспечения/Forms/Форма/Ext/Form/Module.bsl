...
Функция ТаблицаКорректировкиГрафикаПоступления(МенеджерВременныхТаблиц, СторнируемыеЗаказы)
	...
	ЗапросГрафикаПоступлений.Текст =
		...
		|			= ВЫБОР КОГДА Заказы.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОбособленно) ТОГДА
		|						Заказы.Назначение
		|					ИНАЧЕ
...
Функция ТребуетсяДинамическоеРаспределениеОстатков(ТипНоменклатуры, ВариантОбеспечения)
	...
	Результат = ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.УдалитьОбособленно")
		Или ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно");
...
Процедура ЗаполнитьВНаличииЗаказано(Потребность, ДатыОбеспечения);
	...
	Если Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОбособленно
		Или Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно Тогда
	...
	ИначеЕсли Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.РезервироватьПоМереПоступления Тогда
		Потребность.Заказано = Истина;
...
Функция ДатыОбеспеченияПотребности(Потребность, ТаблицаОстатков)
	...
	Если ТаблицаОстатков.Количество() = 0
		И (Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОбособленно
			Или Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно) Тогда
	...
	КорректироватьИтог = Потребность.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить")
		...
		Или Потребность.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.РезервироватьПоМереПоступления");
	...
	Корректировать = Потребность.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.РезервироватьПоМереПоступления");
...
Функция ЗаполнитьРекомендацииВСтрокеТовары(СтрокаТовары, Статистика)
	...
	ЖелаемаяДатаСлева =
		СтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.КОбеспечению
		Или СтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.РезервироватьПоМереПоступления
			И СтрокаТовары.ДатаОтгрузки > СтрокаТовары.ЖелаемаяДатаОтгрузки
		Или СтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОбособленно
			И СтрокаТовары.ДатаОтгрузки > СтрокаТовары.ЖелаемаяДатаОтгрузки
		Или СтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно
	...
	СдвинутьВлево =
		ЖелаемаяДатаСлева И(
			...
			Или СтрокаТовары.ЕстьДатаЧастичногоОбеспечения И Не СтрокаТовары.ВНаличии И Не СтрокаТовары.Заказано
				И (СтрокаТовары.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.УдалитьОбособленно
						И СтрокаТовары.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно
...
Процедура ВыполнитьОбеспечение(Источник, Заказ, Параметр, ЖелаемаяДатаОтгрузки, НеОтгружатьЧастями)
	...
	КонецЕсли;
	ПереченьВариантов.Добавить(Перечисления.ВариантыОбеспечения.РезервироватьПоМереПоступления);
	ПереченьВариантов.Добавить(Перечисления.ВариантыОбеспечения.КОбеспечению);
	Если ИспользоватьОбособленно Тогда
		ПереченьВариантов.Добавить(Перечисления.ВариантыОбеспечения.УдалитьОбособленно);
	КонецЕсли;
