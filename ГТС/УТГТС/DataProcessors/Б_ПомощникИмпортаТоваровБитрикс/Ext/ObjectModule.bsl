...
Процедура СоздатьТовары(КоличествоКартинок, АдресВХранилищеСтруктураДанных, Протокол) Экспорт
	...
	Если УстанавливатьЦены
		...
		ДокУстановкиЦен.Дата = ТекущаяДатаСеанса();
...
Процедура НайтиСоздатьСвойство(ДанныеСвойства, Новое)
	...
	Свойство.НаборСвойств		= ВидНоменклатуры.НаборСвойств;
	Свойство.Записать();
...
Процедура НайтиСоздатьСвойствоХарактеристики(Характеристика, СтрокаСвойства)
	...
	Если Свойство = НеОпределено Тогда
		...
		НовоеСвойство.НаборСвойств	= ВидНоменклатуры.НаборСвойствХарактеристик;
		НовоеСвойство.Доступен 		= Истина;
		НовоеСвойство.Виден 		= Истина;
	...
	Если ВидНоменклатуры.НаборСвойствХарактеристик.ДополнительныеРеквизиты.Найти(Свойство, "Свойство") = Неопределено  И ЗначениеЗаполнено(ВидНоменклатуры.НаборСвойствХарактеристик) Тогда
...
Процедура СохранитьКартинку(ФайлКартинки, АдресКартинки, РасширениеФайла, Номенклатура)
	...
	Спр.ДатаМодификацииУниверсальная = Спр.ДатаСоздания;
	Спр.Записать();
	
	ПрисоединенныйФайл = Спр.Ссылка;
	
	Рег = РегистрыСведений.ДвоичныеДанныеФайлов.СоздатьМенеджерЗаписи();
	Рег.Файл = ПрисоединенныйФайл;
	Рег.ДвоичныеДанныеФайла = Новый ХранилищеЗначения(Новый ДвоичныеДанные(ФайлКартинки));
	Рег.Записать();
...
Процедура ДобавитьВПротокол(Протокол, ТекстРаздела, Обработано, Найдено, Создано)
	...
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "[Время]", Формат(ТекущаяДатаСеанса(), "ДЛФ=DT"));
