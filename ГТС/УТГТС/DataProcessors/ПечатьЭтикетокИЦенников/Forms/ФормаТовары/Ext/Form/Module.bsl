...
&НаСервере
ФункЦия Назначение(Имя)                  
	//ASP_Crow 20.03.2025 000023395 В расширение
	Назначение = Справочники.ШаблоныЭтикетокИЦенников.НайтиПоНаименованию(Имя).Назначение;
	Если Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляКомплектов Тогда 
		Возврат Истина ;
	Иначе Возврат Ложь;
	КонецЕсли;
КонецФункции
...
Функция ПолучитьПараметры()
	...
	Товары.Колонки.Добавить("ВариантКомплекта", Новый ОписаниеТипов("СправочникСсылка.ВариантыКомплектацииНоменклатуры")); /// Мочалина //ASP_Crow 20.03.2025 000023395 В расширение
