		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьУпаковкиНоменклатуры") Тогда
			...
			ТекстЗамены = СтрЗаменить(ТекстЗамены, "&ТекстЗапросаКоэффициентУпаковки",
				Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
					...
					"ИсходныеДанныеПоследнийЗапрос.Номенклатура"));
			Запрос.Текст = СтрЗаменить(Запрос.Текст, ЗаменяемыйТекст, ТекстЗамены);
			//++ASP_Crow 06.03.2026 000029567 поместить в расширение
			Если ИсходныеДанные.Колонки.Найти("ВариантКомплекта") <> неопределено и  ИсходныеДанные.Колонки.Найти("ШККомплект") <> Неопределено и
				ИсходныеДанные[0].ШаблонЭтикетки.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляКомплектов Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекущееВремя КАК ТекущееВремя,", "ИсходныеДанныеПоследнийЗапрос.ВариантКомплекта КАК ВариантКомплекта," +
				Символы.ПС + Символы.Таб + "&ТекущееВремя КАК ТекущееВремя,");	
			КонецЕсли;
