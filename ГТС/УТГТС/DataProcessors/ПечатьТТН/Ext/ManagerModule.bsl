...
Процедура ЗаполнитьРеквизитыСтрокиТовараТТН(ДанныеПечати, СтрокаТовары, ДанныеСтроки, ОбластьМакета, ЕдиницаИзмеренияВеса = Неопределено, КоэффициентПересчетаВТонны = 0)
	...
	Иначе
		...
		Если ?(СтрокаТовары.КоличествоМест=null,1,СтрокаТовары.КоличествоМест) - Цел(?(СтрокаТовары.КоличествоМест=null,1,СтрокаТовары.КоличествоМест)) > 0 Тогда
			ДанныеСтроки.Мест = Цел(?(СтрокаТовары.КоличествоМест=null,1,СтрокаТовары.КоличествоМест)) + 1;
		//ASP_Dante 12/07/21
		Иначе
			//ASP_Dante 12/07/21 Задача 000011492 перенос доработок
			//ДанныеСтроки.Мест = СтрокаТовары.КоличествоМест;
			ДанныеСтроки.Мест = ?(СтрокаТовары.КоличествоМест=null,1,СтрокаТовары.КоличествоМест);
			//ASP_Dante 12/07/21
		КонецЕсли;
