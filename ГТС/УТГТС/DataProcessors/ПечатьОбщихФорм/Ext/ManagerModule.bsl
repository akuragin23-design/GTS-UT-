...
Процедура ЗаполнитьРеквизитыШапкиТОРГ12(ДанныеПечати, Макет, ТабличныйДокумент)
	...
	//ASP КАВ начало
	ОбластьМакета.Параметры.НомерДокумента = ?(Лев(ДанныеПечати.Номер, 3) = "ГТ-", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Истина, Истина), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Ложь, Истина));
	ОбластьМакета.Параметры.Основание = СтрЗаменить(ОбластьМакета.Параметры.Основание, "ГТ-", "");
	//ASP КАВ конец
	//--ASP_Crow 10.03.2026 000029567 Перенести в расширение
...
Процедура ЗаполнитьТабличныйДокументТОРГ12(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	//++ASP_Crow 10.03.2026 000029567 добавить в расширение 
	// Альтер ЛОГО+
...
Процедура ЗаполнитьРеквизитыШапкиСчетФактура(ДанныеПечати, ДанныеОснований, СведенияОПоставщике, ДанныеКонтрагентов, ТабличныйДокумент, ОпцииПечатиСчетаФактуры, ДанныеПоставщиков)
	...
	//++ASP_Crow 10.03.2026 000029567 Перенести в расширение
	//ASP КАВ начало
	//ОбластьМакета.Параметры.Номер = "Счет-фактура № " + ?(Лев(ДанныеПечати.Ссылка.Номер, 3) = "ГТ-", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Истина, Истина), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Ложь, Ложь));
	
	//++ASP Куренков 08.06.2026 000030649 Теперь номер на печать если не ГТ- просто берется из документа, так как иначе обрезались буквы
	ОбластьМакета.Параметры.Номер = "Счет-фактура № " 
	+ ?(Лев(ДанныеПечати.Ссылка.Номер, 3) = "ГТ-",
	ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Истина, Истина),
	ДанныеПечати.Номер);  
	//ASP Куренков 08.06.2026 000030649
...
Процедура ЗаполнитьРеквизитыШапкиУПД(ДанныеПечати, СведенияОПоставщике, ДанныеКонтрагентов, ОбластьМакета, ДанныеПоставщиков, ОпцииПечати)
	...
	//++ASP_Crow 10.03.2026 000029567 Перенести в расширение
	//ASP_Dante 22/07/21 задача 000011492 Перенос доработок
	...
	//КонецЕсли;
	//ASP_Dante 22/07/21	ПараметрыШапки.Вставить("Дата", Формат(ДанныеПечати.Дата, "ДЛФ=ДД")); 
	//++ASP_Crow 10.03.2026 000029567 Перенести в расширение
	...
	СтрокаПоДокументу = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1'"),
		//++ASP_Crow 10.03.2026 000029567 Перенести в расширение
		?(ПустаяСтрока(СтрЗаменить(ДанныеПечати.СтрокаПоДокументу, "ГТ-", "")),  //ASP_Dante 22/07/21 СтрЗаменить(, "ГТ-", "") 
		НСтр("ru='-- от --'", Метаданные.Языки.Русский.КодЯзыка),
		СтрЗаменить(ДанныеПечати.СтрокаПоДокументу, "ГТ-", "")));
	    //--ASP_Crow 10.03.2026 000029567 Перенести в расширение
	...
	//++ASP_Crow 30.01.2026 000029400 Добавляем ввод данных по авансовым СФ
	//++ASP_Crow 10.03.2026 000029567 Перенести в расширение
	//ПараметрыШапки.Вставить("АвансовыеСчетаФактуры",ASP_ПолучитьДанныеАвансов(ДанныеПечати)); 
	//--ASP_Crow 30.01.2026 000029400
...
Процедура ЗаполнитьРеквизитыПодвалаУПД(ОбластьПодвала, ДанныеПечати, СведенияОбОрганизации, ДанныеКонтрагентов)
	...
	//++ASP_Crow 10.03.2026 000029567 добавить в расширение
	//ASP_Dante 02/09/19
	Если Лев(ДанныеПечати.Основание, 13) = "Заказ клиента" Тогда
		Если Не Сред(ДанныеПечати.Основание,17, 3) = "ГТ-" Тогда
			Заказ= ФормированиеПечатныхФорм.НайтиЗаказГТС(ИННПокупателя,ДанныеПечати.ОснованиеНомер);
			ТекстОснования = "Счет на оплату №" + ?(Лев(Заказ, 3) = "ГТ-", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Заказ, Истина, Истина), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Заказ, Ложь, Истина));
			СтруктураПараметров.Вставить("Основание", ТекстОснования);  
		Иначе
			ТекстОснования = "Счет на оплату №" + ?(Лев(ДанныеПечати.Основание, 3) = "ГТ-", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Основание, Истина, Истина), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Основание, Ложь, Истина));
			СтруктураПараметров.Вставить("Основание", ТекстОснования);
		КонецЕсли;
	КонецЕсли;
	//ASP_Dante 02/09/19
	...
	Если ЗначениеЗаполнено(ДанныеПечати.ДоверенностьНомер) И ЗначениеЗаполнено(ДанныеПечати.ДоверенностьДата)
		...
		//++ASP_Crow 10.03.2026 000029567 добавить в расширение
			//ASP_Dante 02/09/19
			Основание1 = ДанныеПечати.Основание;
			Если Лев(ДанныеПечати.Основание, 13) = "Заказ клиента" Тогда
				Основание1 = "Счет на оплату №" + ?(Лев(ДанныеПечати.Основание, 3) = "ГТ-", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Основание, Истина, Истина), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Основание, Ложь, Истина));
			КонецЕсли;
			
			ТекстОснования = СокрЛП(Основание1) + "; "
			//ASP_Dante 02/09/19ТекстОснования = СокрЛП(ДанныеПечати.Основание) + "; "
			//--ASP_Crow 10.03.2026 000029567 добавить в расширение			
...
КонецПроцедуры
	...
	Функция ASP_ПолучитьДанныеАвансов(ДанныеПечати)   
		//ASP_Crow 20.03.2025 000023395 В расширение
		//ASP_Crow Юрий сказал пока что без вывода авансовых СФ
		//Запрос = новый Запрос;
		//Запрос.Текст = ;
		Возврат "№ -- от --, исправление № -- от --";
...
Процедура ЗаполнитьРеквизитыШапкиУКД(ДанныеПечати, ДанныеОснований, СведенияОПоставщике, СведенияОПокупателе, ДанныеПоставщиков, Макет, ТабличныйДокумент, ОпцииПечати)
	...
	ИначеЕсли СтрокиОснований.Следующий() И ЗначениеЗаполнено(СтрокиОснований.НомерСчетаФактуры) Тогда
		...
		КонецЕсли;
		Если ОпцииПечати.ИспользоватьПечатныеФормыСФ2_5 Тогда
			//++ASP_Crow 08.06.2026 000029567
			//ОбластьМакета.Параметры.НомерОснования = НомерСчетаФактурыБезПрефикса;
			ОбластьМакета.Параметры.НомерОснования = НомерСчетаФактурыНаПечать(НомерСчетаФактурыБезПрефикса, ДанныеПечати.ИндексПодразделения);
			//--ASP_Crow 08.06.2026 000029567
...
Процедура ЗаполнитьРеквизитыПодвалаУКД(ОбластьПодвала, ДанныеПечати, ДанныеОснований, СведенияОбОрганизации, СведенияОКонтрагенте, ОпцииПечати)
	...
	Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		...
		Пока СтрокиОснований.СледующийПоЗначениюПоля("ИсходныйДокумент") Цикл
			...
			НомерСчетаФактурыБезПрефикса = ПрефиксацияОбъектовКлиентСервер.УдалитьПрефиксыИзНомераОбъекта(НомерСчетаФактурыБезПрефикса, Истина, Истина); //ASP Жуков 11-03-22   000014507
