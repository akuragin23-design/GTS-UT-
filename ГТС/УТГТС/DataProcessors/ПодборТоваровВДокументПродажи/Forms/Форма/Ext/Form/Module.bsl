...
Процедура ASP_Цены(Команда)                    
	...
	ТекущаяСтрока = ЭтаФорма.Элементы[ПодборТоваровКлиентСервер.ИмяСпискаНоменклатурыПоВариантуПоиска(ЭтаФорма)].ТекущаяСтрока;
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура НоменклатураНаСайте(Команда)  
	//ASP_Crow 20.03.2025 000023395 В расширение
	ТекущаяСтрока = ЭтаФорма.Элементы[ПодборТоваровКлиентСервер.ИмяСпискаНоменклатурыПоВариантуПоиска(ЭтаФорма)].ТекущаяСтрока;
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ЗапуститьПриложение("https://gortorgsnab.ru/search/?q="+ПолучитьАртикул(ТекущаяСтрока));
КонецПроцедуры
