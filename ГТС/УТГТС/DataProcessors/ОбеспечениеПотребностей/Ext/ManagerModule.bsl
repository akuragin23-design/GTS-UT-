...
Функция КоличествоКЗаказуДляПоддержанияЗапаса(Товар) Экспорт
	...
	//Меняем на свой алгоритм, ГТС 
	Если значениезаполнено(Товар.РезервыНаСкладе) и значениезаполнено(Товар.МинимальныйЗапас) тогда
		КЗаказу = Товар.МинимальныйЗапас-Товар.РезервыНаСкладе-Товар.ЗаказыКпоступлению;
		Отметка = ?(КЗаказу>0,истина,ложь)
	КонецЕсли;
	//Конец изменения ГТС
	Результат = Новый Структура();
...
Функция ВременнаяТаблицаСвободныеОстатки(ВТаблицеОтбораЕстьНазначения)
	...
	Текст =
		...
		//++ASP_Crow 24.07.2024 000023663 Отбор по складу //ASP_Crow 18.03.2026 000029567 В расширение
		|   И Остатки.Склад = &ASP_Склад
		//--ASP_Crow 24.07.2024 000023663
		|;
		|
		|///////////////////////////////////////////////////////////////////
