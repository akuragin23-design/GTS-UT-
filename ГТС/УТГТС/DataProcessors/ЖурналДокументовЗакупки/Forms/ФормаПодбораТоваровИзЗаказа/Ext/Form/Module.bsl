...
Процедура ПерезаполнитьКоличествоПодобрано(СЗаполнением = Ложь, ТоварыНакладной = Неопределено, ДанныеОтбора = Неопределено)
	...
	Если СЗаполнением Тогда
		...
		Для каждого ТекСтрока Из ТаблицаТоваров Цикл
			...
			//++ГТС добавляем срок отгрузки
			ТекСтрока.ДатаОтгрузки=ПолучитьДатуОтгрузкиПоНазначению(ТекСтрока.Номенклатура, ТекСтрока.назначение);
			//--ГТС
...
#КонецОбласти
 ...
 //ГТС Добавляем дату отгрузки по назначению 23.06.2020
&НаСервере
Функция ПолучитьДатуОтгрузкиПоНазначению(номенклатура, назначение)
	//ASP_Crow 18.03.2026 000029567 В расширение 		
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ГрафикОтгрузкиТоваров.Номенклатура КАК Номенклатура,
		|	ГрафикОтгрузкиТоваров.Назначение КАК Назначение,
		|	МИНИМУМ(ГрафикОтгрузкиТоваров.ДатаОтгрузки) КАК ДатаОтгрузки
		|ИЗ
		|	РегистрНакопления.ГрафикОтгрузкиТоваров КАК ГрафикОтгрузкиТоваров
		|ГДЕ
		|	ГрафикОтгрузкиТоваров.Номенклатура = &Номенклатура
		|	И ГрафикОтгрузкиТоваров.Назначение = &Назначение
		|	И ГрафикОтгрузкиТоваров.Регистратор ССЫЛКА Документ.ЗаказКлиента
		|
		|СГРУППИРОВАТЬ ПО
		|	ГрафикОтгрузкиТоваров.Номенклатура,
		|	ГрафикОтгрузкиТоваров.Назначение";
