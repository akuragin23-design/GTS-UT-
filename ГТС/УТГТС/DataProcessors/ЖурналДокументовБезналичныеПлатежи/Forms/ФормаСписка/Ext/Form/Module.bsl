...
Процедура ОбновитьГиперссылкиКОформлению()
	...
	Если ЭлементСписка <> Неопределено Тогда
		ЭлементСписка.Представление = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Зачисление на лицевые счета'"), , ЦветаСтиля.ЦветГиперссылки, , "ЗачислениеДС");
	КонецЕсли;
...
Функция ГиперссылкаКПоступлению(КоличествоКПоступлению)
	...
	Возврат Новый ФорматированнаяСтрока(ТекстГиперссылки,,
		?(КоличествоКПоступлению > 0, ЦветаСтиля.ЦветГиперссылки, ЦветаСтиля.НезаполненноеПолеТаблицы),, "КПоступлению");
...
Функция ГиперссылкаКОплатеПереводу(КоличествоКОплатеПереводу)
	...
	Возврат Новый ФорматированнаяСтрока(ТекстГиперссылки,,
		?(КоличествоКОплатеПереводу > 0, ЦветаСтиля.ЦветГиперссылки, ЦветаСтиля.НезаполненноеПолеТаблицы),, "КОплате");
...
Процедура ОбновитьГиперссылкиКОформлениюЛокализация()
	...
	Если ИспользоватьВалютныеПлатежи Тогда
		...
		Если ЭлементСписка <> Неопределено Тогда
			ЭлементСписка.Представление = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Валютный контроль'"),, ЦветаСтиля.ЦветГиперссылки,, "ВалютныйКонтроль");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЭлементСписка <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ЗаголовокПерепискаСБанками) Тогда
			...
			Иначе
				ЭлементСписка.Представление = Новый ФорматированнаяСтрока(
					ЗаголовокПерепискаСБанками,, ЦветаСтиля.ЦветГиперссылки,, "ПерепискаСБанками");
			КонецЕсли;
		Иначе
