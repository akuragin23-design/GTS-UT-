...
Функция API(Запрос)
	...
	Если ИспользоватьЛогирование И НЕ Роутинг = "Ping" Тогда // отключаем лог ping 		
		Попытка 	
			ЗаписатьВЛог(Запрос, Ответ, Результат, Роутинг, "", КаталогХраненияЛогов);		
		Исключение 		
			Попытка 				
				ЗаписатьВЛог("", "", "", "", ОписаниеОшибки(), КаталогХраненияЛогов); 
			Исключение
			КонецПопытки;		
		КонецПопытки;	
	КонецЕсли;	
...
// ЛОГИРОВАНИЕ
Процедура ЗаписатьВЛог(Запрос, Ответ, Результат, Роутинг, ОписаниеОшибки, КаталогСохраненияЛогов) 		
	...
	ВремяШтампЗаписи = Число(Формат(ВремяЗаписи - дата(1970, 1, 1, 1, 0, 0), "ЧГ=0"));
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) тогда
		...
		LogИмяФайла = КаталогСохраненияЛогов + "error"+"_"+Формат(ВремяЗаписи, "ДФ='dd_MM_yyyy_HH_mm_ss'") + "_1C.log";
		
		ТекущийФайл = Новый Файл(LogИмяФайла);
		n=0;
		Пока ТекущийФайл.Существует() и n<20 цикл
			n=n+1;
			LogИмяФайла = КаталогСохраненияЛогов + "error"+"_"+Формат(ВремяЗаписи, "ДФ='dd_MM_yyyy_HH_mm_ss'") + "_"+n+ "_1C.log";
			ТекущийФайл = Новый Файл(LogИмяФайла);
		КонецЦикла;
		...
		LogДанные.Вставить("date_time",     Формат(ВремяЗаписи,"ДФ='dd.MM.yyyy HH:mm:ss'"));
	...
	Иначе	
		...
		LogИмяФайла = КаталогСохраненияЛогов+LogИмяМетода+LogРоутинг+"_"+Формат(ВремяЗаписи,"ДФ='dd_MM_yyyy_HH_mm_ss'") + ErrorТэг + "_1C.log";
		
		ТекущийФайл = Новый Файл(LogИмяФайла);
		n=0;
		Пока ТекущийФайл.Существует() и n<20 цикл
			n=n+1;
			LogИмяФайла = КаталогСохраненияЛогов+LogИмяМетода+LogРоутинг+"_"+Формат(ВремяЗаписи,"ДФ='dd_MM_yyyy_HH_mm_ss'") + ErrorТэг + "_"+n+ "_1C.log";
			ТекущийФайл = Новый Файл(LogИмяФайла);
		КонецЦикла;
		...
		LogДанные.Вставить("timestamp", ВремяШтампЗаписи);
	...
	ИначеЕсли МетодЗапроса = "PUT" Тогда
		...
		ИначеЕсли ИмяМетода = "" Тогда
			...
			Если ЗначениеЗаполнено(obj_egais_art) Тогда
				...
				//Ответ = ДатаМобайл_HTTP_Логика.CreateEgaisArt(device_id, username, obj_art, obj_egais_art);
		...
		Если ИмяМетода = "" Тогда
			...
			Если ЗначениеЗаполнено(art_id) Тогда
				...
				Если Строка(operation_type) =  "0" Тогда	
					...
					Ответ = ДатаМобайл_HTTP_Логика.GetArtCellsSelect(device_id, username, doc_id, art_id, art_sn);
					
				ИначеЕсли Строка(operation_type) =  "1" Тогда
					
					Роутинг = "GetArtCellsInsert";
					 
					Ответ = ДатаМобайл_HTTP_Логика.GetArtCellsInsert(device_id, username, doc_id, art_id, art_sn);				
	...
	ИначеЕсли МетодЗапроса = "PUT" Тогда
		...
		Если ИмяМетода = "" Тогда
			...
			Иначе
				...
				Ответ = ДатаМобайл_HTTP_Логика.CreateDoc(device_id, username, template_id, AppVersion);
			КонецЕсли;
	...
	Если МетодЗапроса = "GET" Тогда
		...
		Если ИмяМетода = "/arts" Тогда  	
			...
			Иначе
				...
				Ответ = ДатаМобайл_HTTP_Логика.GetEgaisArts(device_id, username);
	...
	ИначеЕсли МетодЗапроса = "POST" Тогда	
		...
		Если ИмяМетода = "" Тогда
			...
			Иначе
				...
				Ответ = ДатаМобайл_HTTP_Логика.SendArtToPrint(device_id, username, art_id, barcode, qty, qty_int, qty_fraction, art_sn, unit_name, is_km, gs1);
