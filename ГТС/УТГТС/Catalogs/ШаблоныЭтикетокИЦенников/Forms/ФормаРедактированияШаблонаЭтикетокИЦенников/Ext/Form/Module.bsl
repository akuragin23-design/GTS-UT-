...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Назначение.СписокВыбора.Добавить(Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляКомплектов);
...
Процедура НапечататьОбразец(Команда)
    ...
    //++ASP_Crow 11.03.2026 000029567 В расширение 
	////////ASP_Дша(000023656)
	ИначеЕсли Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляКомплектов") Тогда

	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Обработка.ПечатьЭтикетокИЦенников", "ЭтикеткаТовары", ПараметрКоманды, Неопределено, ПолучитьПараметрыДляПечатиОбразцаЭтикеткиДляКомплектов());
    ////////ASP_Дша(000023656)
	//--ASP_Crow 11.03.2026 000029567 В расширение 
...
Процедура ОбновитьЗаголовок(Форма)
	...
	//++ASP_Crow 11.03.2026 000029567 В расширение	
	//ASP_Дша(000023656)	
	ИначеЕсли Форма.Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляКомплектов") Тогда
		Форма.Заголовок = НСтр("ru = 'Шаблон этикетки:'") + " " + Наименование;
  	//ASP_Дша(000023656)
  	//--ASP_Crow 11.03.2026 000029567 В расширение
...
Процедура ЗаполнитьДоступныеПоля()
	...
	//++ASP_Crow 11.03.2026 000029567 В расширение  
	////////ASP_Дша(000023656)
 	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляКомплектов Тогда
		СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаТоварыДляКомплектов"); 
	////////ASP_Дша(000023656)
	//--ASP_Crow 11.03.2026 000029567 В расширение
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаУпаковочныхЛистов Тогда
	...
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЦенникДляТоваров
		...
		ИменаПолей.Добавить("Артикул");
		ИменаПолей.Добавить("Состав"); 
		ИменаПолей.Добавить("СрокСлужбы"); 
		ИменаПолей.Добавить("Комплектация");
		//--ASP_Crow 11.03.2026 000029567 В расширение
	...
	ИменаПолей.Добавить("Штрихкод");
		...
		//ИменаПолей.Добавить("Номенклатура.ДополнительныеРеквизиты, Свойства");
		...
		ИменаПолей.Добавить("СтавкаНДС");
        //Комплекты
		ИменаПолей.Добавить("Комплектующая");
		ИменаПолей.Добавить("КоличествоКомплектующих");
		ИменаПолей.Добавить("НомерМеста");
		ИменаПолей.Добавить("КоличествоМестВсего");
		 //Комплекты
	//--ASP_Crow 11.03.2026 000029567 В расширение
...
////////ASP_Дша(000023656)
&НаСервере
Функция ПолучитьПараметрыДляПечатиОбразцаЭтикеткиДляКомплектов()  
	//ASP_Crow 19.03.2026 000029567 В расширение

	ПараметрыПечати = Справочники.ШаблоныЭтикетокИЦенников.ПолучитьПараметрыДляПечатиОбразцаЭтикеткиДляКомплектов(
		Объект.ДляЧего, ТипКода, УникальныйИдентификатор);
	Возврат ДополнитьПараметрыДляПечатиОбразца(ПараметрыПечати);
