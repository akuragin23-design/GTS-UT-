...
//  Структура - Параметры печати
//
Функция ПолучитьПараметрыДляПечатиОбразцаЭтикеткиДляКомплектов(ДляЧего, ТипКода, УникальныйИдентификатор) Экспорт
	...
	ПараметрыПечати.Вставить("Организация", ПолучитьОрганизациюДляПечатиОбразца());  
	////////Комплекты
 	ПараметрыПечати.Вставить("Комплектующая", ПолучитьНаименованиеКомплектацийДляПечатиОбразца());
	ПараметрыПечати.Вставить("КоличествоКомплектующих", ПолучитьКоличествоКомплектацийДляПечатиОбразца());
	//ПараметрыПечати.Вставить("КоличествоУпКомплектующих", ПолучитьКоличествоУпаковокКомплектацийДляПечатиОбразца());
 ////////Комплекты
...
Функция ПолучитьНаименованиеКомплектацийДляПечатиОбразца()
	//ASP_Crow 20.03.2026 000029567 В расширение
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	                      |	Данные.Ссылка КАК Ссылка
	                      |ИЗ
	                      |	Справочник.ВариантыКомплектацииНоменклатуры КАК Данные
	                      |ГДЕ
	                      |	НЕ Данные.ПометкаУдаления");
...
КонецФункции
 ...
 Функция ПолучитьКоличествоКомплектацийДляПечатиОбразца()
	//ASP_Crow 20.03.2026 000029567 В расширение
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
...
Функция ПолучитьКоличествоУпаковокКомплектацийДляПечатиОбразца()
	//ASP_Crow 20.03.2026 000029567 В расширение
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.КоличествоУпаковок;
	Иначе
		Возврат Справочники.ВариантыКомплектацииНоменклатуры.ПустаяСсылка();
	КонецЕсли;
