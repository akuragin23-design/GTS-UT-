...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПередЗаписьюНаСервере(
		...
		ПараметрыЗаписи);
	// Конец ИнтеграцияС1СДокументооборотом
	//ASP заполнение срока гарантии по умолчанию //ASP_Crow 11.03.2026 000029567 В расширение
	Если ТекущийОбъект.СрокГарантииНаТовар=0 
		Тогда ТекущийОбъект.СрокГарантииНаТовар=12;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьПоУмолчаниюНаСервере()
	...
	Объект.Наименование = строка(Объект.Контрагент) + ", с покупателем, " + Объект.ВалютаВзаиморасчетов;
	Объект.ДатаНачалаДействия = ТекущаяДата();   
	Объект.Организация=Справочники.Организации.НайтиПоНаименованию("ООО ""Горторгснаб""");
	Объект.ПорядокРасчетов=Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов;
	Объект.БанковскийСчет=Справочники.БанковскиеСчетаОрганизаций.НайтиПоНаименованию("СБ РФ");
	Объект.Подразделение=ПользователиКлиентСервер.ТекущийПользователь().Подразделение;
	Объект.Менеджер=ПользователиКлиентСервер.ТекущийПользователь();
