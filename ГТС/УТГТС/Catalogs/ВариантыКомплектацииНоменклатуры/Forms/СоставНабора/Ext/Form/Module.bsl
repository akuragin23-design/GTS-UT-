...
&НаКлиенте
Процедура СкопироватьИзДругогоНабора(Команда)
	 Перем ВыбЗнач; 
	 //ASP_Crow 11.03.2026 000029567 В расширение
	...
	Массив=ПолучитьОсновнуюКомпоненту(ВыбЗнач);
	ЗаполнитьОсновнойКомпонент(Массив[0], Массив[1]);
	КонецЕсли;
...
КонецПроцедуры
&НаСервере
Функция ПолучитьОсновнуюКомпоненту(ВыбЗнач)
	//ASP_Crow 11.03.2026 000029567 В расширение
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	Массив = Новый Массив;
	Массив.Добавить(ВыборкаДетальныеЗаписи.НоменклатураОсновногоКомпонента);
	Массив.Добавить(ВыборкаДетальныеЗаписи.ХарактеристикаОсновногоКомпонента);
КонецЦикла;
Возврат Массив;
