...
Процедура Обработчик_Завершения_ЗагрузкиСервер(Адрес, ИмяФайла = "") 
	...
	ИмяВременногоФайла 	= ПолучитьИмяВременногоФайла(Б_ОбщиеПроцедурыИФункцииСервер.ПолучитьРасширениеФайлаПоНаименованию(ИмяФайла));
...
Процедура РазобратьЗагрузитьXMLСервер(СтрокаXML)
	...
	Для каждого ТекСтрока из ТзнДанных Цикл
		...
		Иначе
			...
			Раздел = ПолучитьРазделПоИд(ТекСтрока.Ид);
			Если НЕ ЗначениеЗаполнено(Раздел) Тогда
				РазделОбъект = Справочники.Б_ПользовательскиеГруппыТоваров.СоздатьЭлемент();
			Иначе
				РазделОбъект = Раздел.ПолучитьОбъект();	
			КонецЕсли;
			
			РазделОбъект.Наименование 			= ТекСтрока.Наименование;
			РазделОбъект.ИдентификаторРаздела 	= ТекСтрока.Ид;
			
			Если ИдИмпортируемогоИнфоблока = ТекСтрока.ИдРодителя ИЛИ ТекСтрока.ИдРодителя = "" тогда
				РазделОбъект.Родитель	= Инфоблок;
			Иначе   
				
				Родитель = ПолучитьРазделПоИд(ТекСтрока.ИдРодителя);
			...
			Для каждого ТекТовар из ТекСтрока.Товары Цикл
				РазделОбъект.Товары.Добавить().Номенклатура = XMLЗначение(Тип("СправочникСсылка.Номенклатура") , ТекТовар);
			КонецЦикла;
...
&НаСервере
Функция ПолучитьРазделПоИд(ИдРодителя)
