...
Процедура ПоискПоТелефону(Команда)  
	//ASP_Crow 11.03.2026 000029567 В расширение
	  ИмяОткрываемойФормы = "Обработка.ПоискПоТелефону.Форма.Форма";
	ОткрытьФорму(ИмяОткрываемойФормы, );
	//ПараметрыФормы = Новый Структура;
	//ПараметрыФормы.Вставить("", "");
КонецПроцедуры

//ГТС запрещаем менять менеджера кроме как ему самому или его руководителю 29.06.2020
&НаСервере
Процедура ОсновнойМенеджерПриИзмененииНаСервере() 
	//ASP_Crow 11.03.2026 000029567 В расширение
	...
	СтарыйМенеджер=ВыборкаДетальныеЗаписи.ОсновнойМенеджер;
	КонецЦикла;
	...
	Запрос.УстановитьПараметр("ФизическоеЛицо", СтарыйМенеджер.Подразделение.ТекущийРуководитель);
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	РуководительОтдела=ВыборкаДетальныеЗаписи.Ссылка;
	КонецЦикла;
	ТекущийПользователь=Пользователи.ТекущийПользователь();  
	
	Если (СтарыйМенеджер<>ТекущийПользователь И РуководительОтдела<>ТекущийПользователь) и Не Параметры.Ключ.Пустая() И ЗначениеЗаполнено(СтарыйМенеджер) Тогда
	сообщить("Изменить основного менеджера может только он сам или руководитель его подразделения");
	Объект.ОсновнойМенеджер=СтарыйМенеджер;
	КонецЕсли;         
...
Процедура ОсновнойМенеджерПриИзменении(Элемент) 
	//ASP_Crow 11.03.2026 000029567 В расширение
	ОсновнойМенеджерПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Процедура РассчитатьТекущийЛимитОтгрузки()
	//ASP_Crow 11.03.2026 000029567 В расширение
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Дата", '20200101');
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл     
		КурсСегодня=РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВыборкаДетальныеЗаписи.Валюта, ТекущаяДата());
		ТекущийИспользованныйЛимит=ТекущийИспользованныйЛимит+ВыборкаДетальныеЗаписи.СуммаОстаток*(КурсСегодня.Курс/КурсСегодня.Кратность);
	КонецЦикла;
	
КонецПроцедуры

Процедура ПерезаписатьКонтактныхЛиц();   
	//ASP_Crow 11.03.2026 000029567 В расширение
	...
	Запрос.Текст = 
		...
		|	И КонтактныеЛицаПартнеров.Автор <> &Автор";
	
	Запрос.УстановитьПараметр("Владелец", Объект.Ссылка);
	Запрос.УстановитьПараметр("Автор", Объект.ОсновнойМенеджер);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		КонтактноеЛицо=ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		КонтактноеЛицо.Автор=Объект.ОсновнойМенеджер;
