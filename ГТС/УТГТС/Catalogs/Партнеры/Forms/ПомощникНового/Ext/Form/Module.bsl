...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы И Не ВыполняетсяЗакрытие Тогда
		...
		//ASP. Изменено //ASP_Crow 11.03.2026 000029567 В расширение
...
Процедура ЗаполнитьИтоговуюИнформацию()
	...
	Если (ИспользуютсяТолькоПартнеры Или ДействиеСКонтрагентом = 1) И УказыватьЮридическиеРеквизиты Тогда
		...
		//ASP. Изменено //ASP_Crow 11.03.2026 000029567 В расширение
		ЕстьДубльПоОснРеквизитам = Ложь;//ЕстьДубльПоИННКПП=Ложь;
		//ASP. Изменено
...
Процедура УправлениеДоступностьюДополнительныеСведения(Форма)
	...
	//ASP. Изменено  //ASP_Crow 11.03.2026 000029567 В расширение
	Элементы.ASP_ФактическийАдресРавенЮридическому.Доступность = (Форма.УказыватьЮридическиеРеквизиты ИЛИ Форма.ЭтоКомпания = 1);
	//ASP. ИзмененоЭлементы.ЮридическийАдресРавенФактическому.Доступность = (УказыватьЮридическиеРеквизиты);
...
Функция КорректноВведенНомерТелефона(СписокЗначенияПолей)
	...
	КонецЦикла;
	//ASP. Изменено  ASP_Crow 11.03.2026 000029567 В расширение
	Если СтрДлина(НомерТелефона) = 1 Тогда
		Возврат Истина; 
	КонецЕсли;
	//ASP. Изменено
...
Процедура ВыполнитьНеобходимыеДействияПриПереходеПоСтраницам(Знач Оповещение, Команда)
	...
	Если Команда.Имя = "Далее" Тогда
		...
		ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаДополнительныеСведения Тогда
			...
			Элементы.БИКБанкаДляРасчетов.Доступность = Ложь;
			Элементы.КоррСчетБанкаДляРасчетов.Доступность = Ложь;
			ИспользоватьТекстКорреспондента = Ложь;			
...
Функция ДополнительныеСведенияЗаполненыНеправильно()
	...
	КонецЕсли;
	//ASP. Изменено //ASP_Crow 11.03.2026 000029567 В расширение
	Если ASP_КодКлиента >0 и ASP_КодКлиента <=99 Тогда
		...
		Если НЕ ЗначениеЗаполнено(КаналПервичногоИнтереса) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать канал первичного интереса'"),ИспользуютсяТолькоПартнеры,"КаналПервичногоИнтереса");
			Отказ = Истина;	 

		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ИсточникПервичногоИнтереса) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать источник первичного интереса'"),ИспользуютсяТолькоПартнеры,"ИсточникПервичногоИнтереса");
			Отказ = Истина;	 
...
Процедура ЗаписатьПартнераИСвязанныеДанные(СписокСозданных)
	...
	Если ДействиеПоРезультатамНайдено <> 0 Тогда
		...
		//ASP. Изменено //ASP_Crow 11.03.2026 000029567 В расширение
		ОбъектПартнер.КодКлиента = ASP_КодКлиента;
		//ASP. Изменено
...
Процедура ЗаполнитьМаркетинговоеМероприятиеПоУмолчаниюСервер()
	...
	Если КаналПервичногоИнтереса = ПланыВидовХарактеристик.КаналыРекламныхВоздействий.НайтиПоНаименованию("Старый клиент") 
		...
		ИсточникПервичногоИнтереса = Справочники.МаркетинговыеМероприятия.НайтиПоНаименованию("Старый клиент");	
	КонецЕсли;
...
&НаКлиенте
Процедура СкопироватьИзТелефонаПартнера(Команда) 
	//ASP_Crow 11.03.2026 000029567 В расширение
	ТелефонКЛ=ТелефонПартнера;
	ТелефонКЛЗначенияПолей=ТелефонПартнераЗначенияПолей;
	
КонецПроцедуры


&НаКлиенте
Процедура НаименованиеБанковскогоСчетаНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	СтандартнаяОбработка = Ложь;
	
	СформироватьАвтоНаименование();
	ВыбранныйЭлемент = ВыбратьИзСписка(Элементы.НаименованиеБанковскогоСчета.СписокВыбора, Элемент);
	Если ВыбранныйЭлемент <> Неопределено Тогда
		НаименованиеБанковскогоСчета = ВыбранныйЭлемент.Значение;
	КонецЕсли;
КонецПроцедуры
...
Функция СформироватьАвтоНаименование()
	...
	Элементы.НаименованиеБанковскогоСчета.СписокВыбора.Очистить();
	...
	Элементы.НаименованиеБанковскогоСчета.СписокВыбора.Добавить(СтрокаНаименования);
	
	СтрокаНаименовЯания = ?(ЗначениеЗаполнено(Банк), Строка(Банк), "")
		+ " (" + Строка(ВалютаДенежныхСредств) + ")";
	СтрокаНаименования = Лев(СтрокаНаименования, 150);
...
&НаКлиенте
Процедура ЗаполнитьКПППоИНН(Команда)  
	//ASP_Crow 11.03.2026 000029567 В расширение
	ЗаполнитьКППпоИНННАСервере(ИНН, КПП);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьКППпоИНННАСервере(Знач ИНН, КПП) Экспорт 
	//ASP_Crow 11.03.2026 000029567 В расширение
