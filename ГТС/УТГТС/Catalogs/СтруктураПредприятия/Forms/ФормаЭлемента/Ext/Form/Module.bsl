...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ASP_КартБаннер = ПолучитьНавигационнуюСсылку(ТекущийОбъект.Ссылка, "Баннер");
	///
КонецПроцедуры
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//ллл  ASP_Crow 11.03.2026 000029567 В расширение
	Если ЭтоАдресВременногоХранилища(ASP_КартБаннер) Тогда
		ДвоичныеДанные = ПолучитьИзВременногоХранилища(ASP_КартБаннер);
		ТекущийОбъект.Баннер = Новый ХранилищеЗначения(ДвоичныеДанные, Новый СжатиеДанных());
	КонецЕсли; 
	//ллл
	Если ЭтоАдресВременногоХранилища(ASP_КартБаннер) Тогда
		ДвоичныеДанные = ПолучитьИзВременногоХранилища(ASP_КартБаннер);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)  
	...
	/////ASP_Crow 11.03.2026 000029567 В расширение
	Если ЭтоАдресВременногоХранилища(ASP_КартБаннер) Тогда
		УдалитьИзВременногоХранилища(ASP_КартБаннер);
	КонецЕсли;
	ASP_КартБаннер = ПолучитьНавигационнуюСсылку(ТекущийОбъект.Ссылка, "Баннер");
КонецПроцедуры

&НаКлиенте
Процедура УбратьБаннер(Команда)
	//ASP_Crow 11.03.2026 000029567 В расширение
	УбратьБаннерНаСервере();
	ASP_КартБаннер="";
КонецПроцедуры


&НаСервере
Процедура УбратьБаннерНаСервере() 
	//ASP_Crow 11.03.2026 000029567 В расширение
	ЭтотОбъект = РеквизитФормыВЗначение("Объект");
	ЭтотОбъект.Баннер=Неопределено;
	ЭтотОбъект.Записать();
КонецПроцедуры
