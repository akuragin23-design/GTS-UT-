...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//++ASP_Трунькин 000026541 21/03/25 //ASP_Crow 11.03.2026 000029567 В расширение
	Если Объект.ИспользуетсяЛКП Тогда
		ИспользуетсяЛКП();//действия для записи в ЛКП
	КонецЕсли;	
	//--ASP_Трунькин 000026541 21/03/25
...
&НаСервере
Процедура ИспользуетсяЛКПНаСервере(Почта, телефон) 
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	//++ASP ТрунькинБ 000026656 (Пункт 2) 07/04/25	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ASP_ЛКПКонтрагенты.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ASP_ЛКПКонтрагенты КАК ASP_ЛКПКонтрагенты
	|ГДЕ
	|	ASP_ЛКПКонтрагенты.Контрагент = &Контрагент";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда 	
		...
		Элемент.Контрагент = Объект.Ссылка;
		Элемент.Менеджер = Объект.Партнер.ОсновнойМенеджер;
		Элемент.ПочтаМенеджера = Почта;
		Элемент.ТелефонМенеджера = Телефон;
		Элемент.ВидВалютнойЦены = Объект.ВидВалютнойЦены;
		Элемент.Лимит = Объект.ЛимитОтгрузки;
		Элемент.ЛимитРезерва = Объект.ЛимитРезерва;
		Элемент.ОсновнойСклад = Объект.ОсновнойСклад;
		Элемент.ИНН = Объект.ИНН;
		Элемент.КонтрагентЮрНаименование = Объект.НаименованиеПолное;
		Элемент.Записать();
		Сообщить("Элемент " + Элемент.Наименование + ", в справочнике ASP ЛКП (личный кабинет покупателя), изменен!");
	Иначе
		НовыйЭлемент = Справочники.ASP_ЛКПКонтрагенты.СоздатьЭлемент();
		НовыйЭлемент.Наименование = Объект.Наименование;
		НовыйЭлемент.Партнер = Объект.Партнер;
		НовыйЭлемент.Контрагент = Объект.Ссылка;
		НовыйЭлемент.Менеджер = Объект.Партнер.ОсновнойМенеджер;
		НовыйЭлемент.ПочтаМенеджера = Почта;
		НовыйЭлемент.ТелефонМенеджера =Телефон;
		НовыйЭлемент.ВидВалютнойЦены = Объект.ВидВалютнойЦены;
		НовыйЭлемент.Лимит = Объект.ЛимитОтгрузки;
		НовыйЭлемент.ЛимитРезерва = Объект.ЛимитРезерва;
		НовыйЭлемент.ОсновнойСклад = Объект.ОсновнойСклад;
		НовыйЭлемент.ИНН = Объект.ИНН;
		НовыйЭлемент.КонтрагентЮрНаименование = Объект.НаименованиеПолное;
		НовыйЭлемент.Записать();
		Сообщить("Элемент " + НовыйЭлемент.Наименование + ", в справочнике ASP ЛКП (личный кабинет покупателя), успешно создан!");
	КонецЕсли;
	//--ASP ТрунькинБ 000026656 (Пункт 2) 07/04/25
...
&НаКлиенте
Процедура ИспользуетсяЛКП()  
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	ПараметрыФ = новый структура ("Ключ",ОсновнойМенеджер());
	Форма = ПолучитьФорму("Справочник.Пользователи.Форма.ФормаЭлемента",ПараметрыФ);
	Почта = Форма.КонтактнаяИнформацияПолеEmailПользователя;
	Телефон = Форма.КонтактнаяИнформацияПолеТелефонПользователя;
	
	ИспользуетсяЛКПНаСервере(Почта, Телефон);
	
КонецПроцедуры

&НаСервере
Функция ОсновнойМенеджер()  
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	Возврат Объект.Партнер.ОсновнойМенеджер;
