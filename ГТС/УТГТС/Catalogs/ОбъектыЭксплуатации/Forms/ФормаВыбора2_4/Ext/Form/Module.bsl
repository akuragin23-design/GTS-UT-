...
Процедура ЗаполнитьФиксированныеОтборы()
	...
	ИначеЕсли ОтборСписка.Свойство("ВидНалога") Тогда
		//Заголовок = ВнеоборотныеАктивыЛокализация.ЗаголовокФормыВыбораОСПоВидуНалога(ОтборСписка.ВидНалога, Параметры.ДатаСведений); //ASP_Crow 06.03.2026 000029567 часть методов отсутствует убираю
	КонецЕсли; 
	...
	Если ТипЗнч(ЗначениеОтбора) = Тип("ФиксированныйМассив") 
		...
		Если ИмяОтбора = "Состояние"
			...
			Если ЗначениеОтбора.Количество() > 1 Тогда
				...
				//ЗначениеОтбора = ВнеоборотныеАктивыСлужебный.ФиксированныйМассивВСписок(ЗначениеОтбора); //ASP_Crow 06.03.2026 000029567 часть методов отсутствует убираю
			Иначе
