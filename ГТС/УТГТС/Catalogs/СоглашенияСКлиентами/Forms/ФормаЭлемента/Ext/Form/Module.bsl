...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ГТС 21.06.2024 разрешение работы с одобрением соглашения только по праву управления ценовой колонкой
	Если РольДоступна("ASP_РазрешитьЦенуНижеКолонки") Тогда Элементы.ASP_Одобрено.ТолькоПросмотр=Ложь
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//++ASP_ТрунькинБ 000026541 26/03/25 ASP_Crow 11.03.2026 000029567 В расширение 
	СозданиеЭлементаВСправочникеДЛК();
	//++ASP_ТрунькинБ 000026541 26/03/25
...
&НаКлиенте
Процедура СозданиеЭлементаВСправочникеДЛК()  
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	//Если НЕ Объект.Ссылка.Пустая()Тогда
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		Если ИспользуетсяЛКП(Объект.Контрагент) Тогда 							// смотрим в карточке контрагента используется ли ЛКП  Справочники.СкидкиНаценки.НайтиПоНаименованию("Округление: до 1 руб. По арифметическим правилам с учетом ручных скидок без условий")
			ЗначениеСкидки = ПолучитьСкидку(Объект.Ссылка);
			Если ЗначениеСкидки.Количество() > 0 Тогда    						//если скидки нет в соглашении, то и не чего в справочник ЛКД не записываем
				УдалитьЭлементСправочника(Объект.Ссылка, ЗначениеСкидки);       //удаляем элемент в справочнике, если скидки по соглашению больше нет
				Для Каждого Скидка из ЗначениеСкидки Цикл
					НоменклатураОтбора = ПолучитьНоменклатуруОтбора(Скидка);
					СоздатьЭлементВЛКД(Скидка, НоменклатураОтбора);	
				КонецЦикла;
			Иначе
				УдалитьЭлементСправочника(Объект.Ссылка); 						//если действующих скидок не нашел, тогда удаляем из справочника существующие скидки по данному соглашению
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//КонецЕсли;	
...
&НаСервереБезКонтекста
Функция ПолучитьНоменклатуруОтбора(СкидкаСсылка)
	//ASP_Crow 11.03.2026 000029567 В расширение
	МассивВидНоменклатур = Новый Массив;
	ТЧ = СкидкаСсылка.ХранилищеНастроекКомпоновкиДанных.Получить().ОтборПоНоменклатуре.Отбор.Элементы[0].ПравоеЗначение;
	Для каждого Строка из ТЧ Цикл
		МассивВидНоменклатур.Добавить(Строка.значение);	
	КонецЦикла; 
	Возврат  МассивВидНоменклатур;
	
КонецФункции

 &НаСервереБезКонтекста
Функция ПолучитьСкидку(Ссылка)
	//ASP_Crow 11.03.2026 000029567 В расширение
	...
	Запрос.Текст = 
		...
		|			,
		|			Источник = &Источник
		|				И СкидкаНаценка.Наименование <> ""Округление: до 1 руб. По арифметическим правилам с учетом ручных скидок без условий"") КАК ДействиеСкидокНаценокСрезПоследних";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если ВыборкаДетальныеЗаписи.Статус = Перечисления.СтатусыДействияСкидок.Действует Тогда
			МассивСкидок.Добавить(ВыборкаДетальныеЗаписи.СкидкаНаценка);
		КонецЕСли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ИспользуетсяЛКП(Контрагент)
	//ASP_Crow 11.03.2026 000029567 В расширение
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ИспользуетсяЛКП");
	
КонецФункции


&НаСервереБезКонтекста
Процедура УдалитьЭлементСправочника(Соглашение, Скидка = Неопределено) 
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ASP_ЛКД.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ASP_ЛКД КАК ASP_ЛКД
		|ГДЕ
		|	ASP_ЛКД.Соглашение = &Соглашение
		|	И ВЫБОР
		|			КОГДА &ОтборПоСсылкеНаСкидку = ИСТИНА
		|				ТОГДА НЕ ASP_ЛКД.СсылкаНаСкидку В (&СсылкаНаСкидку)
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ";
	
	Запрос.УстановитьПараметр("Соглашение", Соглашение);
	Запрос.УстановитьПараметр("СсылкаНаСкидку", Скидка);
	Запрос.УстановитьПараметр("ОтборПоСсылкеНаСкидку", Скидка <> Неопределено);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл    
		...
		Сообщить("Элемент - " + ЭлементОбъект.Наименование + ", удален из справочника ЛКД, так как действующих скидок нет!");
...
&НаСервереБезКонтекста
Процедура УдалитьЭлементыСправочника(СсылкаСоглашение)  
	//ASP_Crow 11.03.2026 000029567 В расширение
	...
	Запрос.УстановитьПараметр("Соглашение", СсылкаСоглашение);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
		...
		// проверка использования // ссылочности
		ЭлементОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		Сообщить("Элемент - " + ЭлементОбъект.Наименование + ", удален из справочника ЛКД, так как действующих скидок нет!");
		ЭлементОбъект.Удалить();
...
&НаСервере
Процедура СоздатьЭлементВЛКД(Скидка, Номенклатура) 
	//ASP_Crow 11.03.2026 000029567 В расширение
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ASP_ЛКД.Ссылка КАК Ссылка,
		|	ASP_ЛКД.ВидНоменклатуры КАК ВидНоменклатуры,
		|	ASP_ЛКД.СсылкаНаСкидку КАК СсылкаНаСкидку
		|ИЗ
		|	Справочник.ASP_ЛКД КАК ASP_ЛКД
		|ГДЕ
		|	ASP_ЛКД.ВидНоменклатуры В(&ВидНоменклатуры)
		|	И ASP_ЛКД.Контрагент = &Контрагент
		|	И ASP_ЛКД.СсылкаНаСкидку = &СсылкаНаСкидку
		|	И ASP_ЛКД.Соглашение = &Соглашение";
	
	Запрос.УстановитьПараметр("ВидНоменклатуры", Номенклатура);
	...
	Для каждого строка из Номенклатура Цикл
		Отбор = Новый Структура("ВидНоменклатуры", Строка);
		СтрокиОтбора = РезультатЗапроса.НайтиСтроки(Отбор);
		Если СтрокиОтбора.Количество() = 0 Тогда
			НовыйЭлемент = Справочники.ASP_ЛКД.СоздатьЭлемент();
			НовыйЭлемент.Наименование = Строка(Объект.Контрагент) + " " + Строка.Наименование ;
			НовыйЭлемент.Партнер = Объект.Партнер;
			НовыйЭлемент.Контрагент = Объект.Контрагент;
			НовыйЭлемент.Скидка = Скидка.ЗначениеСкидкиНаценки;
			НовыйЭлемент.ВидНоменклатуры = Строка.Ссылка;
			НовыйЭлемент.КонтрагентЮрНаименование = Объект.Контрагент.НаименованиеПолное;
			НовыйЭлемент.СсылкаНаСкидку = Скидка;
			НовыйЭлемент.Соглашение = Объект.Ссылка;
			НовыйЭлемент.Записать();
			ОбщегоНазначения.СообщитьПользователю("Элемент " + НовыйЭлемент.Наименование + ", в справочнике ASP ЛКД (личный кабинет дилера), успешно создан!");
		Иначе
			Для каждого строкаОтбора из СтрокиОтбора Цикл
				ЭлементОтбора = строкаОтбора.Ссылка.ПолучитьОбъект();
				ЭлементОтбора.Наименование = Строка(Объект.Контрагент) + " " + строка.Наименование; 
				ЭлементОтбора.Партнер = Объект.Партнер;
				ЭлементОтбора.Контрагент = Объект.Контрагент;
				ЭлементОтбора.Скидка = Скидка.ЗначениеСкидкиНаценки;					
				ЭлементОтбора.ВидНоменклатуры = Строка.Ссылка;
				ЭлементОтбора.КонтрагентЮрНаименование = Объект.Контрагент.НаименованиеПолное;
				ЭлементОтбора.СсылкаНаСкидку = Скидка;
				ЭлементОтбора.Соглашение = Объект.Ссылка;
				ЭлементОтбора.Записать();
				ОбщегоНазначения.СообщитьПользователю("Элемент " + ЭлементОтбора.Наименование + ", в справочнике ASP ЛКД (личный кабинет дилера), изменен!");
			КонецЦикла;
		КонецЕсли;
	...
	//Для Каждого элемента Из Номенклатура Цикл
	//	
	//	Ссылка = Справочники.ASP_ЛКД.НайтиПоНаименованию(Строка(Объект.Контрагент) + " " + элемента.Наименование); //увеличить количество символов до 150 в справочнике ЛКД   
	//	
	//	Если НЕ Ссылка.Пустая() Тогда 
	//		
	//		Элемент = Ссылка.ПолучитьОбъект();
	//		Элемент.Наименование = Строка(Объект.Контрагент) + " " + элемента.Наименование; 
	//		Элемент.Партнер = Объект.Партнер;
	//		Элемент.Контрагент = Объект.Контрагент;
	//		Элемент.Скидка = Скидка.ЗначениеСкидкиНаценки;					
	//		Элемент.ВидНоменклатуры = элемента.Ссылка;
	//		Элемент.КонтрагентЮрНаименование = Объект.Контрагент.НаименованиеПолное;
	//		Элемент.СсылкаНаСкидку = Скидка;
	//		Элемент.Соглашение = Объект.Ссылка;
	//		Элемент.Записать();
	//		Сообщить("Элемент " + Элемент.Наименование + ", в справочнике ASP ЛКД (личный кабинет дилера), изменен!");
	//		
	//	Иначе
	//		
	//		НовыйЭлемент = Справочники.ASP_ЛКД.СоздатьЭлемент();
	//		НовыйЭлемент.Наименование = Строка(Объект.Контрагент) + " " + элемента.Наименование ;
	//		НовыйЭлемент.Партнер = Объект.Партнер;
	//		НовыйЭлемент.Контрагент = Объект.Контрагент;
	//		НовыйЭлемент.Скидка = Скидка.ЗначениеСкидкиНаценки;
	//		НовыйЭлемент.ВидНоменклатуры = элемента.Ссылка;
	//		НовыйЭлемент.КонтрагентЮрНаименование = Объект.Контрагент.НаименованиеПолное;
	//		НовыйЭлемент.СсылкаНаСкидку = Скидка;
	//		НовыйЭлемент.Соглашение = Объект.Ссылка;
	//		НовыйЭлемент.Записать();
	//		Сообщить("Элемент " + НовыйЭлемент.Наименование + ", в справочнике ASP ЛКД (личный кабинет дилера), успешно создан!");
	//					
	//	КонецЕсли;
	//КонецЦикла;
КонецПроцедуры
