...
Процедура ПередЗаписью(Отказ)
	...
	//ASP_Hlopkov
	АрхивДоп = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Архивная позиция");
	Если НЕ АрхивДоп = Неопределено Тогда 	
		АрхивСтрока = ДополнительныеРеквизиты.Найти(АрхивДоп, "Свойство");
		
		ASP_АрхивнаяПозиция = ЗначениеЗаполнено(АрхивСтрока) И АрхивСтрока.Значение;     
	//ASP_Hlopkov
	КонецЕсли;
	//ASP_Dante 18/12/19
	Если Не Код = "0-000005135" И ЗначениеЗаполнено(Б_Идентификатор) Тогда
		Б_Идентификатор = "";
	КонецЕсли;
	//ASP_Dante 18/12/19
	
	//++ASP_Crow 21.03.2024 000021639 При записи номенклатуры будет проверяться доп реквизит
	СвВо_ВыгружатьНаСайт = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Выгружать на сайт");
	ДопРекв_Выгружать = ДополнительныеРеквизиты.Найти(СвВо_ВыгружатьНаСайт);
	Если ДопРекв_Выгружать = Неопределено Тогда
		ВыгружатьНаСайт = Ложь;
	Иначе
		ВыгружатьНаСайт = ДопРекв_Выгружать.Значение;
	КонецЕсли;
	//--ASP_Crow 21.03.2024 000021639     
	//--//ASP_Crow 11.03.2026 000029567 В расширение
...
Процедура ПриКопировании(ОбъектКопирования)
	//++//ASP_Crow 11.03.2026 000029567 В расширение
	Б_Идентификатор = "";//ASP_Dante 03/09/19
	Артикул=""; 
	//--//ASP_Crow 11.03.2026 000029567 В расширение
