...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//ГТС проверка уникальности
	Если Не ПустаяСтрока(Объект.Артикул) Тогда 
		ИмяОбъекта = "Номенклатура";
		СтрРеквизитов = Новый Структура; //
		СтрРеквизитов.Вставить("Артикул", Объект.Артикул);
		Если Не Альтерлого.ПрошелКонтрольУникальностиСправочникаПоРеквизиту(ИмяОбъекта, СтрРеквизитов, Объект.Ссылка) Тогда
			Сообщить("Подобный элемент справочника /" + Объект.Артикул + "/ существует!
...
Процедура СвернутьРазвернутьГруппу(Элемент) 
	...
	ИзменитьСвернутостьГруппы(ИмяГруппыСвернутьРазвернуть, Истина);
...
// Альтер ЛОГО+
&НаСервере
Процедура РассчитатьВесПоКомплектующимНаСервере()
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СУММА(КомплектующиеНоменклатуры.Комплектующая.Вес * КомплектующиеНоменклатуры.Количество) КАК Вес
	|ИЗ
	...
	Если Результат.Пустой() Тогда
		Сообщить("Не удалось рассчитать вес, проверьте состав комплекта и веса комплектующих товаров!");
	Иначе
		Выборка = Результат.Выбрать();
		Выборка.Следующий();
		Если Выборка.Вес = 0 или Выборка.Вес = Null Тогда
			Сообщить("Не удалось рассчитать вес, проверьте состав комплекта и веса комплектующих товаров!");
			Возврат;
		КонецЕсли;	
...
&НаКлиенте
Процедура ВесОткрытие(Элемент, СтандартнаяОбработка)
	//ASP_Crow 11.03.2026 000029567 В расширение
	СтандартнаяОбработка = Ложь;
	РассчитатьВесПоКомплектующимНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура СоответствиеНоменклатурыНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка) 
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	Профиль = ПрофильКомплектующего();
	Если Профиль = "" тогда	
		Подстрока = Объект.Наименование;
	Иначе
		Позиция = Найти(Объект.Наименование, Профиль);
		Подстрока = Сред(Объект.Наименование, 1, Позиция+СтрДлина(Профиль)-1);
	КонецЕсли;
	
	//
	Если Найти(Объект.Наименование, "ГТС")>0 тогда
		Заменяемое = "ГТС";
		Замена = "СЕТ";
	Иначе
		возврат;
	...
	//
	Подстрока = СтрЗаменить(Подстрока, Заменяемое, Замена);	
	Если Сред(Объект.Наименование, СтрДлина(Объект.Наименование), 1)<>")" тогда
		ПодстрокаОкончания = Сред(Объект.Наименование, СтрДлина(Объект.Наименование)-4, 4);
		ПодСтрока = Подстрока + "%" + ПодстрокаОкончания;
	КонецЕсли;
	
	//
	Вариант = НоменклатураПоПодстрокеНаименования(Подстрока);
	Элемент.СписокВыбора.Очистить();
	Для каждого е из Вариант цикл
		Элемент.СписокВыбора.Добавить(е);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Функция НоменклатураПоПодстрокеНаименования(Подстрока)
	//ASP_Crow 11.03.2026 000029567 В расширение
...
Процедура Цены(Команда)    
	...
	ПараметрыФормы.Вставить("Номенклатура", ТекЭл);
...
КонецФункции
   &НаКлиенте
Функция ПрофильКомплектующего()
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	ПозицияПервойСкобки = Найти(Объект.Наименование, "(");
	ПозицияВторойСкобки = Найти(Объект.Наименование, ")");
	КоличествоСимволов = ПозицияВторойСкобки-ПозицияПервойСкобки-1;
	ПолныйТекст = Сред(Объект.Наименование, ПозицияПервойСкобки+1, КоличествоСимволов);
	Массив = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПолныйТекст, ".");
	
	Если Массив.Количество()<3 тогда 
		возврат "";
	Иначе
		Текст = Массив[0]+"."+Массив[1]+"."+Массив[2];
	КонецЕсли;
