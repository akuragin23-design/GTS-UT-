...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Запрос.УстановитьПараметр("ЭтоНовый",    ?(ДополнительныеСвойства.Свойство("СвойстваДокумента"),ДополнительныеСвойства.СвойстваДокумента.ЭтоНовый, ДополнительныеСвойства.ЭтоНовый));
	Запрос.МенеджерВременныхТаблиц = ?(ДополнительныеСвойства.Свойство("МенеджерВременныхТаблиц"), ДополнительныеСвойства.МенеджерВременныхТаблиц, ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц);
	Запрос.Текст =
	"ВЫБРАТЬ
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если Не ТипЗнч(Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ЗаказПереработчику") и Не ТипЗнч(Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ПоступлениеОтпереработчика") и Не ТипЗнч(Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ПоступлениеОтпереработчика")Тогда
	...
	иначе
		СтруктураВременныеТаблицы.Вставить("ДвиженияЗаказыПоставщикамИзменение", ЕстьИзменения);
	конецЕсли;
	Если ЕстьИзменения
