...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	//++ASP_Crow 02.06.2025 000027079 Попробуем подменить данные в наборе записей для поступления
	// Мы знаем что некорректное движение это движение расхода
	Если  ТипЗнч(ЭтотОбъект.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ПоступлениеТоваровНаСклад") Тогда
		ЕстьИзменения = ложь;
		ТаблицаЗапасов = ЭтотОбъект.Отбор.Регистратор.Значение.ВидыЗапасов.Выгрузить();
		ТаблицаОбработки = ЭтотОбъект.Выгрузить();
		Для каждого СтрТаб из ТаблицаЗапасов Цикл
			ОтборСтрук = новый Структура;
			ОтборСтрук.Вставить("ВидДвижения", ВидДвиженияНакопления.Расход);
			ОтборСтрук.Вставить("АналитикаУчетаНоменклатуры", СтрТаб.АналитикаУчетаНоменклатуры);
			
			СтрокиОтбора = ТаблицаОбработки.НайтиСтроки(ОтборСтрук);
			Для каждого СтрОтб из СтрокиОтбора Цикл
				ЕстьИзменения = Истина;
				СтрОтб.АналитикаУчетаНоменклатуры = СтрТаб.АналитикаУчетаНоменклатурыТоварыУПартнеров;	
			КонецЦикла;
		КонецЦикла;
		Если ЕстьИзменения Тогда
			ЭтотОбъект.Загрузить(ТаблицаОбработки);
		КонецЕсли;
		
	КонецЕсли;
	//--ASP_Crow 02.06.2025 000027079
