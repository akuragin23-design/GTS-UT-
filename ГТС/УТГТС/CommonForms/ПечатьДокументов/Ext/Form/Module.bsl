...
//ASP_Mental 2018-12-13 +++ 
&НаКлиенте
Процедура ВыборПринтераПриЗавершении(Результат, ДополнительныеПараметры) Экспорт 
	//ASP_Crow 11.03.2026 000029567 В расширение
	
	Сч = ЭтотОбъект.НастройкиПечатныхФорм.Количество();
	Для Й = 1 По Сч Цикл
		//ASP__A.Zhel 21.01.2019
		//Если не создана транспортная накладная то печатных форм меньше чем Объектов печати
		Попытка
			ЭтотОбъект["ПечатнаяФорма"+Й].ИмяПринтера = Результат;			
		Исключение
			Продолжить;
		КонецПопытки;
		//__ASP
	КонецЦикла;
	ТекущаяПечатнаяФорма.ИмяПринтера = Результат;	
КонецПроцедуры	
//ASP_Mental 2018-12-13 ---

//ASP_Mental 2018-12-13 +++
&НаКлиенте
Процедура ASP_ВыборПринтера(Команда)
	//ASP_Crow 11.03.2026 000029567 В расширение
	Оповещение = Новый ОписаниеОповещения("ВыборПринтераПриЗавершении", ЭтотОбъект);
	ОткрытьФорму("РегистрСведений.НастройкиПечатиОбъектов.Форма.ASP_ФормаВыбораПринтеров",,ЭтаФорма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
//ASP_Mental 2018-12-13 ---
#КонецОбласти
