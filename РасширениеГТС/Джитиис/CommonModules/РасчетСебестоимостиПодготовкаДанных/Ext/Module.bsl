...
Процедура асп_ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	#Вставка
	|			И Декларация.УчетВСебестоимости
	#КонецВставки
	|	ГДЕ
	...
	#Вставка
	|		И (ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ТаможеннаяДекларацияИмпорт) и Т.Регистратор.УчетВСебестоимости)
	#КонецВставки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		И Т.ИдентификаторСтроки = """"
	#Вставка
	|		И ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ТаможеннаяДекларацияИмпорт) ТОГДА Т.Регистратор.УчетВСебестоимости КОНЕЦ 
	#КонецВставки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	#Вставка
	...
	РасшифровкаКодовОшибок.Вставить(78, НСтр("ru='Не заполнен идентификатор строки в движениях документа ""Таможенная декларация на импорт"" или ""Заявление о ввозе товаров из ЕАЭС"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(90, НСтр("ru = 'Аналитика финансового учета в расходом движении не соответствует партии производства.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
...
//-- НЕ УТ
	...
	#Вставка
	//ASP_DevP закомментировал
	//Запрос.УстановитьПараметр("ТипДокументаИмпорта", ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Обработка.УправлениеСистемойВзаимодействия"));
    #КонецВставки
...
&ИзменениеИКонтроль("ТекстЗапросаВключениеУчетаСебестоимости")
Функция асп_ТекстЗапросаВключениеУчетаСебестоимости()
	Возврат "
		...
		#Вставка
		//ASP_DevP закомментировал
		|		И ( ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ТоварыОрганизаций.Регистратор) = ТИП(Документ.ТаможеннаяДекларацияИмпорт) ТОГДА ТоварыОрганизаций.Регистратор.УчетВСебестоимости КОНЕЦ)
		#КонецВставки
		//++ НЕ УТ
		...
		#Вставка
		//ASP_DevP закомментировал
		|		И Выбор Когда ТИПЗНАЧЕНИЯ(ТоварыКОформлениюДокументовИмпорта.Регистратор) = ТИП(Документ.ТаможеннаяДекларацияИмпорт) ТОГДА ТоварыКОформлениюДокументовИмпорта.Регистратор.УчетВСебестоимости КОНЕЦ
		#КонецВставки
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		...
		#Вставка
		//ASP_DevP закомментировал
		|		И ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ДокументыСДвижениями.Регистратор) = ТИП(Документ.ТаможеннаяДекларацияИмпорт) ТОГДА ДокументыСДвижениями.Регистратор.УчетВСебестоимости КОНЕЦ
		#КонецВставки
		|
		// Исключаем из ошибок возвраты и сторно возвратов, многооборотной тары, операция - "Возврат от комиссионера (2.0)"
